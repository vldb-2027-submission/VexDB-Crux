...
function check_pkg() {
  ...
  bin_name="vexdb-Lite.*-${kernel}-${platform_arch}"
...
function start_dbnode() {
  ...
  log "start vexdb by cmd: $start_cmd"
...
function check_upgrade_config() {
  ...
  # guc output from vexdb or gauss is diff
...
function rollback_bin() {
  ...
  if [[ "$current_step" -lt 3 ]]; then
    ...
    if ! start_dbnode; then
      die "Start vexdb failed" ${err_rollback_bin}
    fi
    record_step 2
...
function upgrade_pre_step2() {
  ...
  if [[ "$big_cfg" == "True" ]]; then
    if ! check_db_process; then
      die " Vexdb isnot running" ${err_upgrade_pre}
    fi
    if ! reload_upgrade_config upgrade_mode 2; then
...
function upgrade_bin_step4() {
  ...
  query_dn_role
  if ! stop_dbnode; then
    die "Stop vexdb failed" ${err_upgrade_bin}
  fi
