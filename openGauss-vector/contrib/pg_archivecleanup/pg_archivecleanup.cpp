...
static void CleanupPriorWALFiles(void)
{
    ...
    if ((xldir = opendir(archiveLocation)) != NULL) {
        while ((xlde = readdir(xldir)) != NULL) {
             ...
             */
            if (strlen(walfile) == XLOG_DATA_FNAME_LEN && strspn(walfile, "0123456789ABCDEF") == XLOG_DATA_FNAME_LEN &&
                ...
                if (dryrun) {
                    ...
                    printf("%s\n", WALFilePath);
                    if (debug)
                        fprintf(stderr, "%s: file \"%s\" would be removed\n", progname, WALFilePath);
                    continue;
                }
...
{
    printf("%s removes older WAL files from vexdb archives.\n\n", progname);
    printf("Usage:\n");
...
#else
    printf("\nReport bugs to community@vexdb.org> or join vexdb community <https://vexdb.org>.\n");
#endif
}
