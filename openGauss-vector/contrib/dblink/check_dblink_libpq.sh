...
function database_install()
{
	...
	delete
        ...
        echo "vexdb start success，the port is $host_port"
    fi
}
...
function create_sql()
{
    cp sql/dblink_libpq.tmp sql/dblink_libpq.sql
    sed -i "s/portIp/$host_port/g" sql/dblink_libpq.sql
    sed -i "s/userName/$user_name/g" sql/dblink_libpq.sql
    cp expected/dblink_libpq.tmp expected/dblink_libpq.out
    sed -i "s/portIp/$host_port/g" expected/dblink_libpq.out
    sed -i "s/userName/$user_name/g" expected/dblink_libpq.out
}
...
function run_check()
{
    ...
    gsql -d regression -p $host_port -a <  sql/dblink_libpq.sql > result/dblink_libpq.out 2>&1
    diff -u result/dblink_libpq.out expected/dblink_libpq.out > diff.log
...
{
    rm -rf test_dblink
    rm -rf sql/dblink_libpq.sql
    rm -rf expected/dblink_libpq.out
}
