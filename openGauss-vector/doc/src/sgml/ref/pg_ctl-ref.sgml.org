<!--
$PostgreSQL: pgsql/doc/src/sgml/ref/gs_ctl-ref.sgml,v 1.51.2.1 2010/08/17 04:37:19 petere Exp $
PostgreSQL documentation
-->

<refentry id="app-pg-ctl">
 <refmeta>
  <refentrytitle><application>gs_ctl</application></refentrytitle>
  <manvolnum>1</manvolnum>
  <refmiscinfo>Application</refmiscinfo>
 </refmeta>

 <refnamediv>
  <refname>gs_ctl</refname>
 ...
 <indexterm zone="app-pg-ctl">
  <primary>gs_ctl</primary>
 </indexterm>

 <refsynopsisdiv>
  <cmdsynopsis>
   <command>gs_ctl</command>
 ...
 <refsect1 id="app-pg-ctl-description">
  ...
  <para>
   <application>gs_ctl</application> is a utility for initializing a
   ...
   backend server (<xref linkend="app-vexdb">), or displaying the
   status of a running server.  Although the server can be started
   manually, <application>gs_ctl</application> encapsulates tasks such
  ...
  <para>
   The <option>init</option> or <option>gs_initdb</option> mode creates a
   new
   <productname>PostgreSQL</productname> database cluster.  A database
   cluster is a collection of databases that are managed by a single
   server instance.  This mode invokes the <command>gs_initdb</command>
   command.  See <xref linkend="app-gs_initdb"> for details.
  </para>
  ...
  <para>
   ...
   standard error are send to <application>gs_ctl</application>'s
   standard output (not standard error).  The standard output of
   <application>gs_ctl</application> should then be redirected to a
   file or piped to another process such as a log rotating program
   like <application>rotatelogs</>; otherwise <command>vexdb</command>
  ...
  <para>
   ...
   by a start.  This allows changing the <command>vexdb</command>
   command-line options.
  </para>

  <para>
   <option>reload</option> mode simply sends the
   <command>vexdb</command> process a <systemitem>SIGHUP</>
  ...
  <title>Options</title>
    ...
    <variablelist>
     ...
     <varlistentry>
      ...
      <listitem>
       <para>
        Specifies the location of the <filename>vexdb</filename>
        executable.  By default the <filename>vexdb</filename> executable is taken from the same
        directory as <command>gs_ctl</command>, or failing that, the hard-wired
        installation directory.  It is not necessary to use this
        option unless you are doing something unusual and get errors
        that the <filename>vexdb</filename> executable was not found.
       </para>

       <para>
        In <literal>init</literal> mode, this option analogously
        specifies the location of the <filename>gs_initdb</filename>
        executable.
       </para>
      </listitem>
     </varlistentry>
     ...
     <varlistentry>
      ...
      <listitem>
       <para>
        ...
        file. For starting up, a successful <command>gsql -l</command> 
        indicates success. <command>gs_ctl</command> will attempt to 
        use the proper port for <application>gsql</>. If the environment variable 
        ...
        (5432 by default). When waiting, <command>gs_ctl</command> will
 ...
 <refsect1>
  ...
  <variablelist>
   ...
   <varlistentry>
    ...
    <listitem>
     <para>
      ...
      linkend="app-gsql"> (used by the <option>-w</option> option).
     </para>
    </listitem>
   </varlistentry>
   ...
   <varlistentry>
    ...
    <listitem>
     <para>
      Default port number for <xref linkend="app-gsql"> (used by the <option>-w</option> option).
     </para>
    </listitem>
   </varlistentry>
  ...
  <para>
   For additional server variables, see <xref linkend="app-vexdb">.
 ...
 <refsect1>
  ...
  <variablelist>
   <varlistentry>
    <term><filename>gaussmaster.pid</filename></term>

    <listitem>
     <para>
      The existence of this file in the data directory is used to help
      <application>gs_ctl</application> determine if the server is
   ...
   <varlistentry>
    <term><filename>gaussmaster.opts</filename></term>

    <listitem>
     <para>If this file exists in the data directory,
      <application>gs_ctl</application> (in <option>restart</option> mode) 
      will pass the contents of the file as options to
      <application>vexdb</application>, unless overridden 
   ...
   <varlistentry>
    ...
    <listitem>
     <para>
      ...
      proper port to use with <application>gsql</application> when the
...
<screen>
<prompt>$</prompt> <userinput>gs_ctl start</userinput>
</screen>
   </para>
   ...
   <para>
    ...
    come up is:
<screen>
<prompt>$</prompt> <userinput>gs_ctl -w start</userinput>
</screen>
   </para>
...
<screen>
<prompt>$</prompt> <userinput>gs_ctl -o "-F -p 5433" start</userinput>
</screen>
   </para>
  </refsect2>
  ...
  <refsect2 id="R2-APP-PGCTL-4">
   ...
   <para>
<screen>
<prompt>$</prompt> <userinput>gs_ctl stop</userinput>
</screen>
  ...
  <refsect2 id="R2-APP-PGCTL-5">
   ...
   <para>
    ...
    except that <command>gs_ctl</command> saves and reuses the command line options that
    were passed to the previously running instance.  To restart
    the server in the simplest form, use:
<screen>
<prompt>$</prompt> <userinput>gs_ctl restart</userinput>
</screen>
   </para>
...
<screen>
<prompt>$</prompt> <userinput>gs_ctl -w restart</userinput>
</screen>
   </para>
...
<screen>
<prompt>$</prompt> <userinput>gs_ctl -o "-F -p 5433" restart</userinput>
</screen>
   </para>
  </refsect2>
  ...
  <refsect2 id="R2-APP-PGCTL-6">
   ...
   <para>
    ...
    <application>gs_ctl</application>:
<screen>
<prompt>$</prompt> <userinput>gs_ctl status</userinput>
<computeroutput>
gs_ctl: server is running (pid: 13718)
Command line was:
/usr/local/pgsql/bin/vexdb '-D' '/usr/local/pgsql/data' '-p' '5433' '-B' '128'
</computeroutput>
</screen>
 ...
 <refsect1>
  ...
  <simplelist type="inline">
   <member><xref linkend="app-gs_initdb"></member>
   <member><xref linkend="app-vexdb"></member>
  </simplelist>
 </refsect1>
