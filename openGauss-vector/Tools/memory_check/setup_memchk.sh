...
help()
{
    echo "$0 [-t action|-m mode|-u user|-f user-profile|-v version|-r revision|-p jeprof] [/path/to/vexdb/package]"
    echo " "
...
_EOF2_
	...
	inst_pkg_dir=$HOME/memchk/$revision/memcheck/vexdb
...
replace_binaries()
{
    echo "cp -f $inst_pkg_dir/bin/vexdb $GAUSSHOME/bin/vexdb"
    cp -f $inst_pkg_dir/bin/vexdb $GAUSSHOME/bin/vexdb
}

restore_binaries()
{
    echo "cp -f $GAUSSHOME/bin.orig/vexdb $GAUSSHOME/bin/vexdb"
    cp -f $GAUSSHOME/bin.orig/vexdb $GAUSSHOME/bin/vexdb
}
...
genreport_jeprof()
{
	...
	for f in $(ls $jeprofdir/${jeproffile}*.heap); do
		fn=$(basename $f)
	 	pprof --show_bytes --pdf $GAUSSHOME/bin/vexdb  $f > jeprof-report-$SUFFIX/$fn.pdf
	done
