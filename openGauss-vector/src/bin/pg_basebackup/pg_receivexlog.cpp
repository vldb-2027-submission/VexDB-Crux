/* -------------------------------------------------------------------------
 *
 * vex_receivexlog.c - receive streaming transaction log data and write it
 *					  to a local file.
 *
 ...
 *
 * IDENTIFICATION
 *		  src/bin/pg_basebackup/vex_receivexlog.c
 * -------------------------------------------------------------------------
 */
...
static void usage(void)
{
    printf(_("%s receives vexdb streaming transaction logs.\n\n"), progname);
    printf(_("Usage:\n"));
    printf(_("  %s [OPTION]...\n"), progname);
    printf(_("\nOptions:\n"));
...
int main(int argc, char** argv)
{
    ...
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_receivexlog"));
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-?") == 0) {
            ...
            puts("vex_receivexlog " DEF_GS_VERSION);
            exit(0);
        }
    }
