...
char *label = "vex_basebackup base backup";
...
static void usage(void)
{
    printf(_("%s takes a base backup of a running vexdb server.\n\n"), progname);
    printf(_("Usage:\n"));
    printf(_("  %s [OPTION]...\n"), progname);
...
static void GsTarUsage(void)
{
    printf(_("%s untars files created by vex_basebackup tar format.\n\n"), progname);
    printf(_("Usage:\n"));
    printf(_("  %s [OPTION]...\n"), progname);
    printf(_("\nOptions controlling the output:\n"));
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(progname, "vex_basebackup")) {
        return GsBaseBackup(argc, argv);
    } else if (!strcmp(progname, "vex_tar")) {
...
static int GsBaseBackup(int argc, char** argv)
{
    ...
    GS_FREE(progname);
    progname = "vex_basebackup";
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_basebackup"));
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-?") == 0) {
            ...
            puts("vex_basebackup " DEF_GS_VERSION);
            exit(0);
        }
    }
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (strlen(optstr) == 1 && optstr[0] == '-') {
            /* ignore the case of redirecting output like "vex_probackup ... -D -> xxx.tar.gz" */
    ...
    while ((c = getopt_long(argc, argv, "D:l:c:h:p:U:s:X:F:T:Z:t:wWvPxz", long_options, &option_index)) != -1) {
        switch (c) {
            ...
            case 'X':
                ...
                break;
            case 'l':
                if (label != NULL && strcmp(label, "vex_basebackup base backup") != 0) {
                    GS_FREE(label);
                }
                check_env_value_c(optarg);
