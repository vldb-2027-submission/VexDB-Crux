/* -------------------------------------------------------------------------
 *
 * pg_ctl --- start/stops/restarts the vexdb server
 *
...
static char* register_servicename = "vexdb";
...
#define MAXCONNINFO 1024
    ...
    if (pmpid > 0) {
        ...
        /* add user and passwd for vex_ctl if needed */
        while (true) {
...
static void do_init(void)
{
    ...
    if (exec_path == NULL)
#ifdef ENABLE_MULTIPLE_NODES
        exec_path = find_other_exec_or_die(argv0, "vex_initdb", "vex_initdb (PostgreSQL) " PG_VERSION "\n");
#else
        exec_path = find_other_exec_or_die(argv0, "vex_initdb", "vex_initdb (vexdb) " PG_VERSION "\n");
#endif
...
/* No -D or -D already added during server start */
    ...
    if (exec_path == NULL)
        exec_path = find_other_exec_or_die(argv0, "vexdb", PG_BACKEND_VERSIONSTR);
}
...
static void do_switchover(uint32 term)
{
    ...
    if (!do_wait) {
         ...
         * again, after wait_seconds, vex_ctl will exit.
         */
...
static char* pgwin32_CommandLine(bool registration)
{
    ...
    if (registration) {
        ...
        ret = find_other_exec(argv0, "vexdb", PG_BACKEND_VERSIONSTR, cmdLine);
        if (ret != 0) {
            pg_log(PG_WARNING, _(" could not find vexdb program executable\n"));
            exit(1);
        }
    }
...
static void do_help(void)
{
    printf(_("%s is a utility to initialize, start, stop, or control an vexdb server.\n\n"), progname);
    printf(_("Usage:\n"));
...
#ifdef ENABLE_MULTIPLE_NODES
    printf(_("  -Z NODE-TYPE           can be \"coordinator\" or \"datanode\" (vexdb)\n"));
...
#if defined(HAVE_GETRLIMIT) && defined(RLIMIT_CORE)
    printf(_("  -c, --core-files       allow vexdb to produce core files\n"));
#else
    printf(_("  -c, --core-files       not applicable on this platform\n"));
#endif
    printf(_("  -l, --log=FILENAME     write (or append) server log to FILENAME\n"));
    printf(_("  -o OPTIONS             command line options to pass to vexdb\n"
             "                         (vexdb server executable) or vex_initdb\n"));
    ...
    printf(_("  -N SERVICENAME  service name with which to register vexdb server\n"));
    printf(_("  -P PASSWORD     password of account to register vexdb server\n"));
    printf(_("  -U USERNAME     user name of account to register vexdb server\n"));
    printf(_("  -S START-TYPE   service start type to register vexdb server\n"));
...
static void adjust_data_dir(void)
{
    ...
    if (exec_path == NULL) {
        my_exec_path = find_other_exec_or_die(argv0, "vexdb", PG_BACKEND_VERSIONSTR);
    } else {
...
static void do_restore(void)
{
    ...
    if (pid != 0 && postmaster_is_alive((pid_t)pid)
        ...
        pg_log(PG_WARNING, _("terminating restore process due to vexdb is still alive"));
        return;
    }
...
static void do_xlog_copy(void)
{
    ...
    if (pid != 0 && postmaster_is_alive((pid_t)pid)
        ...
        pg_log(PG_WARNING, _("terminating xlog copy process due to vexdb is still alive"));
        return;
    }
...
static void do_full_backup(uint32 term)
{
    ...
    pgpid_t pid = get_pgpid();
    if (pid == 0) {
        pg_log(PG_WARNING, _("terminating backup process due to vexdb is not alive"));
        exit(1);
    }
    ...
    conn = get_connection();
    if (PQstatus(conn) != CONNECTION_OK) {
        pg_log(PG_WARNING, _("terminating backup process due to connect vexdb failed"));
        close_connection();
        exit(1);
    }
...
static void do_full_restore(void)
{
    ...
    pg_log(PG_WARNING,
        _("set vexdb state file when full restore in obs mode:"
...
bool build_check_main(uint32 term)
{
    ...
    /* 
     * Save connection info from command line or vexdb file.
     */
...
static void do_overwrite()
{
    ...
    nRet = snprintf_s(
        cmd, sizeof(cmd), sizeof(cmd) - 1, "vexdb --single -Q \'%s\' postgres", xlog_overwrite_opt);
    securec_check_ss_c(nRet, "\0", "\0");
...
static bool do_actual_build(uint32 term)
{
    ...
    UpdateDBStateFile(gaussdb_state_file, &state);
    pg_log(PG_WARNING,
           _("set vexdb state file when %s build:"
...
static bool do_incremental_build_xlog()
{
    ...
    UpdateDBStateFile(gaussdb_state_file, &state);
    pg_log(PG_WARNING,
        _("set vexdb state file when full build:"
          "db state(BUILDING_STATE), server mode(STANDBY_MODE), build mode(FULL_BUILD).\n"));
...
#define MAX_REALPATH_LEN 4096
    ...
    char cmd[MAX_REALPATH_LEN + 1] = {
        "ps c -eo pid,euid,cmd | grep vexdb | grep -v grep | awk '{if($2 == curuid && $1!=\"-n\") print "
        "\"/proc/\"$1\"/cwd\"}' curuid=`id -u`| xargs ls -l | awk '{if ($NF==\""};
    char cmdexten[] = {"\")  print $(NF-2)}' | awk -F/ '{print $3 }' | xargs kill -9 >/dev/null 2>&1 "};
    errno_t tnRet = EOK;

    pg_log(PG_WARNING, _("stop failed, killing vexdb by force ...\n"));
...
#ifdef ENABLE_MULTIPLE_NODES
            puts("vex_ctl " DEF_GS_VERSION);
#else
            puts("vex_ctl (vexdb) " PG_VERSION);
#endif
            exit(0);
        }
    }
...
void do_add_member(void)
{
    ...
    if (!SendSigUsr1ToPM()) {
        ...
        pg_log(PG_WARNING, _("Please check adding member result by command 'vex_ctl query'"));
        return;
    }
}
...
void do_remove_member(void)
{
    ...
    if (!SendSigUsr1ToPM()) {
        ...
        pg_log(PG_WARNING, _("Please check removing member result by command 'vex_ctl query'"));
        return;
    }
}
...
void do_set_run_mode(void)
{
     ...
     * This vex_ctl command should be called on every alive DN node 
     * to achieve minority start mode, there xmode is set to 1.
     * When the majority recover, this vex_ctl command should be called 
...
int main(int argc, char** argv)
{
    ...
    if (strcmp(progname, progname_tmp) != 0 && strcmp("gs_ctl", progname_tmp) != 0) {
        free(const_cast<char*>(progname_tmp));
        do_help();
        exit(0);
    }
    free(const_cast<char*>(progname_tmp));
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_ctl"));
    start_time = time(NULL);
    ...
    while (optind < argc) {
...
#endif
        {
            switch (c) {
                ...
                case 'm':
                    ...
                    break;
                case 'N':
                    if (register_servicename != NULL && strcmp(register_servicename, "vexdb") != 0) {
        ...
        if (optind < argc) {
            ...
            if (strcmp(argv[optind], "init") == 0 || strcmp(argv[optind], "vex_initdb") == 0)
    ...
    switch (ctl_command) {
        case INIT_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl init,datadir is %s \n"), pg_data);
            do_init();
            break;
        case STATUS_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl status,datadir is %s \n"), pg_data);
            do_status();
            break;
        case START_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl started,datadir is %s \n"), pg_data);
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                do_start();
                pg_ctl_unlock(lockfile);
            } else {
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running,start failed !\n"));
            }
            break;
        case RESTART_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl restarted ,datadir is %s \n"), pg_data);
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                do_restart();
                pg_ctl_unlock(lockfile);
            } else {
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running,restart failed !\n"));
            }
            break;
        case STOP_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl stopped ,datadir is %s \n"), pg_data);
            do_stop(false);
            break;
        case RELOAD_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl reload ,datadir is %s \n"), pg_data);
            do_reload();
            break;
        case FINISH_REDO_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl finish redo ,datadir is %s \n"), pg_data);
            do_finish_redo();
            break;
        case FAILOVER_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl failover ,datadir is %s \n"), pg_data);
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                do_failover(term);
                pg_ctl_unlock(lockfile);
            } else {
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running,failover failed !\n"));
                goto Error;
            }
            break;
        case SWITCHOVER_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl switchover ,datadir is %s \n"), pg_data);
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                do_switchover(term);
            } else {
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running,switchover failed !\n"));
                goto Error;
            }
            break;
        case NOTIFY_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl notify ,datadir is %s \n"), pg_data);
            do_notify(term);
            break;
        case QUERY_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl query ,datadir is %s \n"), pg_data);
            do_query();
            break;
        case BUILD_QUERY_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl build query ,datadir is %s \n"), pg_data);
            do_build_query();
            break;
        case KILL_COMMAND:
            pg_log(PG_PROGRESS, _("vex_ctl kill ,datadir is %s \n"), pg_data);
            do_kill(killproc);
            break;
#if defined(WIN32) || defined(__CYGWIN__)
        ...
        case BUILD_COMMAND:
            ...
            if (conn_str != NULL) {
                if (build_mode == FULL_BUILD || build_mode == CROSS_CLUSTER_FULL_BUILD) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl full build ,datadir is %s,conn_str is \'%s\'\n"),
                        pg_data,
                        conn_str);
                } else if (build_mode == STANDBY_FULL_BUILD || build_mode == CROSS_CLUSTER_STANDBY_FULL_BUILD) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl standby full build ,datadir is %s,conn_str is \'%s\'\n"),
                        pg_data,
                        conn_str);
                } else if (build_mode == COPY_SECURE_FILES_BUILD) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl copy secure files from remote build ,datadir is %s,conn_str is \'%s\'\n"),
                        pg_data,
                        conn_str);
                } else if (build_mode == BUILD_CHECK) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl build check ,datadir is %s,conn_str is \'%s\'\n"),
                        pg_data,
                        conn_str);
                } else {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl incremental build ,datadir is %s,conn_str is \'%s\'\n"),
                        pg_data,
                        conn_str);
                }
            } else {
                if (build_mode == FULL_BUILD || build_mode == CROSS_CLUSTER_FULL_BUILD) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl full build ,datadir is %s\n"),
                        pg_data);
                } else if (build_mode == STANDBY_FULL_BUILD || build_mode == CROSS_CLUSTER_STANDBY_FULL_BUILD) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl standby full build ,datadir is %s\n"),
                        pg_data);
                } else if (g_is_obsmode) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl full backup to obs ,datadir is %s\n"),
                        pg_data);
                } else if (build_mode == BUILD_CHECK) {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl build check ,datadir is %s\n"),
                        pg_data);
                } else {
                    pg_log(PG_PROGRESS,
                        _("vex_ctl incremental build ,datadir is %s\n"),
                        pg_data);
                }
            }
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running,build failed !\n"));
                goto Error;
            }
            break;
        case HOTPATCH_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("[PATCH-ERROR] Another vex_ctl command is still running,hotpatch failed !\n"));
            }
            break;
        case GS_STACK_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("[STACK-ERROR] Another vex_ctl command is still running,gs_stack failed !\n"));
            }
            break;
#ifndef ENABLE_MULTIPLE_NODES
        case ADD_MEMBER_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running, add member failed.\n"));
                goto Error;
            }
            break;
        case REMOVE_MEMBER_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running, remove member failed.\n"));
                goto Error;
            }
            break;
        case CHANGE_ROLE_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running, change role failed.\n"));
                goto Error;
            }
            break;
        case MINORITY_START_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running, minority start failed.\n"));
                goto Error;
            }
            break;
#endif
        case RESTORE_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running, restore failed.\n"));
            }
            break;
        case COPY_COMMAND:
            if (-1 != pg_ctl_lock(pg_ctl_lockfile, &lockfile)) {
                ...
                pg_log(PG_PROGRESS, _("Another vex_ctl command is still running, copy failed.\n"));
            }
            break;
        default:
            break;
