...
int main(int argc, char* argv[])
{
    ...
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_resetxlog"));
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "--version") == 0 || strcmp(argv[1], "-V") == 0) {
            ...
            puts("vex_resetxlog (PostgreSQL) " PG_VERSION);
#else
            puts("vex_resetxlog (vexdb) " PG_VERSION);
#endif
            exit(0);
        }
    }
    ...
    /*
     * Don't allow vex_resetxlog to be run as root, to avoid overwriting the
    ...
    if (geteuid() == 0) {
        ...
        fprintf(stderr, _("You must run %s as the vexdb system admin.\n"), progname);
        exit(1);
    }
#endif
...
static void usage(void)
{
    printf(_("%s resets the vexdb transaction log.\n\n"), progname);
    printf(_("Usage:\n  %s [OPTION]... DATADIR\n\n"), progname);
    printf(_("Options:\n"));
