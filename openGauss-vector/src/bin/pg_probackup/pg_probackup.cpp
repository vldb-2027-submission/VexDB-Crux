...
static void parse_non_subcommand_option(char *option, int argc, char *argv[])
{
        ...
        if (strcmp(argv[1], "agent") == 0)
        {
            if (argc > 2)
            {
                elog(ERROR, "Version mismatch, vex_probackup binary with version '%s' "
                        "is launched as an agent for vex_probackup binary with version '%s'",
                 ...
                 strcmp(argv[1], "help") == 0)
        {
            if (argc > 2)
                ...
                help_pg_probackup();
                 ...
                 || strcmp(argv[1], "-V") == 0)
        {
            puts("vex_probackup " DEF_GS_VERSION);
            exit(0);
        }
        else
