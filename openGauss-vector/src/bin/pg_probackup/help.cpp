...
void help_command(const char *command)
{
    if (strcmp(command, "init") == 0)
        ...
        printf(_("No help page for \"%s\" command. Try vex_probackup help\n"), command);
    else
        printf(_("Unknown command \"%s\". Try vex_probackup help\n"), command);
    exit(0);
}

void help_pg_probackup(void)
{
    printf(_("\n%s - utility to manage backup/recovery of vexdb database.\n\n"), PROGRAM_NAME);
...
static void help_add_instance(void)
{
    ...
    printf(_("      --remote-path=path           path to directory with vex_probackup binary on remote host\n"));
