...
backup_stopbackup_callback(bool fatal, void *userdata)
{
    ...
    /*
     * If backup is in progress, notify stop of backup to vexdb
     */
                                 ...
                                 XLogRecPtr *prev_backup_start_lsn, PGconn *backup_conn)
{
    ...
    if (prev_backup)
    {
        if (parse_program_version(prev_backup->program_version) > parse_program_version(PROGRAM_VERSION))
            elog(ERROR, "vex_probackup binary version is %s, but backup %s version is %s. "
                        "vex_probackup do not guarantee to be forward compatible. "
                        "Please upgrade vex_probackup binary.",
     ...
     * Though it will not save us if vexdb instance is actually
    ...
    if (current.backup_mode != BACKUP_MODE_FULL &&
        ...
        prev_backup->start_lsn > current.start_lsn)
            elog(ERROR, "Current START LSN %X/%X is lower than START LSN %X/%X of previous backup %s. "
                "It may indicate that we are trying to backup vexdb instance from the past.",
...
{    
    ...
    if (parray_num(backup_files_list) < 100)
        elog(ERROR, "PGDATA is almost empty. Either it was concurrently deleted or "
            "vex_probackup do not possess sufficient permissions to list PGDATA content");
...
/*
 * Take a backup of a single vexdb instance.
...
do_backup_instance(PGconn *backup_conn, PGNodeInfo *nodeInfo, bool no_sync, bool backup_logs, bool backup_replslots)
{
    ...
    /* notify start of backup to vexdb server */
    time2iso(label, lengthof(label), current.start_time);
    rc = strncat_s(label,1024, " with pg_probackup", lengthof(label) -
        strlen(" with pg_probackup"));
    securec_check_c(rc, "\0", "\0");

    /* Call pg_start_backup function in vexdb connect */
...
pgdata_basic_setup(const ConnectionOptions conn_opt, PGNodeInfo *nodeInfo)
{
    ...
    if (current.checksum_version)
        elog(LOG, "This vexdb instance was initialized with data block checksums. "
                        "Data block corruption will be detected");
    else
        elog(WARNING, "This vexdb instance was initialized without data block checksums. "
                                "vex_probackup have no way to detect data block corruption without them. "
                                "Reinitialize PGDATA with option '--data-checksums'.");

    if (nodeInfo->is_superuser)
        elog(WARNING, "Current vexdb role is superuser. "
        ...
        bool no_validate, bool no_sync, bool backup_logs, bool backup_replslots)
{
    ...
    elog(INFO, "Backup start, vex_probackup version: %s, instance: %s, backup ID: %s, backup mode: %s, "
    ...
    /*
     * Ensure that backup directory was initialized for the same vexdb
...
/*
 * Check if the instance is vexdb fork.
 */
static bool
                                ...
                                bool file_exists, int timeout_elevel)
{
    ...
    if (!stream_wal && is_start_lsn && *try_count == 30)
        elog(WARNING, "By default vex_probackup assume WAL delivery method to be ARCHIVE. "
...
static void get_valid_stop_lsn(pgBackup *backup, bool *stop_lsn_exists, XLogRecPtr stop_backup_lsn_tmp)
{
    ...
    /*
        * Even though the value is invalid, it's expected vexdb behaviour
    ...
    /* vexdb returned something very illegal as STOP_LSN, error out */
    else
...
static void wait_stop_result(PGresult **res, PGconn  *conn)
{
    ...
    while (1)
    {
        ...
        if (PQisBusy(conn))
        {
            ...
            /*
                * If vexdb haven't answered in archive_timeout seconds,
                * send an interrupt.
                */
...
/*
 * Notify end of backup to vexdb server.
 */
static void
                                ...
                                PGNodeInfo *nodeInfo)
{
     ...
     * vexdb archive_command problem and in this case we will
...
IdentifySystem(StreamThreadArg *stream_thread_arg)
{
    ...
    */
    ...
    if (stream_conn_sysidentifier != instance_config.system_identifier)
        elog(ERROR, "System identifier mismatch. Connected vexdb instance has system id: "
    ...
    if (stream_conn_tli != current.tli)
        elog(ERROR, "Timeline identifier mismatch. "
            "Connected vexdb instance has timeline id: %X. Expected: %X.",
