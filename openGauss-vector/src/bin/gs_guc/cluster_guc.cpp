 ...
 *
 * IDENTIFICATION
 *        src/bin/vex_guc/cluster_guc.cpp
 *
 * ---------------------------------------------------------------------------------------
 */
...
char* modify_parameter_value(const char* value, bool localMode)
{
    ...
    for (i = 0, j = 0; i < (int)strlen(value) && j < MAX_VALUE_LEN; i++, j++) {
        if (value[i] == '$') {
             ...
             * nodename "vex_guc set -Z datanode -I all -c \"dynamic_library_path='\\\$libdir/xxx'\"" local  command:
             * vex_guc set -Z datanode -I all -c \"dynamic_library_path='\$libdir/xxx'\""
             */
...
#define ALLIG_HBA_CONF_LEN 10
    ...
    curlen = snprintf_s(
        buffer, buflen, buflen - 1, "vex_guc %s %s ", get_ctl_command_type(), get_instance_type());
 ...
 ******************************************************************************
 ...
 Description : validate remote instance name.
               The vex_guc commands like this: "-I instance_name -N nodename".
...
int validate_cluster_guc_options(char* nodename, int type, char* instance_name, char* indatadir)
{
    ...
    if ((NULL == instance_name) && (NULL == indatadir)) {
        if (type == INSTANCE_CMAGENT || type == INSTANCE_CMSERVER) {
            write_stderr("ERROR: -I all are mandatory for executing vex_guc.\n");
        } else {
            write_stderr("ERROR: -D or -I are mandatory for executing vex_guc.\n");
        }
        return 1;
    } else if ((NULL != instance_name) && (NULL != indatadir)) {
        write_stderr("ERROR: -D or -I only need one for executing vex_guc.\n");
        return 1;
    }
...
void process_cluster_guc_option(char* nodename, int type, char* instance_name, char* indatadir)
{
    ...
    /* only execute vex_guc in one node and one instance. Only specify the -D parameter */
...
printExecErrorMesg(const char* fcmd, const char *nodename)
{
    if (g_remote_command_result == 127) {
        write_stderr(_("ERROR: Failed to execute vex_guc command: %s on node \"%s\", error code is %u, "
                "please ensure that vex_guc exists.\n"), fcmd, nodename, g_remote_command_result);
        ...
        write_stderr(_("ERROR: Failed to execute vex_guc command: %s on node \"%s\", error code is %u, "
                "please get more details from current node path \"$GAUSSLOG/bin/vex_guc\".\n"),
                fcmd, nodename, g_remote_command_result);
    }
}
...
static void do_all_nodes_instance_local_in_parallel_loop(const char* instance_name, const char* indatadir)
{
    ...
    if (g_incorrect_nodeInfo->num == 0) {
        (void)write_stderr("ALL: Success to perform vex_guc!\n\n");
    }
    else {
        (void)write_stderr("ALL: Failure to perform vex_guc!\n\n");
        exit(1);
    }
}
...
static void executePopenCommandsParallel(const char* cmd, int idx, bool is_local_node)
{
    ...
    size_t len_fcmd = 0;
    char* nodename = NULL;
    /* the temp directory that storage vex_guc result information */
    char gausshome[MAXPGPATH] = {0};
    ...
    /* execute vex_guc commands by 'ssh' */
    if (mpprvFile == NULL) {
...
static void readPopenOutputParallel(const char* cmd, bool if_for_all_instance)
{
    ...
    if (nodetype == INSTANCE_COORDINATOR) {
        instance_nums = get_all_coordinator_num();
        write_stderr("Begin to perform vex_guc for coordinators.\n");
    } else if (nodetype == INSTANCE_DATANODE) {
        instance_nums = get_all_datanode_num();
        write_stderr("Begin to perform vex_guc for datanodes.\n");
    } else if (nodetype == INSTANCE_CMSERVER) {
        instance_nums = get_all_cmserver_num();
        write_stderr("Begin to perform vex_guc for cm_servers.\n");
    } else if (nodetype == INSTANCE_CMAGENT) {
        instance_nums = get_all_cmagent_num();
        write_stderr("Begin to perform vex_guc for cm_agents.\n");
    } else {
        instance_nums = get_all_gtm_num();
        write_stderr("Begin to perform vex_guc for gtms.\n");
    }
    ...
    while (true == read_pending) {
        ...
        for (idx = 0; idx < g_max_commands_parallel; idx++) {
            ...
            if (fgets(result, MAX_P_READ_BUF - 1, curr_cxt->pfp) != NULL) {
                ...
                endsp = strstr(result, "Success to perform vex_guc");
                if (NULL != endsp) {
                    successNumber++;
                    if (NULL != curr_cxt->pfp) {
                ...
                endsp = strstr(result, "Failure to perform vex_guc");
                if (NULL != endsp) {
                    failedNumber++;
                    g_incorrect_nodeInfo->nodename_array[g_incorrect_nodeInfo->num++] = xstrdup(curr_cxt->nodename);
                    if (NULL != curr_cxt->pfp) {
 ...
 ******************************************************************************
 ...
 Input       : idx - node id index
               command - vex_guc execute command
 Output      : None
 Return      : None
 ******************************************************************************
*/
 ...
 ******************************************************************************
 ...
 Description : get keywords from the command, the information is used for analysis vex_guc
 Input       : command
 Output      : None
 Return      : keywords
 ******************************************************************************
...
char* get_keywords(char* command)
{
    ...
    if (!is_hba_conf) {
        if (strstr(command, " set ") != NULL)
            keywords = xstrdup("vex_guc set:");
        else if (strstr(command, " reload ") != NULL)
            keywords = xstrdup("vex_guc reload:");
        else
            keywords = xstrdup("vex_guc check:");
    } else {
        if (strstr(command, " set ") != NULL)
            keywords = xstrdup("vex_guc sethba:");
        else
            keywords = xstrdup("vex_guc reloadhba:");
    }
...
void save_parameter_info(char* buffer, gucInfo* guc_info)
{
     ...
     *     vex_guc check: NodeName: pamameter=value: [$PATH]
     */
 ...
 ******************************************************************************
 ...
 Description : save the instance information which parse from the result file that
               do remote vex_guc set/reload  into global parameter
 Input       : nodename    - node name
    ...
    const char* result_file, const char* nodename, char* command, gucInfo* guc_info, bool isRealGucInfo)
{
    ...
    while (*line != NULL) {
        if (strstr(*line, keywords) != NULL) {
             ...
             *     vex_guc set: pamameter=value: [$PATH]
             * The result type of check
             *     expected guc information: NodeName: max_connections=NULL: [$PATH]
             *     vex_guc check: NodeName: pamameter=value: [$PATH]
             */
            ...
            if (is_found && *p == '/') {
                ...
                /*the vex_guc result information is "[gucconfig]\n", so remove ']\n' first.*/
...
int do_gucopt_parse(const char* guc_opt, struct guc_config_enum_entry& guc_variable_list)
{
    ...
    if (NULL != ptr) {
        ...
        (void)write_stderr(
            "ERROR: Failed to parse the guc \"%s\" option. The value range information is incorrect.\n",
            guc_variable_list.guc_name);
        return FAILURE;
    }
