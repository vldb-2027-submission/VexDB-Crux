...
do_gucset(const char *action_type, const char *data_dir)
{
    ...
    for (i = 0; i < config_param_number; i++)
    {
        ...
        /* get the type of vex_guc execution */
...
/*************************************************************************************
 ...
 Desc    : function to print the help information about "vex_guc check"
 Return  :
 *************************************************************************************/
static void do_help_check_guc(void)
{
...
static void check_build_status(const char* path)
{
    ...
    if (is_file_exist(tmpfilename1) || is_gs_build_pid(tmpfilename2)) {
        write_stderr(_("%s:The vex_ctl build is doing now, the vex_guc process should be stopped.\n"), progname);
        exit(1);
    }
}
...
void do_check_parameter_value_type()
{
    ...
    for (i = 0; i < config_value_number; i++) {
        ...
        /* "vex_guc check" only support "parameter" not "parameter=value"*/
...
void doGenerateOperation(const char* datadir, const char* loginfo)
{
    ...
    (void)write_log("vex_guc generate %s\n", loginfo);
}
...
#ifdef ENABLE_MULTIPLE_NODES
        puts("vex_guc " DEF_GS_VERSION);
#else
        puts("vex_guc (vexdb) " PG_VERSION);
#endif
        exit(0);
    }
}
...
#define MAXLOGINFOLEN 1024
    ...
    errno_t rc = 0;
    progname = PROG_NAME;
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_guc"));
    arraysize = argc - 1;
#ifndef ENABLE_LITE_MODE
    ...
    for (int i = 0; i < argc; i++) {
        if ((strlen(arguments) + strlen(argv[i])) >= (MAX_BUF_SIZE - 2)) {
            if (*arguments) {
                (void)write_log("The vex_guc run with the following arguments: [%s].\n", arguments);
            }
            (void)write_log("The vex_guc run with the following arguments: [%s].\n", argv[i]);
    ...
    if (ctl_command == ENCRYPT_KEY_COMMAND) {
        ...
        (void)write_log("vex_guc encrypt %s\n", loginfo);
...
/*************************************************************************************
 ...
 Desc    : print the "vex_guc check" result.
...
int find_same_paravalue_index(const char* paraname)
{
    ...
    /* We can makesure that the index > -1. Because after doing ""vex_guc check", the paraname must be in
...
/*******************************************************************************
 ...
 Description : print the "vex_guc check" result
 Input       : None
 Output      : None
 Return      : void
*******************************************************************************/
...
int print_guc_result(const char* nodename)
{
    ...
    if (!g_remote_connection_signal || g_remote_command_result != 0) {
        ...
        if (0 == g_expect_gucInfo->gucinfo_num) {
            if (CHECK_CONF_COMMAND == ctl_command)
                ...
                (void)write_stderr("Success to perform vex_guc!\n\n");
            return 0;
        }
    }
    ...
    if (g_expect_gucInfo->gucinfo_num > g_real_gucInfo->gucinfo_num) {
        ...
        (void)write_stderr("\n");
        if (CHECK_CONF_COMMAND != ctl_command)
            (void)write_stderr("Failure to perform vex_guc!\n\n");
...
/*******************************************************************************
 ...
 Description : do vex_guc set/reload
...
void do_guc_check_for_each_parameter(const char* action_type)
{
    ...
    for (i = 0; i < config_param_number; i++) {
        ...
        if (NULL == guc_value) {
            ...
            (void)write_stderr("vex_guc %s: %s: %s=%s: [%s]\n",
                action_type,
                g_local_node_name,
                config_param[i],
                guc_value,
