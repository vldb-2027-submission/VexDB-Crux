...
void RestoreArchive(Archive* AHX)
{
    ...
    if (parallel_mode) {
        ...
        if (AH->version < K_VERS_1_8)
            exit_horribly(modulename, "parallel restore is not supported with archives made by pre-8.0 vex_dump\n");
    ...
    if (AH->publicArc.verbose) {
        ...
        if (AH->archiveDumpVersion != NULL) {
            (void)ahprintf(AH, "-- Dumped by vex_dump version %s\n", AH->archiveDumpVersion);
        }
