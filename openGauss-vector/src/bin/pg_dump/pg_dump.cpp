...
int main(int argc, char** argv)
{
    ...
    progname = get_progname("vex_dump");
    ...
    /*
     * During vex_dump, PQfnumber() is matched according to the lowercase column name.
     ...
     * need to turn it on cause this connection is for vex_dump only, will not affect others.
     */
...
void help(const char* pchProgname)
{
    ...
    printf(_("  -q, --target=VERSION                        dump data format can compatible Vexdb version "
             "(v1 or ..)\n"));
    ...
    printf(_("  --with-salt=RANDVALUES                      used by vex_dumpall, pass rand value array\n"));
#ifdef ENABLE_MULTIPLE_NODES
...
TableInfo* getTables(Archive* fout, int* numTables)
{
    ...
    *numTables = ntups;
            ...
            while (p != NULL) {
                ...
                for (j = 0; j < (int)lengthof(timeseries_opt); j++) {
                    if (pg_strncasecmp(p, timeseries_opt[j], strlen(timeseries_opt[j])) == 0) {
                        write_stderr("%s: vex_dump dump timeseries options.\n", p);
                        is_timeseries = true;
                    }
                }
    ...
    /*
     * When do vex_dump using option --role and -t/--include-table-file. Because permission reason(backup other user's
...
static void dumpTableSchema(Archive* fout, TableInfo* tbinfo)
{
    ...
    if (tbinfo->relkind == RELKIND_VIEW || tbinfo->relkind == RELKIND_CONTQUERY) {
        ...
        if (RELKIND_FOREIGN_TABLE == tbinfo->relkind || RELKIND_STREAM == tbinfo->relkind) {
             ...
             * is oprerated by vex_dump. For datanode, it is not necessary to dump
             * distributeby clause.
             */
