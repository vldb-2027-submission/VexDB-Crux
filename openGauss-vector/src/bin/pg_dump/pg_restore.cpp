...
int main(int argc, char** argv)
{
    ...
    progname = get_progname("vex_restore");
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "--version") == 0 || strcmp(argv[1], "-V") == 0) {
            ...
            puts("vex_restore " DEF_GS_VERSION);
#else
            puts("vex_restore (vexdb) " PG_VERSION);
#endif
            exit_nicely(0);
        }
    }

    /* parse the restore options for vex_restore*/
    ...
    if (NULL == inputFileSpec) {
        write_stderr(_("Mandatory to specify dump filename/path for vex_restore\n"));
    ...
    if ((NULL == opts->dbname) && (0 == opts->tocSummary)) {
        write_stderr(_("Mandatory to specify either dbname or list option for vex_restore\n"));
        if (opts->rolepassword != NULL) {
            rc = memset_s(opts->rolepassword, strlen(opts->rolepassword), 0, strlen(opts->rolepassword));
            securec_check_c(rc, "\0", "\0");
        }
    ...
    if ((opts->dbname != NULL) && (1 == opts->tocSummary)) {
        write_stderr(_("Either one of dbname or list option need to be specified for vex_restore\n"));
        if (opts->rolepassword != NULL) {
            rc = memset_s(opts->rolepassword, strlen(opts->rolepassword), 0, strlen(opts->rolepassword));
            securec_check_c(rc, "\0", "\0");
        }
    ...
    /*
     * Whether to keep submitting sql commands as "vex_restore ... | gsql ... "
     */
    ...
    /*free the memory allocated for vex_restore options */
...
{
    printf(_("%s restores an vexdb database from an archive created by vex_dump.\n\n"), pchProgname);
    printf(_("Usage:\n"));
