...
#define PROG_NAME "vex_dumpall"

/* version string we expect back from pg_dump */
#define PGDUMP_VERSIONSTR "vex_dump " DEF_GS_VERSION "\n"
...
int main(int argc, char* argv[])
{
    ...
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_dump"));
#ifdef GSDUMP_LLT
    while (lltRunning) {
        sleep(3);
    }
    ...
    progname = get_progname("vex_dumpall");
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "--version") == 0 || strcmp(argv[1], "-V") == 0) {
            puts("vex_dumpall " DEF_GS_VERSION);
            exit_nicely(0);
        }
    }

    if ((ret = find_other_exec(argv[0], "vex_dump", PGDUMP_VERSIONSTR, pg_dump_bin)) < 0) {
        ...
        if (ret == -1)
            write_stderr(_("The program \"vex_dump\" is needed by %s "
            ...
            write_stderr(_("The program \"vex_dump\" was found by \"%s\"\n"
    ...
    /*
     * During vex_dumpall, PQfnumber() is matched according to the lowercase column name.
     ...
     * need to turn it on cause this connection is for vex_dumpall only, will not affect others.
     */
    ...
    dumpall_printf(OPF, "--\n-- vexdb database cluster dump complete\n--\n\n");
...
void help(void)
{
    printf(_("%s extracts an vexdb database cluster into an SQL script file.\n\n"), progname);
    printf(_("Usage:\n"));
    printf(_("  %s [OPTION]...\n"), progname);
...
/*
 * execute vex_dump command parallel
 */
static void executePopenCommandsParallel(const char* cmd, char* dbname)
{
    int rc;
        ...
        for (idx = 0; idx < g_cur_commands_parallel; idx++) {
            ...
            /* failed to get results from pipe, exit */
                ...
                if (curr_cxt->retvalue != 0) {
                    ...
                    write_stderr(_("%s: vex_dump failed on database \"%s\", exiting\n"), progname, curr_cxt->dbname);
                    break;
                }
            }
        }
