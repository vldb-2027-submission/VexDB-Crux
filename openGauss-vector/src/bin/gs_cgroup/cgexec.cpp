...
static int cgexec_create_default_cgroups(void)
{
    ...
    for (i = 1; i <= WDCG_END_ID; i++) {
        ...
        // 1. if Vexdb range is changed, all subdir's quota should be changed
...
static int cgexec_update_fixed_top_cgroup(void)
{
    ...
    if (0 == strcmp(cgutil_opt.topname, GSCGROUP_ROOT)) {
        ...
        top = TOPCG_GAUSSDB;
        if (cgutil_opt.setspct) {
            fprintf(stderr, "ERROR: users can't modify the cpu cores percentage of Vexdb cgroup with \"--fixed\"!\n");
            return -1;
        }
    } else if (0 == strcmp(cgutil_opt.topname, GSCGROUP_TOP_BACKEND))
        top = TOPCG_BACKEND;
 ...
 *                delete Vexdb group. Otherwise, it will delete
