...
{
    fprintf(stdout,
        "\nvex_cgroup is used to manage the Gauss Cgroups on each node.\n"
        "Usage:\n vex_cgroup [OPTION]...\n\n"
        "OPTIONS:\n"
        ...
        "vex_cgroup -U name -H path -c : create the default control groups.\n"
        "vex_cgroup -U name -d : drop all control groups.\n"
        "vex_cgroup -m: mount cgroup\n"
        "vex_cgroup -M: umount cgroup\n"
        "\n"
        "Non-root user can execute:\n"
        "vex_cgroup -p: display the control groups configuration information.\n"
        "vex_cgroup -c -S class: create the default control groups for class\n"
        "vex_cgroup -d -S class: drop all control groups of class\n"
        "vex_cgroup -c -S class -G wg1: create wg1 groups for class.\n"
        "vex_cgroup -d -S class -G wg2 : \n"
        "  drop wg2 groups of class, its child group is moved to its level.\n"
        "vex_cgroup -u -T Vexdb -t 70: \n"
        "  update CPU percentage of Vexdb cgroup as 70%%.\n"
        "vex_cgroup -u -f 2-8 -T Gaussdb: \n"
        "  update the CPU cores of Gaussdb:user cgroup as 2~8.\n"
        "vex_cgroup -u --fixed -S class1 -s 40: \n"
        "  update the CPU cores percentage of class1 group as 40%% of the Top group: Class\n"
        "vex_cgroup -S class -G wg -E \"blocktime=5,elapsedtime=5\" -a\n"
        "vex_cgroup -S class -G wg -E \"spillsize=256,broadcastsize=100\" -a\n"
        "vex_cgroup -c -N ngname: create control groups for the logical cluster ngname.\n"
        "vex_cgroup -c -N ngname -S class -G wg1: create wg1 groups for class in the logical cluster ngname.\n"
        "vex_cgroup -d -N ngname -S class: drop class control groups in the logical cluster ngname.\n"
        "vex_cgroup -p -N ngname: display the control groups configuration information of the logical cluster ngname.\n"
        "\n");
...
static int check_input_valid(void)
{
    ...
    if ((*cgutil_opt.clsname || *cgutil_opt.wdname || *cgutil_opt.bkdname ||
        ...
        *cgutil_opt.sets) {
        fprintf(stderr, "ERROR: Only specify \'-f\' option on Vexdb Group.\n");
        return -1;
    }
 ...
 * description  : parse the option of vex_cgroup utility
...
/*
 * function name: main
 * description  : main entry of vex_cgroup utility
...
int main(int argc, char** argv)
{
    ...
    /* print the log about arguments of vex_cgroup */
    ...
    write_log("The vex_cgroup run with the following arguments: [%s].\n", arguments);
    ...
    if (-1 == ret) {
        fprintf(stderr, "HINT: please run 'vex_cgroup -h' to display the usage!\n");
        exit(-1);
    }

    if (cgutil_version != NULL) {
        fprintf(stdout, "vex_cgroup %s\n", cgutil_version);
        return 0;
    }
...
error:
    if (cgutil_vaddr[0] != NULL)
        (void)munmap(cgutil_vaddr[0], GSCGROUP_ALLNUM * sizeof(gscgroup_grp_t));
    write_log("vex_cgroup execution error.\n");
    exit(-1);
}
...
{
    char* argv[] = {"vex_cgroup", "-D", "/dev/cgroups/test", "--upgrade"};
