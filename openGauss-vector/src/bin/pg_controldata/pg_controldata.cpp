...
{
    printf(_("%s displays control information of an vexdb database cluster.\n\n"), prog_name);
    printf(_("Usage:\n"));
...
static void display_control_page(ControlFileData ControlFile, int instance_id, bool display_all)
{
    ...
    if (ControlFile.pg_control_version % 65536 == 0 && ControlFile.pg_control_version / 65536 != 0)
        printf(_("WARNING: possible byte ordering mismatch\n"
                 ...
                 "the vexdb installation would be incompatible with this data directory.\n"));
...
int main(int argc, char* argv[])
{
    ...
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_controldata"));
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "--version") == 0 || strcmp(argv[1], "-V") == 0) {
            ...
            puts("vex_controldata (PostgreSQL) " PG_VERSION);
#else
            puts("vex_controldata (vexdb) " PG_VERSION);
#endif
            exit_safely(0);
        }
    }
