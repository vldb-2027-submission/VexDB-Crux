 ...
 * collection of vexdb databases all managed by the same server.
 *
 ...
 * the default database, vexdb, are made just by copying template1.
 *
 * To create template1, we run the vexdb (backend) program in bootstrap
 ...
 * vexdb cluster location, and should be highly compatible with it.
...
/*
 * macros for running pipes to vexdb
 */
#define PG_CMD_DECL      \
    char cmd[MAXPGPATH]; \
    FILE* cmdfd = NULL
...
#ifdef PGXC
    /* Add vexdb node name to configuration file */
...
/*
 * get the superuser password if required, and call vexdb to set it
 */
static void get_set_pwd(void)
{
    PG_CMD_DECL;
...
static void setup_schema(void)
{
     ...
     * 3. vex_dump, gs_restore, and roach are not to ignore/skip this schema
...
static void load_supported_extension(void)
{
    ...
    /* loading foreign-data wrapper for vexdb access */
#ifdef ENABLE_MULTIPLE_NODES
    load_gc_fdw();
#endif
...
static void usage(const char* prog_name)
{
    printf(_("%s initializes an vexdb database cluster.\n\n"), prog_name);
    printf(_("Usage:\n"));
...
#ifdef ENABLE_MULTIPLE_NODES
    printf(_("      --nodename=NODENAME   name of vexdb node initialized\n"));
    ...
    printf(_("\nOther options:\n"));
    printf(_("  -H, --host-ip             node_host of vexdb node initialized\n"));
...
static void check_authmethod_unspecified(const char** authmethod)
{
    if (*authmethod == NULL || strlen(*authmethod) == 0) {
        authwarning = _("\nWARNING: enabling \"trust\" authentication for local connections\n"
                        ...
                        "--auth-local and --auth-host, the next time you run vex_initdb.\n");
        *authmethod = "trust";
    }
}
...
#ifdef WITH_LICENSE
static void generate_license()
{
    time_t from = time(NULL);
    struct tm* timeInfo = localtime(&from);
    timeInfo->tm_mon += 3;
    time_t to = mktime(timeInfo);
    char license_file[MAXPGPATH];
    int ret = snprintf_s(license_file, MAXPGPATH, MAXPGPATH - 1, "%s/lic.txt", pg_data);
    securec_check_ss_c(ret, "\0", "\0");
    if (generateLicense(license_file, from, to)) {
        printf(_("generate license ... failed\n"));
        (void)fflush(stdout);
        exit_nicely();
    } else {
        printf(_("generate license ... ok\n"));
        (void)fflush(stdout);
    }
}
#endif
...
int main(int argc, char* argv[])
{
    ...
    set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("vex_initdb"));
...
#ifdef ENABLE_MULTIPLE_NODES
            puts("vex_initdb " DEF_GS_VERSION);
#else
            puts("vex_initdb (vexdb) " PG_VERSION);
#endif
            exit(0);
        }
    }
...
#undef FREE_NOT_STATIC_ZERO_STRING
...
#ifdef PGXC
    if (nodename == NULL) {
        write_stderr(_("%s: vexdb node name is mandatory\n"), progname);
        write_stderr(_("Try \"%s --help\" for more information.\n"), progname);
        exit(1);
    }

    if (!is_valid_nodename(nodename)) {
        write_stderr(_("%s: vexdb node name:%s is invalid.\nThe node name must consist of lowercase letters "
    ...
    /*
     * we have to set PGDATA for vexdb rather than pass it on the command
    ...
    if ((ret = find_other_exec(argv[0], "vexdb", PG_BACKEND_VERSIONSTR, backend_exec)) < 0) {
        ...
        if (ret == -1)
            write_stderr(_("The program \"vexdb\" is needed by %s "
            ...
            write_stderr(_("The program \"vexdb\" was found by \"%s\"\n"
    ...
    *last_dir_separator(bin_path) = '\0';
...
#ifdef ENABLE_MULTIPLE_NODES
    printf(_("\nSuccess.\n You can now start the database server of the vexdb coordinator using:\n\n"
             "    %s%s%svexdb%s --coordinator -D %s%s%s\n"
             "or\n"
             "    %s%s%svex_ctl%s start -D %s%s%s -Z coordinator -l logfile\n\n"
             " You can now start the database server of the vexdb datanode using:\n\n"
             "    %s%s%svexdb%s --datanode -D %s%s%s\n"
             "or \n"
             "    %s%s%svex_ctl%s start -D %s%s%s -Z datanode -l logfile\n\n"),
        QUOTE_PATH,
        bin_dir,
    ...
    printf(_("\nSuccess. You can now start the database server of single node using:\n\n"
             "    %s%s%svexdb%s -D %s%s%s --single_node\n"
             "or\n"
             "    %s%s%svex_ctl%s start -D %s%s%s -Z single_node -l logfile\n\n"),
        QUOTE_PATH,
        bin_dir,
        (strlen(bin_dir) > 0) ? DIR_SEP : "",
        QUOTE_PATH,
