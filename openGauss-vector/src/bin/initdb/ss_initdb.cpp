...
int ss_check_existdir(const char *path, int node_id, const char **subdir, int totalnum)
{
    char *subpath = NULL;
    struct stat statbuf;
    int existnum = 0;
...
int ss_check_shareddir(char *path, int node_id, bool *need_mkclusterdir)
{
    ...
    *need_mkclusterdir = false;
    ...
    switch (ss_check_existdir(path, INVALID_INSTANCEID, ss_clusterdirs, SS_CLUSTERDIRS_NUM)) {
        case 0:
    ...
    switch (ss_check_existdir(path, node_id, ss_instancedirs, SS_INSTANCEDIRS_NUM)) {
        case 0:
            break;
        case 1:
        case 2:
