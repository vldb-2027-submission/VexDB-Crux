...
function restore_settings()
{
	...
	while [ 1 ] ; do
		...
		if [[ $NUM_ERROR -ge 1 ]]; then
			echo "Error occured when starting vexdb, check gs_plan_simulator.log for more information."
			exit 1
		fi
		sleep 10
	done
...
function init_database()
{
	...
	while [ 1 ] ; do
		...
		if [[ $NUM_ERROR -ge 1 ]]; then
			report_error "Error occured when initializing vexdb, check gs_plan_simulator.log for more information." 2
		fi
		sleep 10
	done
	...
	while [ 1 ] ; do
		...
		NUM_ERROR=$(cat $log_file_location |grep "Error" |wc -l)
		if [[ $NUM_ERROR -ge 1 ]]; then
			report_error "Error occured when stoping vexdb, check gs_plan_simulator.log for more information." 2
		fi
		sleep 10
	done
	((progress_done++))
...
function dump_stats()
{
	...
	if [[ ! -z ${tables} ]]; then
		...
		sed -i '/vex_dump\[port/d' $object_info_location
		...
		sed -i '/vex_dump\[port/d' $pg_class_stats_info_location
		...
		sed -i '/vex_dump\[port/d' $pg_statistic_stats_info_location	
		...
		if [ "$enable_extended_statistic"x == "yes"x ] ; then	
			...
			sed -i '/vex_dump\[port/d' $pg_statistic_ext_stats_info_location
			echo "dump "$(cat $pg_statistic_stats_info_location | wc -l)" lines"
		fi
		...
		sed -i '/vex_dump\[port/d' $pgxc_class_ng_info_location
	else
