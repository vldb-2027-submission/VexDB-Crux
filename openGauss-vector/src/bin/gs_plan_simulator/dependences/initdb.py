...
class Pterodb():
	...
	def init_env(self):
		...
		for i in range(1, self.coordinator_num+1):
			coor_cmd_init = bin_path + "bin/vex_initdb -w " + pwd + " -D " + self.data_dir + "/coordinator" + str(i) + " --nodename=coordinator" + str(i)
		...
		for i in range(1, self.data_node_num+1):
			datanode_cmd_init = bin_path + "bin/vex_initdb -w " + pwd + " -D " + self.data_dir + "/data_node" + str(i) + " --nodename=datanode" + str(i)
	...
	def __start_server(self):
		...
		#start coor
		for i in range(1, self.coordinator_num+1):
			coor_cmd = bin_path + "bin/vexdb --coordinator -D " + self.data_dir + "/coordinator" + str(i) + "   > logcn" + str(i) + " 2>&1 &"
			print coor_cmd
			os.system(coor_cmd)
			time.sleep(1)
		#start data_node
		for i in range(1, self.data_node_num+1):
			datanode_cmd = g_valgrind + bin_path + "bin/vexdb --datanode  -D " + self.data_dir + "/data_node" + str(i) + "   > logdn" + str(i) + " 2>&1 &"
			print datanode_cmd
	...
	def __stop_server(self):
		...
		#stop coor
		for i in range(1, self.coordinator_num+1):
			coor_cmd = bin_path + "bin/vex_ctl stop -D " + self.data_dir + "/coordinator" + str(i) + " -Z coordinator"
			print coor_cmd
			os.system(coor_cmd)
		#stop data node
		for i in range(1, self.data_node_num+1):
			datanode_cmd = bin_path + "bin/vex_ctl stop -D "+ self.data_dir + "/data_node" + str(i) + " -Z datanode"
			print datanode_cmd
			os.system(datanode_cmd)
		#stop gtm
