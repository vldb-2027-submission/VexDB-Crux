...
#######################################################################
set -e
vex_loader_version="vex_loader: version 0.1"

# env variables
vex_loader_log_level=${vex_loader_log_level:="warning"}
# vex_loader_log_fmt=${vex_loader_log_fmt:="%Y-%m-%d %H:%M:%S"}
vex_loader_log_fmt=${vex_loader_log_fmt:=""}
...
vex_loader_file_tmp=${vex_loader_file_tmp:=".vex_loader_file.tmp.${rnd_suffix}_end"}
vex_loader_table_name=""
...
function load_log()
{
    ...
    case "$level" in
        debug)
            if [[ "$vex_loader_log_level" =~ debug ]]; then
                echo -e "$( date "+${vex_loader_log_fmt}" )DEBUG: $@" 1>&2
                if [[ -f "$cmd_param_log" ]]; then
                    echo -e "$( date "+${vex_loader_log_fmt}" )INFO: $@" >> $cmd_param_log 2>&1
                fi
            fi
            ;;
        info)
            if [[ "$vex_loader_log_level" =~ debug|info ]]; then
                echo -e "$( date "+${vex_loader_log_fmt}" )INFO: $@" 1>&2
                if [[ -f "$cmd_param_log" ]]; then
                    echo -e "$( date "+${vex_loader_log_fmt}" )INFO: $@" >> $cmd_param_log 2>&1
                fi
            fi
            ;;
        warning)
            if [[ "$vex_loader_log_level" =~ debug|info|warn ]]; then
                echo -e "$( date "+${vex_loader_log_fmt}" )WARNING: $@" 1>&2
                if [[ -f "$cmd_param_log" ]]; then
                    echo -e "$( date "+${vex_loader_log_fmt}" )WARNING: $@" >> $cmd_param_log 2>&1
                fi
            fi
            ;;
        error)
            if [[ ! "$vex_loader_log_level" =~ none ]]; then
                echo -e "$( date "+${vex_loader_log_fmt}" )ERROR: $@" 1>&2
                if [[ -f "$cmd_param_log" ]]; then
                    echo -e "$( date "+${vex_loader_log_fmt}" )ERROR: $@" >> $cmd_param_log 2>&1
                fi
            fi
            ;;
    esac
...
function vex_loader_check_res()
{
    msg="$1"
    res="$2"
    if [ $(echo "$res" | grep -i "ERROR:" | wc -l) -ge 1 ]; then
        load_log error "$msg"
        res=$(echo "$res"|sed 's/gsql:.vex_loader_file.tmp.*_end:/gsql:.vex_loader_file.tmp:/')
        load_log error "$res"
        exit $EXIT_CODE_FAIL
    fi
}

function vex_loader_check_res_with_clean_file()
{
    msg="$1"
    res="$2"
    if [ $(echo "$res" | grep -i "ERROR:" | wc -l) -ge 1 ]; then
...
function vex_loader_check_res_with_clean_file_txid()
{
    msg="$1"
    res="$2"
    txid="$3"
...
function init_logfile()
{
    ...
    if [ "$res" == "0" ]; then
        echo $vex_loader_version > $logfile
    else
...
function loader_stat_summary()
{
    ...
    if [[ "$vex_loader_log_level" =~ info|debug ]]; then
        printf '\nLOG:\n%b\n\n' "$(cat $logfile)"
    fi
}
...
function gen_badfile()
{
    ...
    res=$(exec_sql "$sql")
    vex_loader_check_res_with_clean_file_txid "query pgxc_copy_error_log failed: $sql" "$res" "$txid"
    printf '%b\n' "$res" > $badfile
    if [[ "$vex_loader_log_level" =~ info|debug ]]; then
        printf 'BAD:\n%b\n\n' "$(cat $badfile)"
    fi
...
function gen_discardfile()
{
    ...
    res=$(exec_sql "$sql")
    vex_loader_check_res_with_clean_file_txid "query pgxc_copy_error_log COPY_WHEN_ROWS failed: $sql" "$res" "$txid"
    printf '%b\n' "$res" > $discard
    if [[ "$vex_loader_log_level" =~ info|debug ]]; then
        printf 'DISCARD:\n%b\n\n' "$(cat $discard)"
    fi
...
function gen_full_copy_sql()
{
    ...
    if [[ "$table_act" =~ $pat ]]; then
        if [ "$loader_datafile_index" == 0 ]; then
            ...
            vex_loader_check_res_with_clean_file "check table empty using: $table_act" "$res"
            load_log debug "$res"
        ...
        if [[ "$table_act" =~ $pat ]]; then
            if [ "$loader_datafile_index" == 0 ]; then
                res=$(exec_sql "$table_act")
                vex_loader_check_res_with_clean_file "truncate table using: $table_act" "$res"
            fi
...
function get_vex_loader_version()
{
    echo $vex_loader_version
}

function vex_loader_help()
{
    echo "$(get_vex_loader_version)

Usage: vex_loader key=value [key=value ...]
...
function create_copy_tables()
{
    if [ "$cmd_param_create" == "true" ]; then
        ...
        res=$(exec_sql "$sql")
        vex_loader_check_res "call copy_summary_create failed: $sql" "$res"
        ...
        res=$(exec_sql "$sql")
        vex_loader_check_res "call copy_error_log_create failed: $sql" "$res"
    fi
}
...
{
    sql="select 'VEX_LOADER_CONNECT_OK'"
    res=$(exec_sql "$sql")
    # vex_loader_check_res "check db connection using: $sql" "$res"

    load_log debug "$res"

    conn_ok=$(echo $res | grep 'VEX_LOADER_CONNECT_OK' | wc -l)
    if [ "$conn_ok" != "1" ]; then
...
function recalcuate_errors()
{
    ...
    res=$(exec_sql "$sql")
    vex_loader_check_res_with_clean_file_txid "query gs_copy_summary failed: $sql" "$res" "$txid"
...
function load_one_datafile()
{
    ...
    echo "" > ${vex_loader_file_tmp}
    gen_load_options "$datafile" "$cmd_param_skip" "$cmd_param_errors" >> ${vex_loader_file_tmp}
    load_data_sql "$cmd_param_ctl" >> ${vex_loader_file_tmp}

    pre_process_load_file "$vex_loader_file_tmp"

    # transform to \COPY SQL
    copy_sql=$(trans_load_to_copy ${vex_loader_file_tmp})
    load_log info "copy sql: $copy_sql"
    vex_loader_check_res_with_clean_file "transform load to copy failed: $(cat ${vex_loader_file_tmp})" "$copy_sql"

    gen_full_copy_sql "$copy_sql" > ${vex_loader_file_tmp}

    # execute \COPY SQL
    copy_res=$(copy_into_table ${vex_loader_file_tmp})
    load_log info "copy result: $copy_res"
    vex_loader_check_res_with_clean_file "after transform: $(cat ${vex_loader_file_tmp})" "$copy_res"
...
function main()
{
    ...
    vex_loader_table_name=$(cat ${vex_loader_file_tmp}|grep "\COPY" | awk '{print $2}')
