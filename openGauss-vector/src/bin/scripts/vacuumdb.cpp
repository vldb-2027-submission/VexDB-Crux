...
static void vacuum_one_database(const char* dbname, bool full, bool verbose, bool and_analyze, bool analyze_only,
    bool freeze, const char* table, const char* host, const char* port, const char* username,
    enum trivalue prompt_password, const char* progname, bool echo);
static void vacuum_all_databases(bool full, bool verbose, bool and_analyze, bool analyze_only, bool freeze,
    const char* maintenance_db, const char* host, const char* port, const char* username, enum trivalue prompt_password,
    ...
    enum trivalue prompt_password, const char* progname, bool echo)
{
    ...
    if (!executeMaintenanceCommand(conn, sql.data, echo)) {
        if (table != NULL)
            ...
            fprintf(stderr, _("%s: vacuuming of database \"%s\" failed: %s"), progname, dbname, PQerrorMessage(conn));
        PQfinish(conn);
        exit(1);
    }
    PQfinish(conn);
...
static void help(const char* progname)
{
    printf(_("%s cleans and analyzes a vexdb database.\n\n"), progname);
    printf(_("Usage:\n"));
