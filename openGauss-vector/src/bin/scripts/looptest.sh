...
function dumpgstack()
{
  ...
  vexdbproc=$(ps -f -u $currentuser | grep vexdb | grep tmp_check | grep -v grep | awk '{ print $2 }')
  DT=$(date +%F_%T | sed 's/-//g' | sed 's/://g')

  ps -ef | grep ^$currentuser | grep -P "(gs_gtm|vexdb)" > $logloc/allstack$DT
...
do
  ...
  do
    runningproc=$(ps -f -u $currentuser | grep tmp_check | grep vexdb | grep -v grep | wc -l)
    ...
    then
       ...
       echo ""
       killall -9 vexdb gs_gtm pg_regress
       killflag=1
       sleep 30
       runningproc=$(ps -f -u $currentuser | grep tmp_check | grep vexdb | grep -v grep | wc -l)
    else
