...
extern MdfdVec *_mdfd_openseg(SMgrRelation reln, ForkNumber forknum, BlockNumber segno, int oflags);
extern MdfdVec *mdopen(SMgrRelation reln, ForkNumber forknum, ExtensionBehavior behavior);
extern void check_file_stat(const char *file_name);
extern void register_dirty_segment(SMgrRelation reln, ForkNumber forknum, const MdfdVec *seg);
extern char *_mdfd_segpath(const SMgrRelation reln, ForkNumber forknum, BlockNumber segno);
