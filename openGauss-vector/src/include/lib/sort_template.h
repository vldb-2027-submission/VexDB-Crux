...
#ifdef QSORT_TOPK
static inline size_t partition(ST_POINTER_TYPE *a, size_t left, size_t right ST_SORT_PROTO_ARG) {  
    size_t pivotIndex = left + rand() % (right - left + 1);  
    DO_SWAP(a + pivotIndex, a + right);  

    ST_POINTER_TYPE * pivot = a + right;  
    size_t i = left - 1;
    for (size_t j = left; j < right; ++j) {
        if (DO_COMPARE(a + j, pivot) <= 0) {
            ++i;  
            DO_SWAP(a + i, a + j);
        }
    }
    DO_SWAP(a + i + 1, a + right);
    return i + 1;
}

/* the kth elemet is also been sorted*/
ST_SCOPE void 
qsort_ssup_topk(ST_ELEMENT_TYPE *data, size_t n
                ST_SORT_PROTO_ELEMENT_SIZE
                ST_SORT_PROTO_COMPARE
                ST_SORT_PROTO_ARG
                , size_t sort_start_pos
                , size_t k) {               
    ST_POINTER_TYPE *a = (ST_POINTER_TYPE *) data, *pm;

    bool presorted = true;
    DO_CHECK_FOR_INTERRUPTS();
    auto max_pos = std::min(sort_start_pos + k + 1 , n);
    for (pm = a + sort_start_pos + ST_POINTER_STEP; pm < a + max_pos * ST_POINTER_STEP;
         pm += ST_POINTER_STEP)
    {
        DO_CHECK_FOR_INTERRUPTS();
        if (DO_COMPARE(pm - ST_POINTER_STEP, pm) > 0)
    ...
    if ((sort_start_pos + k) < (n - 1)) {
        size_t low = sort_start_pos, high = n - 1;
        while (true) {  
            size_t index = partition(a, low, high, arg);  
            if (index == k + low) {
				ST_SORT(a + sort_start_pos, index - sort_start_pos, arg);
                break;
            }
            ...
            if (low > high) {  
                elog(ERROR, "qsort_ssup_topk failed , low(%zu) > high(%zu)", low, high);
                break; 
            }  
        }
    } else {
		ST_SORT(a + sort_start_pos, n - sort_start_pos, arg);
    }
}
#endif
