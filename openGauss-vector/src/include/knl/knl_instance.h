...
typedef struct knl_g_pid_context {
    ...
    ThreadId *VecWriterPID;
    ThreadId VecIndexerPID;
...
typedef struct knl_g_diskann_context {
    ...
    int cache_size;
    void *lru_cache;
    DiskAnnPQCacheEntry diskann_pq_cache[MAX_ARR_CACHE_SIZE];
    /* vector read-only buffer */
    MemoryContext vec_buf_ctx;
    int vector_buffers;
    void *vec_buffer_mgr;
    int vec_writer_nproc;
    int16 pool_offset_to_write;
    Latch **vec_writer_latch;
    MemoryContext bulk_buf_ctx;
    void *bulkbuf_mgr;
    /* vector indexer backend */
    MemoryContext vec_indexer_ctx;
    HTAB *vec_indexer_threads;
    List *vec_indexer_tasks;
    int max_indexer_worker_threads;
    std::atomic<uint32> vec_indexer_worker_count;
    /* vector indexer query backend */
    int max_indexer_query_threads;
    Latch *launcher_latch;
    List *vec_query_runners;
    void *vec_query_threads_arr;
    size_t tinfo_valid_idx;
} knl_g_diskann_context;

typedef struct knl_g_bm25_context {
    MemoryContext dict_ctx;
    void *buffer;
} knl_g_bm25_context;
...
typedef struct knl_g_annvec_context {
    ...
    /* Vector All Context */
    MemoryContext ann_cxt;
    /* used by quantizer code updating */
    MemoryContext qt_update_cxt;
    void *qt_update_mgr;
...
typedef struct knl_g_rabitq_context {
    ...
    int cache_size;
    void *lru_cache;
    rabitq::RaBitQCache caches[RABITQ_MAX_CACHE_SIZE];
...
#define GTM_FREE_MODE \
    (g_instance.attr.attr_storage.enable_gtm_free || g_instance.attr.attr_storage.gtm_option == GTMOPTION_GTMFREE)
...
#define GLOBAL_PRC_MEMCONTEXT (g_instance.cache_cxt.global_prc_mem[u_sess->session_id % MAX_GLOBAL_PRC_NUM])
