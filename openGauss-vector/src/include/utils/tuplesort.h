...
typedef struct {
    union {
     ...
     ItemPointerData t_tid; /* the tuple id, xt_tid only occupy 6 bytes, so there is no overheads*/
    };
...
extern void tuplesort_set_state_limitinfo(Tuplesortstate* state, int64 limit_offset, int64 limit_count);

extern Tuplesortstate* tuplesort_begin_tiddatum(Oid sortOperator, Oid sortCollation, bool nullsFirstFlag, int workMem, bool randomAccess);
...
extern Tuplesortstate* tuplesort_begin_index_btree(
    Relation indexRel, bool enforceUnique, int workMem, SortCoordinate coordinate, bool randomAccess, int maxMem, TupleDesc tupDesc = NULL, uint32 firstColAttrNum = 1);
...
extern void tuplesort_puttiddatum(Tuplesortstate* state, const ItemPointerData &t_tid, const Datum &val);

extern void tuplesort_performsort(Tuplesortstate* state);
extern void sorttuple_sort(Tuplesortstate *state, SortTuple* sortTuples, int count, int startIndex, int k);
...
extern bool tuplesort_gettid(Tuplesortstate* state, bool forward, ItemPointerData* tid);
extern bool tuplesort_getsorttuple(Tuplesortstate* state, bool forward, SortTuple* stup);
