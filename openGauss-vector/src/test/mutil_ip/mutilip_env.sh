...
export g_local_ip_v6="::1"

root_dir=$(dirname $(pwd))
export jar_path=$root_dir/regress/jdbc_test/gsjdbc400.jar

host_name=`hostname`
error_ip="10.10.10.10"
error_ip2="10.10.10.20"
error_port="7780ab"
error_port2="7783c"
db=postgres
scripts_dir=`pwd`
...
{
echo "start normal"
$bin_dir/gs_guc set -D ${normal_data_dir} -c "most_available_sync = on" >> ./results/vexdb.log 2>&1
$bin_dir/vexdb --single_node  -M normal -p $dn1_normal_port -D $normal_data_dir >> ./results/vexdb.log 2>&1 &
...
{
echo "start primary"
$bin_dir/vexdb --single_node  -M primary -p $dn1_primary_port -D $primary_data_dir >> ./results/vexdb.log 2>&1 &
...
{
echo "start standby"
$bin_dir/vexdb --single_node  -M cascade_standby -p $casecade_standby_port -D $casecade_standby_data_dir >> ./results/vexdb.log 2>&1 &
...
{
echo "start standby"
$bin_dir/vexdb --single_node  -M standby -p $dn1_standby_port -D $standby_data_dir >> ./results/vexdb.log 2>&1 &
...
{
  echo "start standby2"
  $bin_dir/vexdb --single_node -M standby -p $standby2_port -D $standby2_data_dir >> ./results/vexdb.log 2>&1 &
  sleep 2
}
...
{
  echo "start standby3"
  $bin_dir/vexdb --single_node -M standby -p $standby3_port -D $standby3_data_dir >> ./results/vexdb.log 2>&1 &
  sleep 2
}
...
$bin_dir/vexdb --single_node  -M standby -p $dn1_primary_port -D $primary_data_dir >> ./results/vexdb.log 2>&1 &
check_primary_startup
}
...
function setup_hba()
{
    ...
    do
        ...
        $bin_dir/gs_guc set -Z datanode -D ${dir} -h "${hba_line}" >> ./results/vexdb.log 2>&1
