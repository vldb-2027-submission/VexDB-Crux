...
function start_streaming_cluster(){
  ...
  $bin_dir/vexdb --single_node -M hadr_main_standby -p $standby3_port -D $standby3_data_dir > ./results/vexdb.log 2>&1 &
  echo "start cascade standby $standby4_data_dir"
  $bin_dir/vexdb --single_node -M cascade_standby -p $standby4_port -D $standby4_data_dir > ./results/vexdb.log 2>&1 &
}
