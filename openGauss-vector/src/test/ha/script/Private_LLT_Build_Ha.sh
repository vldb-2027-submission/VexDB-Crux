...
function show_env_detail
{
    ...
    os_name="$(get_osname)"
    if [ "${os_name}" == "" ]; then
        error "vexdb" "Can not get OS type on Current node"
        return 1
    else
        log "vexdb" "OS Type: ${os_name}"
    fi

    # show env var
    log "vexdb" "Work  Dir: ${g_boot_dir}"
    log "vexdb" "Code  Dir: ${g_src_dir}"
    log "vexdb" "Log   Dir: ${g_log_file}"
    log "vexdb" "Build Dir: ${g_build_dir}"
    log "vexdb" "Current       User: ${USER}"
    log "vexdb" "Configure Pkg Type: ${g_release_mode}"
    log "vexdb" "User for HAcheck: ${g_execute_user}"
    log "vexdb" "Port for HAcheck: ${g_check_port}"

    log "vexdb" "ENV VAR -- MPPHOME: ${MPPHOME}"
    log "vexdb" "ENV VAR -- GAUSSHOME: ${GAUSSHOME}"
    log "vexdb" "ENV VAR -- MPPDB_CODE: ${MPPDB_CODE}"
    log "vexdb" "ENV VAR -- CC: ${CC}"
    log "vexdb" "ENV VAR -- CXX: ${CXX}"
    log "vexdb" "ENV VAR -- LD_LIBRARY_PATH: ${LD_LIBRARY_PATH}"
    log "vexdb" "ENV VAR -- PATH: ${PATH}"

    log "vexdb" "OPTION -- Retrans_enable: ${g_retrans_enable}"
    log "vexdb" "OPTION -- g_master_user: ${g_master_user}"
    log "vexdb" "OPTION -- g_master_log_path: ${g_master_log_path}"
    log "vexdb" "OPTION -- g_top_build_id: ${g_top_build_id}"
    log "vexdb" "OPTION -- g_cur_build_id: ${g_cur_build_id}"
}
...
function mk_hacheck
{
    ...
    log "vexdb" "Change current dir to src dir"
    log "vexdb" "Command: cd ${g_src_dir}"
    cd ${g_src_dir} >> ${g_log_file} 2>&1
    if [ $? -ne 0 ]; then
        error "vexdb" "Change dir failed"
        return 1
    fi

    #configure
    configure_start_time=$(date +%s.%N)
    log "vexdb" "configure_start_time="${configure_start_time}""
    log "vexdb" "Begin configure..."
    chmod 755 configure

    if [ "${g_release_mode}" == "release" ]; then
        log "vexdb" "Command: ./configure --prefix="${g_build_dir}" CC=g++  CFLAGS='-O2' --enable-thread-safety --with-readline --without-zlib --disable-debug --gcc-version=8.2.0 "
        ...
        log "vexdb" "Command: ./configure --prefix="${g_build_dir}" --enable-debug --enable-cassert CC=g++ CFLAGS='-g3 -O2 -w' --without-zlib --gcc-version=8.2.0 --3rd="${BINARYLIBS}""
    ...
    log "vexdb" "configure_stop_time="${configure_stop_time}""
    if [ $? -ne 0 ]; then
        error "vexdb" "Configure failed."
        log "vexdb" "Configure detail:"
        sed -n '/configure --prefix/,$'p ${g_log_file}|sed 1d|sed '$d'|sed '$d'
        flag_configure_fail=1
        return 1
    else
        log "vexdb" "Configure succeed."
        flag_configure_fail=0
    fi

    #make clean
    makeclean_start_time=$(date +%s.%N)
    log "vexdb" "makeclean_start_time="${makeclean_start_time}""
    log "vexdb" "Begin make clean..."
    log "vexdb" "Command: make clean -sj 8"
    make clean -sj 8 >> ${g_log_file} 2>&1
    makeclean_stop_time=$(date +%s.%N)
    log "vexdb" "makeclean_stop_time="${makeclean_stop_time}""
    if [ $? -ne 0 ]; then
        error "vexdb" "Make clean failed"
        flag_makeclean_fail=1
        return 1
    else
        log "vexdb" "Make clean succeed."
        flag_makeclean_fail=0
    fi

    #make install
    makeinstall_start_time=$(date +%s.%N)
    log "vexdb" "makeinstall_start_time="${makeinstall_start_time}""
    log "vexdb" "Begin make install..."
    for((i=1;i<=2;i++))
    do
        log "vexdb" "Try $i time ..."
        log "vexdb" "Command: make install -sj 8"
        cd ${g_src_dir} && make install -sj 8 >> ${g_log_file} 2>&1
        if [ $? -eq 0 ]; then
            log "vexdb" "Make install succeed."
            flag_makeinstall_fail=0
            break
        else
            if [ $i -eq 2 ];then
                error "vexdb" "Make install failed."
                log "vexdb" "Make install detail:"
                ...
                error "vexdb" "Make install failed first time."
                log "vexdb" "Execute make clean -sj 8 for make install again."
                cd ${g_src_dir} && make clean -sj 8 >> ${g_log_file} 2>&1
            fi
        fi
    done
    makeinstall_stop_time=$(date +%s.%N)
    log "vexdb" "makeinstall_stop_time="${makeinstall_stop_time}""
    #make hacheck
    makehacheck_start_time=$(date +%s.%N)
    log "vexdb" "makehacheck_start_time="${makehacheck_start_time}""
    log "vexdb" "Begin make hacheck..."
    CUR_USER=`whoami`
    if [ "${CUR_USER}" == "root" ]; then
        log "vexdb" "Current user is root."
        #create user
        #add_user
        ...
        log "vexdb" "Begin do the ${check_mode} ..."
        log "vexdb" "Command: su - ${g_execute_user} -c \"source $g_conf_file && export && cd \"${g_src_dir}/src/test/ha\" && make ${check_mode} PART=${check_part}\""
        ...
        if [ $? -ne 0 ]; then
            ...
            log "vexdb" "makehacheck_stop_time="${makehacheck_stop_time}""
            error "vexdb" "Make ${check_mode} failed."
            ...
            cat ${g_src_dir}/src/test/ha/vexdb.log 2>&1 | tee -a ${g_log_file}
            ...
            log "vexdb" "Make ${check_mode} succeed."
        fi
    else
        error "vexdb" "Need current user to be root"
        return 1
    fi
}
...
function transport_info
{
    ...
    if [ -z "$g_master_ip" -o \
        ...
        error "vexdb" "Fail transport logs. Option Retrans parameters have error(s),please check"
        return 1
    fi
    res=0
    ...
    if [ $res -ne 0 ];then
        error "vexdb" "Fail transport logs"
    fi
}
...
function main
{
    ...
    if [ "${flag_makeinstall_fail}" == 1 ] || [ "${flag_makehacheck_fail}" == 1 ] || [ "${flag_makeclean_fail}" == 1 ];then
        error "vexdb" "Exit the make hacheck whith error."
        exit 1
    fi
}
...
#g_log_file=${g_log_dir}/vexdb_ID_${g_cur_build_id}_$(date +'%Y%m%d_%H%M%S').log
#g_log_file=${g_log_dir}/vexdb_llt_$(printf "%.2d" ${check_part})_$(date +'%Y%m%d_%H%M%S').log
g_log_file=${log_file}
