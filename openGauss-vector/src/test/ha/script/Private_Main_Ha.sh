...
agent_path=/usr1/gauss_jenkins/jenkins/workspace/vexdb/
log_path=/home/log
check_nu=$1
...
#echo "[`date +'%Y-%m-%d %H:%M:%S'`] [vexdb] INFO: Command: cd ${agent_path} && git submodule init && git submodule update --remote && cd $agent_path/binarylibs && git lfs pull"
#cd $agent_path
#git submodule init
