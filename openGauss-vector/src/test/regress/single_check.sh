...
function set_hotpatch_env()
{
   ...
   if [ ! -f "${HOTPATCHDIR}"/tool/fastcheck/patch_tool_llt.sh ]; then
       ...
       echo "git clone ssh://git@lfg-y.codehub.huawei.com:2222/Gauss/VexDBKernel/VexDBKernel-hotpatch.git"
       echo "ln -s <hotpatch-code>/VexDBKernel-hotpatch/samples/ ${HOTPATCHDIR}/sample"
       echo "ln -s <hotpatch-code>/VexDBKernel-hotpatch/tool/ ${HOTPATCHDIR}/tool"
       echo "Can not find hotpatch fastcheck tool"
       exit 1
   fi
...
function set_common_env()
{
    ...
    cp -r $REGRESS_PATH/jdbc_client_lib ./vexdb/src/test/regress/
