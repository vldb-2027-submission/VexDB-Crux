...
vexdb = Jenkinsops(jenkins_url, username, password)


def print_slaves():
    print("----- slaves list ------")
    for host, label in vexdb.get_jenkins_slaves().items():
        print(host, label)
...
def add_slaves():
    ...
    for slave in new_slaves:
        ...
        res = vexdb.add_jenkins_slave(slave[0], slave[1], credential_slave)
        if res:
...
def change_label():
    slaves = ['0.0.0.0']
    for slave in slaves:
        res = vexdb.change_slave_label(slave, 'backup')
        print(res)
