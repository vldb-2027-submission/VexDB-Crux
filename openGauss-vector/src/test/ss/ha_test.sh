...
function db_shutdown
{
    ...
    pid=`ps ux|grep vexdb|grep ${DB_HOME[$1]}|grep -v grep`
    while [[ ! -z $pid ]]; do
        sleep 1
        pid=`ps ux|grep vexdb|grep ${DB_HOME[$1]}|grep -v grep`
    done
...
{
    print "kill node $1"
    ps ux | grep vexdb | grep ${DB_HOME[$1]} | grep -v grep | awk '{print $2}' | xargs kill -9 > /dev/null 2>&1
    pid=`ps ux | grep vexdb | grep ${DB_HOME[$1]} | grep -v grep`
    while [[ ! -z $pid ]]; do
        sleep 1
        pid=`ps ux | grep vexdb | grep ${DB_HOME[$1]} | grep -v grep`
...
function db_start
{
    ...
    ${BIN_PATH}/vexdb -D ${DB_HOME[$1]} > /dev/null 2>&1 &
    print "${BIN_PATH}/vexdb -D ${DB_HOME[$1]} > /dev/null 2>&1 &"
}
...
function deploy_two_inst
{
    ...
    init_vexdb 0 ${SS_DATA}/dss_home0 $SS_DATA $nodedata_cfg
    init_vexdb 1 ${SS_DATA}/dss_home1 $SS_DATA $nodedata_cfg

    set_vexdb_port ${SS_DATA}/dn0 ${PGPORT[0]}
    set_vexdb_port ${SS_DATA}/dn1 ${PGPORT[1]}

    assign_hatest_parameter ${SS_DATA}/dn0 ${SS_DATA}/dn1

    export DSS_HOME=${SS_DATA}/dss_home0
    start_vexdb ${SS_DATA}/dn0
    export DSS_HOME=${SS_DATA}/dss_home1
    start_vexdb ${SS_DATA}/dn1
}
...
{   
    ...
    sh ${CURPATH}/conf_start_dss_inst.sh 2 ${SS_DATA} ${SS_DATA}/dss_disk 
    init_vexdb 0 ${SS_DATA}/dss_home0 $SS_DATA $nodedata_cfg $DORADO_SHARED_DISK
    init_vexdb 1 ${SS_DATA}/dss_home1 $SS_DATA $nodedata_cfg
    ...
    init_vexdb 0 ${SS_DATA_STANDBY}/dss_home0 $SS_DATA_STANDBY $standby_nodedata_cfg
    init_vexdb 1 ${SS_DATA_STANDBY}/dss_home1 $SS_DATA_STANDBY $standby_nodedata_cfg

    set_vexdb_port ${SS_DATA_STANDBY}/dn0 ${STANDBY_PGPORT[0]}
    set_vexdb_port ${SS_DATA_STANDBY}/dn1 ${STANDBY_PGPORT[1]} 
    ...
    start_vexdb ${SS_DATA_STANDBY}/dn0
    export DSS_HOME=${SS_DATA_STANDBY}/dss_home1
    start_vexdb ${SS_DATA_STANDBY}/dn1
...
function failover_alive_tpcc_old_offline()
{
    ...
    pid=`ps ux | grep vexdb | grep ${DB_HOME[$NEXT_REFORMER_ID]} | grep -v grep`
    wait_time=0
    while [[ ! -z $pid ]]; do
        sleep 20
