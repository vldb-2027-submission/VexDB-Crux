...
kill_vexdb()
{
    ps ux | grep vexdb | grep -v grep | awk '{print $2}' | xargs kill -9 > /dev/null 2>&1
...
main()
{
    ...
    init_vexdb 0 ${SS_DATA}/dss_home0
    init_vexdb 1 ${SS_DATA}/dss_home1
...
#    sh ./copy_xlog_to_private_vg.sh ${GAUSSHOME}/bin/dsscmd ${SS_DATA}/dss_home0 2
    ...
    start_vexdb ${SS_DATA}/dn0 ${PGPORT0}
    export DSS_HOME=${SS_DATA}/dss_home1
    start_vexdb ${SS_DATA}/dn1 ${PGPORT1}
