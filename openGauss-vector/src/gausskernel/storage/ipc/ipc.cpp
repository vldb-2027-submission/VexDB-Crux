 ...
 * ----------------------------------------------------------------
 */
void on_proc_exit(pg_on_exit_callback function, Datum arg, bool check_unique)
{
    ...
    if (((function == AtProcExit_Files || function == smgrshutdown) && EnableLocalSysCache()) ||
        ...
        for (int i = 0; i < t_thrd.storage_cxt.on_proc_exit_index; i++) {
            if (t_thrd.storage_cxt.on_proc_exit_list[i].function == function) {
                if (check_unique) {
                    t_thrd.storage_cxt.on_proc_exit_list[i].arg = arg;
                } else {
...
bool cancel_proc_exit(pg_on_exit_callback function, Datum arg)
{
    for (int i = t_thrd.storage_cxt.on_proc_exit_index - 1; i >= 0; i--) {
        if (t_thrd.storage_cxt.on_proc_exit_list[i].function == function &&
            t_thrd.storage_cxt.on_proc_exit_list[i].arg == arg) {
            if (i == t_thrd.storage_cxt.on_proc_exit_index - 1) {
                --t_thrd.storage_cxt.on_proc_exit_index;
                return true;
            }
            errno_t rc = memmove_s(t_thrd.storage_cxt.on_proc_exit_list + i,
                sizeof(ONEXIT) * (MAX_ON_EXITS - i),
                t_thrd.storage_cxt.on_proc_exit_list + i + 1,
                sizeof(ONEXIT) * (t_thrd.storage_cxt.on_proc_exit_index - i - 1));
