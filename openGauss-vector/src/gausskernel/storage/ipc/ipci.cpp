...
void CreateSharedMemoryAndSemaphores(bool makePrivate, int port)
{
    ...
    */
    ...
    if (g_instance.pid_cxt.VecWriterPID == NULL) {
        g_instance.pid_cxt.VecWriterPID = (ThreadId*)MemoryContextAllocZero(
            g_instance.diskann_cxt.vec_buf_ctx, sizeof(ThreadId) * g_instance.diskann_cxt.vec_writer_nproc);
        g_instance.diskann_cxt.vec_writer_latch = (Latch**)MemoryContextAllocZero(
            g_instance.diskann_cxt.vec_buf_ctx, sizeof(Latch*) * g_instance.diskann_cxt.vec_writer_nproc);
    }
