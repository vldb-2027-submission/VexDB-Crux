...
void MOTAdaptor::Init()
{
    ...
    if (gaussdbConfigLoader == nullptr) {
        ...
        elog(FATAL, "Failed to allocate memory for VexDB/MOTEngine configuration loader.");
    }
    MOT_LOG_TRACE("Adding external configuration loader for VexDB");
    if (!m_engine->AddConfigLoader(gaussdbConfigLoader)) {
        delete gaussdbConfigLoader;
        gaussdbConfigLoader = nullptr;
        MOT::MOTEngine::DestroyInstance();
        elog(FATAL, "Failed to add VexDB/MOTEngine configuration loader");
    }
    ...
    if ((g_instance.attr.attr_memory.max_process_memory < (int32)maxReserveMemoryKb) ||
        ...
        // we allow one extreme case: VexDB is configured to its limit, and zero memory is left for us
