 ...
 *
 * IDENTIFICATION
 *	  src/gausskernel/storage/access/gist/gistvacuum.cpp
 *
 * -------------------------------------------------------------------------
 */
#include "postgres.h"
...
Datum gistbulkdelete(PG_FUNCTION_ARGS)
{
    ...
    while (stack != NULL) {
        ...
        buffer = ReadBufferExtended(rel, MAIN_FORKNUM, stack->blkno, RBM_NORMAL, info->strategy);
        LockBuffer(buffer, GIST_SHARE);
