...
static relopt_bool boolRelOpts[] = {
    ...
    {{"enable_toast", "Enables toast", RELOPT_KIND_IVFFLAT}, true},
    {{"by_residual", "Residual vector quantization", RELOPT_KIND_IVFPQ }, true},
    {{"enable_quantization", "Whether quantization is enabled for diskann index build", RELOPT_KIND_DISKANN}, false},
    {{"enable_subgraph", "Whether split data into subgraphs for faster index construction", RELOPT_KIND_DISKANN}, false},
    {{"rabitq_keep_vecs", "Whether original vectors are kept in vector buffers for HNSW index build", RELOPT_KIND_HNSW}, false},
    /* list terminator */
    {{NULL}}};
...
static relopt_int intRelOpts[] = {
    ...
    {{ "ivf_nlist", "Number of inverted lists", RELOPT_KIND_IVFFLAT }, 100, 1, 65535},
    {{ "ivf_nlist2", "Number of secondary inverted lists", RELOPT_KIND_IVFFLAT }, 0, 0, 65535},
    {{ "ivf_nlist", "Number of inverted lists", RELOPT_KIND_IVFPQ }, 100, 1, 65535},
    {{ "num_subquantizers", "Number of subquantizers", RELOPT_KIND_IVFPQ }, 8, 1, 65535},
    {{ "nbits", "Number of bits per quantization index", RELOPT_KIND_IVFPQ }, 8, 1, 16},
    {{ "m", "Max number of connections", RELOPT_KIND_HNSW | RELOPT_KIND_HYBRIDANN }, 16, 2, 100},
    {{ "ef_construction", "Size of the dynamic candidate list for construction", RELOPT_KIND_HNSW | RELOPT_KIND_HYBRIDANN }, 64, 4, 1000},
    {{ "parallel_workers", "Number of parallel workers for index build",
        RELOPT_KIND_HNSW | RELOPT_KIND_DISKANN |  RELOPT_KIND_IVFFLAT | RELOPT_KIND_IVFPQ | RELOPT_KIND_BM25 | RELOPT_KIND_HYBRIDANN },
     0,
     0,
     64},
    {{ "fillfactor", "Packs btree index pages only to this percentage", RELOPT_KIND_DISKANN | RELOPT_KIND_HYBRIDANN }, BTREE_DEFAULT_FILLFACTOR, BTREE_MIN_FILLFACTOR, 100 },
    {{ "m", "Maximum degree of the vamana graph for diskann build", RELOPT_KIND_DISKANN }, 99, 8, 200 },
    {{ "ef_construction", "Size of the candidate list for diskann build", RELOPT_KIND_DISKANN }, 100, 16, 1000 },
    /* list terminator */
    {{NULL}}
};
...
static relopt_int64 int64RelOpts[] = {
    ...
    {{ "num_cluster", "Size of graph centers for HNSW", RELOPT_KIND_HNSW },
     INT64CONST(0),
     INT64CONST(0),
     INT64CONST(10'000'000'000)},
    {{ "graph_magnitude_threshold", 
    "threshold of graph magnitude to build graph index for HybridAnn ", 
    RELOPT_KIND_HYBRIDANN }, 20000, 1, 10'000'000'000},
    /* list terminator */
    {{NULL}}
};
...
static relopt_real realRelOpts[] = {
    ...
    {{ "n_distinct_inherited",
     ...
     DBL_MAX },
    {{ "occlusion_factor",
       "diskann occlusion factor",
       RELOPT_KIND_DISKANN },
     1.2,
     1.00001,
     1.2 },
     {{ "adaptive_relaxation",
       "diskann subgraph adaptive relaxation",
       RELOPT_KIND_DISKANN },
     0.4,
     0.1,
     1.0}, 
    {{ "subgraph_max_vectors_factor",
       "diskann subgraph max number of vectors factor",
       RELOPT_KIND_DISKANN },
     1.5,
     1.0,
     2.0},
...
static relopt_string stringRelOpts[] = {
    ...
    {
        {"quantizer", "Vector Index Quantizer.", RELOPT_KIND_HNSW},
        0,
        true,
        validate_quantizer,
        NULL
    ...
    {
        {"vec_index_magnitudes", "Vector index magnitudes.", RELOPT_KIND_DISKANN | RELOPT_KIND_HYBRIDANN},
        0,
        true,
        validate_magnitudes,
        NULL
    ...
    {
        {"dicts", "BM25 text tokenizer dictionaries.", RELOPT_KIND_BM25},
        0,
        true,
        validate_dicts,
        NULL
    ...
    {
        {"algorithms", "BM25 scoring algorithms.", RELOPT_KIND_BM25},
        0,
        true,
        validate_algorithms,
        NULL
    ...
    {
        {"coefficients", "BM25 scoring algorithm coefficients.", RELOPT_KIND_BM25},
        0,
        true,
