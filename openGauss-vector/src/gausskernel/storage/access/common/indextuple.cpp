 ...
 * ----------------
 */
IndexTuple index_form_tuple(TupleDesc tuple_descriptor, Datum* values, const bool* isnull, bool enabletoast)
{
    ...
    for (i = 0; i < attributeNum; i++) {
         ...
         */   
        if (enabletoast && !VARATT_IS_EXTENDED(DatumGetPointer(untoasted_values[i])) &&
