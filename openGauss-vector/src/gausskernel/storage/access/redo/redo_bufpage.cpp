...
void PageIndexMultiSetUnused(Page page, OffsetNumber *itemnos, int nitems)
{
    ...
    for (int i = 0; i < nitems; i++) {
        ...
        if ((int)offnum <= 0 || (int)offnum > nline) {
            ereport(ERROR, (errcode(ERRCODE_INVALID_ROW_COUNT_IN_RESULT_OFFSET_CLAUSE),
                        errmsg("invalid index offnum: %u, nline: %u", offnum, nline)));
        }    
        ItemId lp = PageGetItemId(page, offnum);
        ItemIdSetUnusedOnly(lp);
    }
...
bool page_index_tuple_overwrite(Page page, OffsetNumber offnum, Item new_item, Size new_size)
{
    ...
    if(ItemIdIsUnused(item_id)) {
        ItemIdSetNormalOnly(item_id);
    }
...
bool PageIsUnused(Page page)
{
    if (PageIsEmpty(page)) {
        return true;
    }
    /*check unused line pointers */
    OffsetNumber maxoffno = PageGetMaxOffsetNumber(page);
    for (OffsetNumber offnum = FirstOffsetNumber; offnum <= maxoffno; offnum = OffsetNumberNext(offnum)) {
        ItemId lp = PageGetItemId(page, offnum);
        if (ItemIdIsUsed(lp)) {
            return false;
        }
...
OffsetNumber PageFindUnsedlp(Page page)
{
    PageHeader phdr = (PageHeader)page;
    ItemId itemId;
    ...
    if (PageHasFreeLinePointers(phdr)) {
        ...
        */
        for (OffsetNumber offsetNumber = FirstOffsetNumber; offsetNumber <= maxOffno; offsetNumber = OffsetNumberNext(offsetNumber)) {
            itemId = PageGetItemId(phdr, offsetNumber);
