...
void XLogBlockDataCommonRedo(XLogBlockHead *blockhead, void *blockrecbody, RedoBufferInfo *bufferinfo)
{
    ...
    switch (rmid) {
        ...
        case RM_SEGPAGE_ID:
            ...
            break;
        case RM_IVF_ID:
            IvfRedoDataBlock(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_HNSW_ID:
            HnswRedoDataBlock(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_DISKANN_ID:
            DiskannRedoDataBlock(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_HYBRIDBT_ID:
            HybridRedoDataBlock(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_VECTORINDEX_ID:
            VectorIndexRedoDataBlock(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_BM25_ID:
            Bm25RedoDataBlock(blockhead, blockdatarec, bufferinfo);
            break;
        default:
...
static const XLogParseBlock g_xlogParseBlockTable[RM_MAX_ID + 1] = {
    ...
    { IvfRedoParseToBlock, RM_IVF_ID },
    { HnswRedoParseToBlock, RM_HNSW_ID },
    { DiskannRedoParseToBlock, RM_DISKANN_ID },
    { HybridRedoParseToBlock, RM_HYBRIDBT_ID },
    { VectorIndexRedoParseToBlock, RM_VECTORINDEX_ID },
    { Bm25RedoParseToBlock, RM_BM25_ID },
};
...
void XLogBlockDoVectorAction(XLogRecParseState *preState)
{
    char *data = preState->blockparse.extra_rec.blockvectorec.mainData;
    Size datalen = preState->blockparse.extra_rec.blockvectorec.datalen;

    if (datalen == sizeof(xl_invalidate_vector_cache)) {
        xl_invalidate_vector_cache *xl_rec = (xl_invalidate_vector_cache *)data;
        vec_invalidate_buffer_cache(xl_rec->relNode, xl_rec->loc, xl_rec->elem_size);
    } else {
        xl_ann_add_vector *xl_rec = (xl_ann_add_vector *)data;
        char *vec = data + sizeof(xl_ann_add_vector);
        RelFileNode tmp_node;
        RelFileNodeCopy(tmp_node, xl_rec->r_Node, preState->blockparse.blockhead.bucketNode);
...
void XLogBlockDoHnswVectorAction(XLogRecParseState *preState)
{
    char *data = preState->blockparse.extra_rec.blockvectorec.mainData;
    Size datalen = preState->blockparse.extra_rec.blockvectorec.datalen;
    ...
    if (datalen == sizeof(xl_invalidate_vector_cache)) {
        ...
        char *vec = data + sizeof(xl_hnsw_add_vec);
        RelFileNode tmp_node;
        RelFileNodeCopy(tmp_node, xl_rec->r_Node, preState->blockparse.blockhead.bucketNode);
        SMgrRelation smgr = smgropen(tmp_node, InvalidBackendId);
