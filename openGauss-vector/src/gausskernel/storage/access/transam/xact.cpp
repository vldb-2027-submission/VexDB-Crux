...
static void AtCleanup_Memory(void)
{
    Assert(StreamThreadAmI() || IsBgWorkerProcess() || t_thrd.role == VECINDEX_WORKER ||
           CurrentTransactionState->parent == NULL);
...
static void CommitTransaction(bool STP_commit)
{
     ...
     */
    if (StreamThreadAmI() || IsBgWorkerProcess() || t_thrd.role == VECINDEX_WORKER) {
