 ...
 */
void
log_newpage_range(Relation rel, ForkNumber forkNum, BlockNumber startblkindex, BlockNumber endblkindex,
                  bool page_std, RmgrId rmid, uint8 info, BlockNumber *startblkno, bool do_report)
{

    char indexName[NAMEDATALEN + 1];
    char partIndexName[NAMEDATALEN + 1];
    populate_index_partition_name(rel, indexName, partIndexName);
    ann_helper::Timer timer(endblkindex - startblkindex + 1u, 100'000ul, indexName, partIndexName);
    if (do_report) {
        timer.set_stage("Logging index");
    }
    int flags;
    BlockNumber blknoIndex;
    ...
    while (blknoIndex < endblkindex)
    {
        ...
        nbufs = 0;
        while (nbufs < XLR_MAX_BLOCK_ID && blknoIndex < endblkindex)
        {
            BlockNumber blkno = startblkno == NULL ? blknoIndex : *(startblkno + blknoIndex);
            Buffer buf = ReadBufferExtended(rel, forkNum, blkno, RBM_NORMAL, strategy);
            ...
            blknoIndex++;
            if (do_report) {
                timer.report_loop("Logging index");
            }
        }
