...
static bool DispatchIvfRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime);
static bool DispatchHnswRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime);
static bool DispatchDiskannRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime);
static bool DispatchHybridRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime);
static bool DispatchVectorIndexRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime);
static bool DispatchBm25Record(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime);
...
static const RmgrDispatchData g_dispatchTable[RM_MAX_ID + 1] = {
    ...
    { DispatchIvfRecord, RmgrRecordInfoValid, RM_IVF_ID, XLOG_IVF_UNLOG_BUILD_INDEX, XLOG_IVF_UPDATE_PAGE_NEXTBLKNO },
    { DispatchHnswRecord, RmgrRecordInfoValid, RM_HNSW_ID, XLOG_HNSW_UNLOG_BUILD_INDEX, XLOG_HNSW_UPDATE_QT },
    { DispatchDiskannRecord, RmgrRecordInfoValid, RM_DISKANN_ID, XLOG_DISKANN_BUILD_INDEX, XLOG_DISKANN_INVALIDATE_VECTOR_CACHE },
    { DispatchHybridRecord, RmgrRecordInfoValid, RM_HYBRIDBT_ID, XLOG_HYBRID_BUILD_INDEX, XLOG_HYBRID_BT_UNLINK_HALFDEAD_PAGE },
    { DispatchVectorIndexRecord, RmgrRecordInfoValid, RM_VECTORINDEX_ID, XLOG_VECTORINDEX_META_AND_NODE, XLOG_VECTORINDEX_MODIFY_META },
    { DispatchBm25Record, RmgrRecordInfoValid, RM_BM25_ID, XLOG_BM25_BUILD_INDEX, XLOG_BM25_ADD_DATA },
};
...
static bool DispatchHnswRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime)
{
    ...
    switch (info) {
        case XLOG_HNSW_ADD_VECTOR: {
            ...
            RelFileNode node;
            RelFileNodeCopy(node, xl_rec->r_Node, XLogRecGetBucketId(record));
            DispatchToOnePageWorker(record, node, expectedTLIs);
            break;
        }
...
static bool DispatchHybridRecord(XLogReaderState *record, List *expectedTLIs, TimestampTz recordXTime)
{
    ...
    if (info == XLOG_HYBRID_ADD_VECTOR) {
        xl_ann_add_vector *xl_rec = (xl_ann_add_vector *)XLogRecGetData(record);
        RelFileNode node;
        RelFileNodeCopy(node, xl_rec->r_Node, XLogRecGetBucketId(record));
        DispatchToOnePageWorker(record, node, expectedTLIs);
