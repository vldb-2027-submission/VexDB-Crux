...
void InitProcess(void)
{
    ...
    if (IsUnderPostmaster && !IsAutoVacuumLauncherProcess() && !IsJobSchedulerProcess() &&
        ...
        !IsFencedProcessingMode() && (t_thrd.role != DMS_WORKER) && t_thrd.role != VECWRITER_THREAD)
...
static void ProcKill(int code, Datum arg)
{
    ...
    if (IsUnderPostmaster && !IsAutoVacuumLauncherProcess() && !StreamThreadAmI() && !IsJobSchedulerProcess() &&
        !IsJobWorkerProcess() && !IsBgWorkerProcess() && !IsDMSWorkerProcess() && t_thrd.role != VECWRITER_THREAD)
