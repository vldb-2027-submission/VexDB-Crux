 ...
 */
size_t BufFileWrite(BufFile* file, const void* ptr, size_t size)
{
    size_t nwritten = 0;
    size_t nthistime;
    errno_t rc = EOK;
    ...
    while (size > 0) {
        ...
        ptr = (const void *)((const char*)ptr + nthistime);
        size -= nthistime;
        nwritten += nthistime;
    }
...
int BufFileGetNums(BufFile *file)
{
    return file->numFiles;
}

File *BufFileGetFiles(BufFile *file)
{
    return file->files;
}
