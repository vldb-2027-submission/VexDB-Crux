...
{
    if (TASK_RUNNER_ENROLLED) {
        TASK_RUNNER->consumer_exit();
    }
    BackgroundWorker *bgw = (BackgroundWorker *)t_thrd.bgworker_cxt.bgworker;
 ...
 */
static bool RegisterBackgroundWorker(BgWorkerContext *bwc)
{
    BGW_HDR* bgworker_base = (BGW_HDR *)g_instance.bgw_base;
    ...
    bwa->bgtaskrunner = TASK_RUNNER_PTR;
...
loop:
    ...
    slist_foreach_modify(iter, &t_thrd.bgworker_cxt.bgwlist) {
        ...
        if (bgw->bgw_status == BGW_FAILED || bgw->bgw_status == BGW_TERMINATED) {
            ...
            if (need_sig && gs_signal_send(bgw->bgw_notify_pid, SIGINT) != 0) {
    ...
    if (!alldone) {
        ...
        ++loop_count;
        need_sig = (loop_count % BGWORKER_STATUS_DURLIMIT == 0);
        goto loop;
    }
...
void BgworkerListWaitStart(int *nparticipants)
{
    slist_iter iter;
    bool alldone = false;
    uint32 disable_count;
    ...
    while (!alldone) {
        ...
        alldone = (nstarted >= *nparticipants);
        if (alldone) {
            CleanupUnstartBgworkers(nunstarts);
            break;
        } else {
...
int LaunchBackgroundWorkers(int nworkers, void *bgshared, bgworker_main bgmain, bgworker_exit bgexit, bool withLockGroupLeader)
{
    int actualWorkers = 0;
    ...
    bwc->leader = withLockGroupLeader ? t_thrd.proc : nullptr;
