...
static void signal_all_vec_buffer_thread(int sig);
static void init_vec_buffer_thread();
static bool is_vec_buffer_thread(ThreadId pid);
static void check_and_reset_ha_listen_port(void);
static void* cJSON_internal_malloc(size_t size);
static bool NeedHeartbeat();

static void* curl_internal_malloc(size_t size);
static void  curl_internal_free(void* ptr);
static void* curl_internal_realloc(void* ptr, size_t size);
static char* curl_internal_strdup(const char* str);
static void* curl_internal_calloc(size_t n, size_t size);
...
void SetShmemCxt(void)
{
    ...
    g_instance.shmem_cxt.MaxBackends = g_instance.shmem_cxt.MaxConnections +
                                       ...
                                       g_instance.diskann_cxt.max_indexer_worker_threads +
                                       g_instance.diskann_cxt.max_indexer_query_threads +
                                       g_instance.diskann_cxt.vec_writer_nproc +
                                       AUXILIARY_BACKENDS +
                                       AV_LAUNCHER_PROCS +
...
int PostmasterMain(int argc, char* argv[])
{
    ...
    /* set memory manager for curl */
    printf("libcurl version: %s\n", curl_version());
    CURLcode curl_init_res = curl_global_init_mem(CURL_GLOBAL_ALL, curl_internal_malloc,curl_internal_free,
                                                  curl_internal_realloc, curl_internal_strdup, curl_internal_calloc); 
    if (curl_init_res != CURLE_OK) {
        write_stderr("curl_global_init_mem failed: %s\n", curl_easy_strerror(curl_init_res));
        ExitPostmaster(1);
    }
    ...
    if (!IS_PGXC_COORDINATOR && !IS_PGXC_DATANODE && !FencedUDFMasterMode && !PythonFencedMasterModel) {
        write_stderr(
            "%s: vexdb: must start as either a Coordinator (--coordinator) or Datanode (--datanode)\n", progname);
        ExitPostmaster(1);
    }
    ...
    */
    ...
    init_vector_engine_buffer_manager();

    init_index_progress_mutex();
...
static void getInstallationPaths(const char* argv0)
{
    ...
    if (find_other_exec(argv0, "vexdb", PG_BACKEND_VERSIONSTR, t_thrd.proc_cxt.postgres_exec_path) < 0) {
        ereport(FATAL, (errmsg("%s: could not locate matching vexdb executable", argv0)));
    }
...
static int ServerLoop(void)
{
    ...
    for (;;) {
        ...
        if (g_instance.pid_cxt.VecIndexerPID == 0 && pmState == PM_RUN && !dummyStandbyMode &&
            !g_instance.streaming_dr_cxt.isInSwitchover && !SS_STANDBY_MODE &&
            !SS_PERFORMING_SWITCHOVER && !SS_STANDBY_FAILOVER && !SS_IN_REFORM &&
            !SS_IN_ONDEMAND_RECOVERY && (vector_indexer_has_activity() ||
            vector_indexer_has_query())) {
            g_instance.pid_cxt.VecIndexerPID = initialize_util_thread(VECINDEX_LAUNCHER);
        }
...
#ifdef WITH_LICENSE
        char license_file[MAXPGPATH];
        int ret = snprintf_s(license_file, MAXPGPATH, MAXPGPATH - 1, "%s/lic.txt", t_thrd.proc_cxt.DataDir);
        securec_check_ss_c(ret, "\0", "\0");
        char errmsg[1024];
        errmsg[0] = '\0';
        if (verifyLicense(license_file, errmsg)) {
            elog(WARNING, "License is invaild, %s, please contact your local authorized dealer.", errmsg);
        }
#endif        
    }
}
    ...
    if (port->user_name == NULL || port->user_name[0] == '\0')
        ereport(elevel,
            (errcode(ERRCODE_INVALID_AUTHORIZATION_SPECIFICATION),
                errmsg("no vexdb user name specified in startup packet")));
...
static void SIGHUP_handler(SIGNAL_ARGS)
{
    ...
    if (g_instance.status <= SmartShutdown && g_instance.demotion <= SmartDemote) {
        ...
        signal_all_vec_buffer_thread(SIGHUP);
        ...
        if (g_instance.pid_cxt.VecIndexerPID != 0) {
            signal_child(g_instance.pid_cxt.VecIndexerPID, SIGHUP);
        }
    ...
    switch (postgres_signal_arg) {
        ...
        case SIGINT:
            ...
            if (pmState == PM_RECOVERY) {
                ...
                if (g_instance.pid_cxt.VecIndexerPID != 0) {
                    signal_child(g_instance.pid_cxt.VecIndexerPID, SIGTERM);
                }

                signal_all_vec_indexer_thread(SIGTERM);

                signal_all_vec_buffer_thread(SIGTERM);
...
#define LOOPHEADER() (exitstatus = (long)(intptr_t)status)
    ...
    for (;;) {
        ...
        if (pid == g_instance.pid_cxt.StartupPID) {
            ...
            if (g_instance.pid_cxt.VecIndexerPID == 0 &&
                !dummyStandbyMode && !g_instance.streaming_dr_cxt.isInSwitchover &&
                !SS_STANDBY_MODE && !SS_PERFORMING_SWITCHOVER && !SS_STANDBY_FAILOVER &&
                !SS_IN_REFORM && !SS_IN_ONDEMAND_RECOVERY && vector_indexer_has_activity()) {
                g_instance.pid_cxt.VecIndexerPID = initialize_util_thread(VECINDEX_LAUNCHER);
            }
        ...
        if (enable_vec_buffer_manager()) {
            int i;
            bool is_vecwriter_thread = false;
            for (i = 0; i < g_instance.diskann_cxt.vec_writer_nproc; i++) {
                if (pid == g_instance.pid_cxt.VecWriterPID[i]) {
                    Assert(!dummyStandbyMode);
                    g_instance.pid_cxt.VecWriterPID[i] = 0;
                    g_instance.diskann_cxt.vec_writer_latch[i] = NULL;
                    if (!EXIT_STATUS_0(exitstatus)) {
                        HandleChildCrash(pid, exitstatus, _("vector buffer writer process"));
                    }
                    is_vecwriter_thread = true;
                    break;
                }
            }

            if (is_vecwriter_thread) {
                continue;
            }
        }
        ...
        if (pid == g_instance.pid_cxt.VecIndexerPID) {
            g_instance.pid_cxt.VecIndexerPID = 0;

            if (!EXIT_STATUS_0(exitstatus)) {
                HandleChildCrash(pid, exitstatus, _("vector index manager launcher process"));
            }
...
static const char* GetProcName(ThreadId pid)
{
    ...
    else if (pid == g_instance.pid_cxt.VecIndexerPID) {
        return "vector index manager process";
    ...
    else if (is_vec_buffer_thread(pid)) {
        return "vector buffer process";
...
static bool all_vec_buffer_thread_exit()
{
    for (int i = 0; i < g_instance.diskann_cxt.vec_writer_nproc; i++) {
        if (g_instance.pid_cxt.VecWriterPID[i] != 0) {
            return false;
        }
    }
    return true;
...
static void signal_all_vec_buffer_thread(int sig)
{
    for (int i = 0; i < g_instance.diskann_cxt.vec_writer_nproc; i++) {
        if (g_instance.pid_cxt.VecWriterPID[i] != 0) {
            signal_child(g_instance.pid_cxt.VecWriterPID[i], sig);
        }
    }
}
...
static void init_vec_buffer_thread()
{
    for (int i = 0; i < g_instance.diskann_cxt.vec_writer_nproc; i++) {
        if (g_instance.pid_cxt.VecWriterPID[i] == 0) {
            g_instance.pid_cxt.VecWriterPID[i] = initialize_util_thread(VECWRITER_THREAD);
        }
    }
}
...
static bool is_vec_buffer_thread(ThreadId pid)
{
    if (g_instance.pid_cxt.VecWriterPID == NULL) {
        return false;
    }
    for (int i = 0; i < g_instance.diskann_cxt.vec_writer_nproc; i++) {
        if (g_instance.pid_cxt.VecWriterPID[i] == pid) {
            return true;
        }
    }
    return false;
...
static void AsssertAllChildThreadExit()
{
    ...
    Assert(g_instance.pid_cxt.StartupPID == 0);
    Assert(all_vec_buffer_thread_exit());
    Assert(all_indexer_thread_exited());
    ...
    Assert(g_instance.pid_cxt.VecIndexerPID == 0);
...
static void PostmasterStateMachine(void)
{
    ...
    if (pmState == PM_WAIT_BACKENDS) {
        ...
        if ((SS_PERFORMING_SWITCHOVER && CountChildren(BACKEND_TYPE_NORMAL | BACKEND_TYPE_AUTOVAC) == 0 &&
            ...
            g_instance.pid_cxt.TxnSnapCapturerPID == 0 &&
            g_instance.pid_cxt.RbCleanrPID == 0 && g_instance.pid_cxt.VecIndexerPID == 0 &&
            ...
            g_instance.pid_cxt.ApplyLauncerPID == 0 &&
#endif
            all_vec_buffer_thread_exit() && all_indexer_thread_exited() &&
...
{
    /* When we do port init at VexDBThreadMain, use knl_thread_arg to init */
    if (arg != NULL) {
...
static char* curl_internal_strdup(const char* str)
{
    ...
    char* dup = (char*)palloc(len);
    memcpy(dup, str, len);
    return dup;
}

static void* curl_internal_calloc(size_t n, size_t size) {
    return palloc0(n * size);
}
...
void SetExtraThreadInfo(knl_thread_arg* arg)
{
    ...
    switch (role) {
        ...
        case BGWORKER: {
            ...
            t_thrd.bgworker_cxt.taskrunner = ((BackgroundWorkerArgs *)arg->payload)->bgtaskrunner;
            pfree_ext(arg->payload);
            break;
        }
        case VECINDEX_WORKER: {
            t_thrd.vector_cxt.tinfo_ptr = ((VectorIndexerArgs *)arg->payload)->tinfo_ptr;
            t_thrd.vector_cxt.task = ((VectorIndexerArgs *)arg->payload)->task;
            t_thrd.vector_cxt.mark = ((VectorIndexerArgs *)arg->payload)->mark;
            pfree_ext(arg->payload);
            break;
        }
        default:
...
int GaussDbThreadMain(knl_thread_arg* arg)
{
    ...
    switch (thread_role) {
        ...
        case VECINDEX_LAUNCHER: {
            t_thrd.proc_cxt.MyPMChildSlot = AssignPostmasterChildSlot();
            if (t_thrd.proc_cxt.MyPMChildSlot == -1) {
                return STATUS_ERROR;
            }
            InitProcessAndShareMemory();
            vector_indexer_launcher();
            proc_exit(0);
        } break;

        case VECINDEX_WORKER: {
            InitProcessAndShareMemory();
            if (t_thrd.vector_cxt.task) {
                vector_indexer_worker();
            } else if(t_thrd.vector_cxt.tinfo_ptr) {
                vector_query_worker();
            }
            proc_exit(0);
        } break;
        ...
        case VECWRITER_THREAD: {
            t_thrd.role = VECWRITER_THREAD;
            InitProcessAndShareMemory();
            vec_writer_main();
            proc_exit(0);
        } break;
...
static ThreadMetaData GaussdbThreadGate[] = {
    ...
    { GaussDbThreadMain<VECINDEX_LAUNCHER>, VECINDEX_LAUNCHER, "vecidxlauncher", "vecindex launcher" },
    { GaussDbThreadMain<VECINDEX_WORKER>, VECINDEX_WORKER, "vecidxworker", "vecindex worker" },
    { GaussDbThreadMain<VECWRITER_THREAD>, VECWRITER_THREAD, "vecwriter", "vector writer" },
