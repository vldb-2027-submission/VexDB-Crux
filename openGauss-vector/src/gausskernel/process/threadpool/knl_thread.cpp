...
static void knl_t_vecwriter_init(knl_t_vector_context* vector_cxt)
{
    vector_cxt->got_SIGHUP = false;
    vector_cxt->shutdown_requested = false;
    vector_cxt->task = NULL;
    vector_cxt->mark = NULL;
}
...
static void knl_t_bgworker_init(knl_t_bgworker_context* bgworker_cxt)
{
    ...
    bgworker_cxt->taskrunner = NULL;
    bgworker_cxt->consumer_enrolled = false;
}
...
void knl_thread_init(knl_thread_role role)
{
    ...
    knl_t_vecwriter_init(&t_thrd.vector_cxt);
