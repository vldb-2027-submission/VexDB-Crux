...
static void knl_g_parallel_redo_init(knl_g_parallel_redo_context *predo_cxt)
{
    ...
    errno_t rc =
        memset_s(const_cast<CheckPoint *>(&predo_cxt->newestCheckpoint), sizeof(CheckPoint), 0, sizeof(CheckPoint));
    securec_check(rc, "\0", "\0");
...
void knl_g_cachemem_create()
{
    g_instance.cache_cxt.global_cache_mem = AllocSetContextCreate(
        g_instance.instance_context, "GlobalCacheMemory", ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE, SHARED_CONTEXT, DEFAULT_MEMORY_CONTEXT_MAX_SIZE, false);

    for (int i = 0; i < MAX_GLOBAL_CACHEMEM_NUM; ++i) {
        g_instance.cache_cxt.global_plancache_mem[i] = AllocSetContextCreate(
            g_instance.instance_context, "GlobalPlanCacheMemory", ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
            ALLOCSET_DEFAULT_MAXSIZE, SHARED_CONTEXT, DEFAULT_MEMORY_CONTEXT_MAX_SIZE, false);
    }
    for (int i = 0; i < MAX_GLOBAL_PRC_NUM; ++i) {
        g_instance.cache_cxt.global_prc_mem[i] =
            AllocSetContextCreate(g_instance.instance_context, "GlobalPackageRuntimeCacheMemory",
                                  ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE,
                                  SHARED_CONTEXT, DEFAULT_MEMORY_CONTEXT_MAX_SIZE, false);
    }
...
static void knl_g_xlog_init(knl_g_xlog_context *xlog_cxt)
{
    ...
    errno_t rc =
        memset_s(&xlog_cxt->shareStorageopCtl, sizeof(ShareStorageOperateCtl), 0, sizeof(ShareStorageOperateCtl));
    securec_check(rc, "\0", "\0");
...
static void KnlGUndoInit(knl_g_undo_context *undoCxt)
{
    ...
    g_instance.undo_cxt.undoContext =
        AllocSetContextCreate(g_instance.instance_context, "Undo", ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
                              ALLOCSET_UNDO_MAXSIZE, SHARED_CONTEXT);
    ...
    for (auto i = 0; i < UNDO_PERSISTENCE_LEVELS; i++) {
        ...
        memset_s((g_instance.undo_cxt.uZoneBitmap[i])->words,
                 (g_instance.undo_cxt.uZoneBitmap[i])->nwords * sizeof(bitmapword), -1,
...
static void InitHotkeyResources(knl_g_stat_context *stat_cxt)
{
    if (stat_cxt->hotkeysCxt == NULL) {
        stat_cxt->hotkeysCxt = AllocSetContextCreate(
            INSTANCE_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_OPTIMIZER), "HotkeysMemory", ALLOCSET_DEFAULT_MINSIZE,
...
static void knl_g_stat_init(knl_g_stat_context *stat_cxt)
{
    ...
    stat_cxt->gInstanceTimeInfo = (int64 *)MemoryContextAllocZero(INSTANCE_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_DFX),
                                                                  TOTAL_TIME_INFO_TYPES * sizeof(int64));
    errno_t rc;
    rc = memset_s(stat_cxt->gInstanceTimeInfo, TOTAL_TIME_INFO_TYPES * sizeof(int64), 0,
                  TOTAL_TIME_INFO_TYPES * sizeof(int64));
    securec_check(rc, "\0", "\0");
    ...
    stat_cxt->fileIOStat =
        (FileIOStat *)MemoryContextAllocZero(INSTANCE_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_DFX), sizeof(FileIOStat));
...
{
    errno_t rc = memset_s(archive_thread_info, sizeof(knl_g_archive_thread_info), 0, sizeof(knl_g_archive_thread_info));
    securec_check(rc, "\0", "\0");
...
{
    pldebug_cxt->PldebuggerCxt =
        AllocSetContextCreate(g_instance.instance_context, "PldebuggerSharedCxt", ALLOCSET_DEFAULT_MINSIZE,
                              ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE, SHARED_CONTEXT);

    errno_t rc = memset_s(pldebug_cxt->debug_comm, sizeof(PlDebuggerComm) * PG_MAX_DEBUG_CONN, 0,
                          sizeof(pldebug_cxt->debug_comm));
    securec_check(rc, "\0", "\0");
...
static void knl_g_csn_barrier_init(knl_g_csn_barrier_context *csn_barrier_cxt)
{
    ...
    errno_t rc =
        memset_s(csn_barrier_cxt->stopBarrierId, MAX_BARRIER_ID_LENGTH, 0, sizeof(csn_barrier_cxt->stopBarrierId));
    securec_check(rc, "\0", "\0");
}

static void knl_g_audit_init(knl_g_audit_context *audit_cxt)
{
    g_instance.audit_cxt.global_audit_context = AllocSetContextCreate(
        g_instance.instance_context, "GlobalAuditMemory", ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE, SHARED_CONTEXT, DEFAULT_MEMORY_CONTEXT_MAX_SIZE, false);
...
static void knl_diskann_init(knl_g_diskann_context *diskann_cxt)
{
    diskann_cxt->ctx = AllocSetContextCreate(g_instance.instance_context, "DiskANNCacheMemory", ALLOCSET_DEFAULT_SIZES,
                                             MEMALIGN_SHRCTX);
    diskann_cxt->cache_size = 0;
    diskann_cxt->lru_cache = NULL;
    diskann_cxt->max_cache_size = DEFAULT_LRU_CACHE_SIZE;

    diskann_cxt->vec_buf_ctx = AllocSetContextCreate(g_instance.instance_context, "Vector Buffer Context",
                                                     ALLOCSET_DEFAULT_SIZES, SHARED_CONTEXT);
    diskann_cxt->pool_offset_to_write = -1;

    diskann_cxt->vec_indexer_ctx = AllocSetContextCreate(g_instance.instance_context, "Vector Indexer Context",
                                                         ALLOCSET_DEFAULT_SIZES, SHARED_CONTEXT);

    diskann_cxt->bulk_buf_ctx = AllocSetContextCreate(g_instance.instance_context, "Bulk Buffer Context",
                                                      ALLOCSET_DEFAULT_SIZES, SHARED_CONTEXT);
    HASHCTL ctl;
    ctl.keysize = sizeof(IndexerThreadStatusKey);
    ctl.entrysize = sizeof(IndexerThreadStatus);
    ctl.hcxt = diskann_cxt->vec_indexer_ctx;
    ctl.hash = [](const void *key, Size keysize) -> uint32 {
        IndexerThreadStatusKey *k = (IndexerThreadStatusKey *)key;
        return oid_hash(&k->rd_id, sizeof(Oid)) ^ uint32_hash(&k->index_blkno, sizeof(BlockNumber)) ^
               uint32_hash(&k->task_type, sizeof(k->task_type));
    };
    if (!IS_PGXC_COORDINATOR) {
        diskann_cxt->vec_indexer_threads =
            hash_create("Vector Indexer Threads", 32, &ctl, HASH_ELEM | HASH_FUNCTION | HASH_SHRCTX);
        diskann_cxt->vec_indexer_tasks = NIL;
        diskann_cxt->vec_indexer_worker_count = 0;
    }

    diskann_cxt->launcher_latch = NULL;
    diskann_cxt->vec_query_runners = NIL;
    diskann_cxt->vec_query_threads_arr =
        (QueryThreadInfo *)palloc0(sizeof(QueryThreadInfo) * MAX_QUERY_THREADS_NUM);
    diskann_cxt->tinfo_valid_idx = 0;
}

static void knl_bm25_init(knl_g_bm25_context *bm25_cxt)
{
    bm25_cxt->dict_ctx = AllocSetContextCreate(g_instance.instance_context, "BM25 Dictionary Context",
        ALLOCSET_DEFAULT_SIZES, SHARED_CONTEXT);
    bm25_dict_init(bm25_cxt);
}

static void knl_annvec_init(knl_g_annvec_context *annvec_cxt)
{
    annvec_cxt->l2_squared_distance = ann_helper::get_general_distance_func(Metric::L2);
    annvec_cxt->negative_inner_product = ann_helper::get_general_distance_func(Metric::INNER_PRODUCT);
    annvec_cxt->cosine_distance = ann_helper::get_general_distance_func(Metric::COSINE);
    annvec_cxt->half_l2_squared_distance = ann_helper::get_general_half_distance_func(Metric::L2);
    annvec_cxt->half_negative_inner_product = ann_helper::get_general_half_distance_func(Metric::INNER_PRODUCT);
    annvec_cxt->half_cosine_distance = ann_helper::get_general_half_distance_func(Metric::COSINE);
    ann_helper::init_rabitq_func();

    annvec_cxt->ann_cxt = AllocSetContextCreate(g_instance.instance_context, "ANN Context",
        ALLOCSET_DEFAULT_SIZES, SHARED_CONTEXT);
    qt_update_init(annvec_cxt);
}

static void knl_rabitq_init(knl_g_rabitq_context *rabitq_ctx)
{
    rabitq_ctx->cache_ctx = AllocSetContextCreate(g_instance.instance_context, "RaBitQ Cache Context",
        ALLOCSET_DEFAULT_SIZES, SHARED_CONTEXT);
    rabitq_ctx->cache_size = 0;
    rabitq_ctx->lru_cache = NULL;
}
...
void knl_instance_init()
{
    ...
    g_instance.instance_context =
        AllocSetContextCreate((MemoryContext)NULL, "ProcessMemory", ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
    ...
    knl_diskann_init(&g_instance.diskann_cxt);
    knl_bm25_init(&g_instance.bm25_cxt);
    knl_annvec_init(&g_instance.annvec_cxt);
    knl_rabitq_init(&g_instance.rabitq_ctx);
