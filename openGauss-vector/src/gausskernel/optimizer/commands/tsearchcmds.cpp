...
static List* verify_dictoptions(Oid tmplId, List* dictOptions, int oldNum, const char* dictPrefix)
{
     ...
     */
    if (!IsUnderPostmaster || (dictPrefix == NULL && tmplId != TSTemplateJiebaId))
        return NIL;
    ...
    if (!OidIsValid(initmethod)) {
         ...
         */
        if (tmplId == TSTemplateJiebaId) {
            bool first_init = true;
            Oid valid_oid = InvalidOid;
            /* here filenames is actually a TemplContentInfos pointer */
            filenames = (List *)DatumGetPointer(OidFunctionCall3(
                initmethod, PointerGetDatum(dictOptions), BoolGetDatum(first_init), ObjectIdGetDatum(valid_oid)));
            ReleaseSysCache(tup);
            return filenames;
        }
        filenames = (List*)DatumGetPointer(OidFunctionCall3(
...
ObjectAddress DefineTSDictionary(List* names, List* parameters)
{
    ...
    if (templId == TSTemplateJiebaId) {
        int current_encoding = GetDatabaseEncoding();
        if (current_encoding != PG_UTF8 && current_encoding != PG_SQL_ASCII) {
            Oid proc = FindDefaultConversionProc(current_encoding, PG_UTF8);
            if (!OidIsValid(proc)) {
                ereport(ERROR,
                    (errcode(ERRCODE_UNDEFINED_FUNCTION),
                     errmsg("Cannot convert current encoding \"%s\" to UTF8",
                            GetDatabaseEncodingName()),
                     errhint("Vexjieba requires UTF8-compatible encoding.")));
            }
        }
    }

    /* get internal dictionary file prefix */
    dictprefix = templId == TSTemplateJiebaId ? NULL : get_tsfile_prefix_internal();
    ...
    if (dictprefix != NULL && (templId != TSTemplateJiebaId && userfiles == NIL))
    ...
    if (templId != TSTemplateJiebaId) {
        ...
        TemplContentInfos *infos = (TemplContentInfos *)userfiles;
        char *buf = NULL;
        if (infos->delimiter) {
            size_t total_len = 
                strlen("stopwords = '', ") + strlen(infos->templ_stopwords_setting) +
                strlen("delimiter = ''") + strlen(infos->delimiter) + 1;
            buf = (char *)palloc(total_len);
            snprintf(buf, total_len,  "stopwords = '%s', delimiter = '%s'",
                infos->templ_stopwords_setting, infos->delimiter);
        } else {
            size_t total_len = 
                strlen("stopwords = '', ") + strlen(infos->templ_stopwords_setting) +
                strlen("userdict = '', ") + strlen(infos->templ_userdict_setting) +
                strlen("KeywordCaseSensitive = ''") + strlen(infos->case_sensitive) + 1;
            buf = (char *)palloc(total_len);
            snprintf(buf, total_len,  "stopwords = '%s', userdict = '%s', KeywordCaseSensitive = '%s'",
                infos->templ_stopwords_setting, infos->templ_userdict_setting, infos->case_sensitive);
        }
        text *t = cstring_to_text(buf);
        values[Anum_pg_ts_dict_dictinitoption - 1] = PointerGetDatum(t);
    }
    ...
    ScanKeyInit(&scanKeys[0],
                ...
                F_NAMEEQ,
                values[Anum_pg_ts_dict_dictname - 1]);
    ScanKeyInit(&scanKeys[1],
                Anum_pg_ts_dict_dictnamespace,
                BTEqualStrategyNumber,
                F_OIDEQ,
                values[Anum_pg_ts_dict_dictnamespace - 1]);
    SysScanDesc scan = systable_beginscan(dictRel, TSDictionaryNameNspIndexId, true, NULL, 2, scanKeys);
    while (HeapTupleIsValid(tup = systable_getnext(scan))) {
        ereport(ERROR,
            (errcode(ERRCODE_DUPLICATE_OBJECT),
                errmsg("text search dictionary \"%s\" already exists in namespace \"%s\".",
                    dictname, get_namespace_name(namespaceoid))));
    }
    ...
    /* insert pg_ts_content */
    if (templId == TSTemplateJiebaId) {
        TemplContentInfos *infos = (TemplContentInfos *)userfiles;
        create_ts_content(dictOid, 's', infos->templ_stopwords);
        if (infos->delimiter == NULL) {
            create_ts_content(dictOid, 'u', infos->templ_userdict);
        }
        infos->destroy();
        userfiles = NIL; /* skip the following command */
    }
...
void RemoveTSDictionaryById(Oid dictId)
{
    ...
    if (tmplId == TSTemplateJiebaId) {
        /* delete pg_ts_content */
        delete_ts_content(dictId, 'u');
        delete_ts_content(dictId, 's');
    }
}
