...
static bool show_scan_distributekey(const Plan* plan)
{
    return (
        ...
        IsA(plan, IndexScan) || IsA(plan, VectorIndexScan) || IsA(plan, TextMatchIndexScan) ||
        IsA(plan, IndexOnlyScan) || IsA(plan, CteScan) || IsA(plan, ForeignScan) || IsA(plan, VecForeignScan) ||
        IsA(plan, BitmapHeapScan) || IsA(plan, TsStoreScan)
    );
}
...
/*
 * Add some additional details about an IndexScan/VectorIndexScan/TextMatchIndexScan or IndexOnlyScan
 */
static void ExplainIndexScanDetails(Oid indexid, ScanDirection indexorderdir, ExplainState* es)
{
