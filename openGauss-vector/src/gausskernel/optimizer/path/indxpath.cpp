...
#define IsBooleanOpfamily(opfamily) ((opfamily) == BOOL_BTREE_FAM_OID || \
    ...
    (opfamily) == BOOL_HYBRIDANN_FAM_OID)
    ...
    bool useful_predicate, SaOpControl saop_control, ScanTypeControl scantype)
{
     ...
     * assuming that the scan result is orderedexcept vector indexes.
     * (Actually, the result is still ordered if there are equality constraints for
     * all earlier columns, but it seems too expensive and
     * non-modular for this code to be aware of that refinement.)
     *
    ...
    pathkeys_possibly_useful = scantype != ST_BITMAPSCAN &&
        (!found_lower_saop_clause || index->relam == diskann_AM_OID ||
         index->relam == hybridann_AM_OID) &&
...
static void match_pathkeys_to_bm25_index(IndexOptInfo* index, PathKey* pathkey,
    List** orderby_clauses_p, List** clause_columns_p)
{
    foreach_cell(lc, pathkey->pk_eclass->ec_members) {
        EquivalenceMember* member = (EquivalenceMember*)lfirst(lc);
        if (!bms_is_empty(member->em_relids) && !bms_equal(member->em_relids, index->rel->relids)) {
            continue;
        }
        if (!IsA(member->em_expr, FuncExpr) && pathkey->pk_strategy != BTLessStrategyNumber) {
            continue;
        }
        Expr *expr = find_bm25_expr(index, member->em_expr);
        if (expr) {
            if (pathkey->pk_strategy != BTGreaterStrategyNumber && IsA(expr, FuncExpr)) {
                ereport(WARNING,
                    (errcode(ERRCODE_INVALID_OPERATION),
                        errmsg("Scoring function bm25_score() only support negative weight or "
                               "DESC ordering, forcing reverse ordering")));
            }
    ...
    IndexOptInfo* index, List* pathkeys, List** orderby_clauses_p, List** clause_columns_p)
{
    ...
    *clause_columns_p = NIL;
    ...
    if (index->relam == bm25_AM_OID) {
        ...
        foreach_cell(lc, pathkeys) {
            PathKey* pathkey = lfirst_node(PathKey, lc);
            match_pathkeys_to_bm25_index(index, pathkey, orderby_clauses_p, clause_columns_p);
        }
        return;
    }
        ...
        case OID_TEXT_ICREGEXEQ_OP:
            isIndexable =
                ...
                ((opfamily == TEXT_BTREE_FAM_OID || opfamily == TEXT_UBTREE_FAM_OID ||
                  opfamily == TEXT_HYBRIDANN_FAM_OID) &&
        ...
        case OID_BPCHAR_ICREGEXEQ_OP:
            isIndexable = (opfamily == BPCHAR_PATTERN_BTREE_FAM_OID || opfamily == BPCHAR_PATTERN_UBTREE_FAM_OID) ||
                          ((opfamily == BPCHAR_BTREE_FAM_OID || opfamily == BPCHAR_UBTREE_FAM_OID ||
                            opfamily == BPCHAR_HYBRIDANN_FAM_OID) &&
                           (pstatus == Pattern_Prefix_Exact || lc_collate_is_c(idxcollation)));
            break;
    ...
    Pattern_Prefix_Status pstatus, int prefixkey_len)
{
    ...
    switch (opfamily) {
        ...
        case BPCHAR_HYBRIDANN_FAM_OID:
            datatype = BPCHAROID;
            break;

        case NAME_BTREE_FAM_OID:
        case NAME_UBTREE_FAM_OID:
        case NAME_HYBRIDANN_FAM_OID:
            datatype = NAMEOID;
            break;

        case BYTEA_BTREE_FAM_OID:
        case BYTEA_UBTREE_FAM_OID:
        case BYTEA_HYBRIDANN_FAM_OID:
            datatype = BYTEAOID;
            break;
