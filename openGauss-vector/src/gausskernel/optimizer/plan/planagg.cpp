...
static void get_pathkeys_for_partiteratorpath(RelOptInfo *final_rel, Expr* mminfo_target)
{
    ...
    foreach (l, final_rel->pathlist) {
        ...
        if (itrpath->path.pathkeys == NULL &&
            (itrpath->subPath->pathtype == T_IndexOnlyScan ||
             itrpath->subPath->pathtype == T_IndexScan ||
             itrpath->subPath->pathtype == T_VectorIndexScan ||
             itrpath->subPath->pathtype == T_TextMatchIndexScan)) {
