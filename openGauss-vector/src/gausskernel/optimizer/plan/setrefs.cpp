...
static uint16 set_index_internal_refs(PlannerInfo* root, Plan* plan, bool require_score, bool from_partiter);
...
struct fix_target_op_context {
    Index varno;
    AttrNumber attrno;
};
typedef Node*(*fix_op)(Node* node, fix_target_op_context* context);
static Node* fix_target_vector_op(Node* node, fix_target_op_context* context);
static Node* fix_target_bm25_op(Node* node, fix_target_op_context* context);
static Node* fix_target_bm25_combine_op(Node* node, fix_target_op_context* context);
static Node* generate_bm25_combine_op(AttrNumber left_attr, AttrNumber right_attr);
static bool has_target_vector_op(Node* node, void*);
static bool has_target_bm25_op(Node* node, void*);
static void fix_index_provided_op_list(PlannerInfo* root, Index relid, AttrNumber attrno,
                                       List* targetlist, fix_op f);
/**
...
Plan* set_plan_references(PlannerInfo* root, Plan* plan)
{
    ...
    plan = set_plan_refs(root, plan, rtoffset);
    if (root->hasIndexProvided && plan) {
        set_index_internal_refs(root, plan, false, false);
    }
    return plan;
}
...
static Plan* set_plan_refs(PlannerInfo* root, Plan* plan, int rtoffset)
{
    ...
    switch (nodeTag(plan)) {
        ...
        case T_VectorIndexScan:
        case T_TextMatchIndexScan: {
            fix_op fop;
            bool has_score_output;
            AttrNumber attrno;
            if (IsA(plan, VectorIndexScan)) {
                fop = fix_target_vector_op;
                has_score_output = ((VectorIndexScan*)plan)->has_score_output;
                attrno = ((VectorIndexScan*)plan)->score_plot;
            } else {
                Assert(IsA(plan, TextMatchIndexScan));
                fop = fix_target_bm25_op;
                has_score_output = ((TextMatchIndexScan*)plan)->has_score_output;
                attrno = ((TextMatchIndexScan*)plan)->score_plot;
            }
            if (has_score_output) {
                IndexScan* splan = (IndexScan*)plan;
                Oid relid = ((Scan*)plan)->scanrelid;
                fix_index_provided_op_list(root, relid, attrno, splan->scan.plan.targetlist, fop);
                fix_index_provided_op_list(root, relid, attrno, splan->scan.plan.qual, fop);
                fix_index_provided_op_list(root, relid, attrno, splan->indexqual, fop);
                fix_index_provided_op_list(root, relid, attrno, splan->indexqualorig, fop);
                root->hasIndexProvided = true;
            }
        }   /* fall through */
        case T_IndexScan: {
            IndexScan* splan = (IndexScan*)plan;
...
static AttrNumber extend_target_list(Plan *plan, Index varno, AttrNumber attrno, fix_op f = NULL)
{
    Expr *node;
    AttrNumber res = 1;
    if (f == fix_target_bm25_combine_op) {
        foreach_cell(l, plan->targetlist) {
            Expr* exp = (Expr*)((TargetEntry*)lfirst(l))->expr;
            if (exp && IsA(exp, OpExpr)) {
                OpExpr* op = (OpExpr*)exp;
                if (op->opfuncid == F_FLOAT4PL && list_length(op->args) == 2) {
                    Var* v1 = (Var*)linitial(op->args);
                    Var* v2 = (Var*)llast(op->args);
                    AttrNumber left_attr = (AttrNumber)varno;
                    if (IsA(v1, Var) && v1->varno == OUTER_VAR && v1->varattno == left_attr &&
                        IsA(v2, Var) && v2->varno == INNER_VAR && v2->varattno == attrno) {
                        return res;
                    }
                }
            }
        ...
        node = (Expr*)generate_bm25_combine_op(varno, attrno);
    } else {
        foreach_cell(l, plan->targetlist) {
            Expr* exp = (Expr*)((TargetEntry*)lfirst(l))->expr;
            if (exp && IsA(exp, Var)) {
                Var* v = (Var*)exp;
                if (v->varno == varno && v->varattno == attrno) {
                    return res;
                }
            }
            ++res;
        }
        node = (Expr*)makeVar(varno, attrno, FLOAT4OID, -1, InvalidOid, 0);
    }
    plan->targetlist = lappend(plan->targetlist,
        makeTargetEntry(node, res, "index_provided_result", false));
    return res;
}

/*
 * Set bm25_score() or vector cmp result that calculated inside indexscan to var that directly read
 * them. Since this function may require both top-down and bottom-up approach which makes plan
 * recursions exponentially slower, we isolate the function and only call it as needed.
 * We don't simply implemented it as set_upper_references(); simply marking an expression
 * with ressortgroupref index is impractical since the actual expression,
 * such as bm25_score() > x where x is not accessible at indexscan, cannot be simply pushed down,
 * so we have to replace the actual part instead of treat it as a magically evaluable thingy.
 * The return value is used for recursion flag.
 */
static uint16 set_index_internal_refs(PlannerInfo* root, Plan* plan, bool require_score, bool from_partiter)
{
    /* op does not handle the case when both are present */
    static const auto get_op = [](uint16 i) FORCE_INLINE -> fix_op {
        return i & 0x8000 ? fix_target_vector_op : fix_target_bm25_op;
    };
    static const auto set_op = [](uint16 &i, NodeTag type) FORCE_INLINE {
        if (type == T_TextMatchIndexScan) {
        } else if (type == T_VectorIndexScan) {
            i |= 0x8000;
        }
    };
    static const auto set_op2 = [](uint16 &i, uint16 base) FORCE_INLINE {
        i = (0x8000 & base) + (i & 0x7ffff);
    };
    static const auto get_attrno = [](uint16 i) FORCE_INLINE -> AttrNumber {
        return (AttrNumber)(i & 0x7fff);
    };
    static const auto set_attrno = [](uint16 &i, AttrNumber attrno) FORCE_INLINE {
        Assert(attrno > 0);
        i = (i & 0x8000) | attrno;
    };
    static const auto has_ref = [](uint32 i) FORCE_INLINE -> bool { return get_attrno(i) > 0; };
    static constexpr uint32 empty_res = 0;

    uint16 res = empty_res;
    bool old_require_score = require_score;
    switch (nodeTag(plan)) {
        case T_VectorIndexScan:
            if (old_require_score && ((VectorIndexScan*)plan)->has_score_output) {
                Assert(false); // not implemented yet
                set_op(res, T_VectorIndexScan);
                AttrNumber attrno = ((VectorIndexScan*)plan)->score_plot;
                if (from_partiter) {
                    set_attrno(res, attrno);
                } else {
                    AttrNumber temp = extend_target_list(plan, ((Scan*)plan)->scanrelid, attrno);
                    set_attrno(res, temp);
                }
            }
            return res;
        case T_TextMatchIndexScan:
            if (old_require_score && ((TextMatchIndexScan*)plan)->has_score_output) {
                set_op(res, T_TextMatchIndexScan);
                AttrNumber attrno = ((TextMatchIndexScan*)plan)->score_plot;
                if (from_partiter) {
                    set_attrno(res, attrno);
                } else {
                    AttrNumber temp = extend_target_list(plan, ((Scan*)plan)->scanrelid, attrno);
        ...
        case T_HashJoin:
        case T_VecHashJoin:
            if (!require_score && (has_target_bm25_op((Node*)((Join*)plan)->joinqual, NULL))) {
                require_score = true;
            }
            /* fall through */
        case T_PartIterator:
        ...
        case T_VecToRow:
        case T_RowToVec:
            if (!require_score && (has_target_bm25_op((Node*)plan->targetlist, NULL) ||
                has_target_bm25_op((Node*)plan->qual, NULL))) {
                require_score = true;
            }
            break;
        case T_SubqueryScan:
        case T_VecSubqueryScan: {
            ...
            res = set_index_internal_refs(root, ((SubqueryScan*)plan)->subplan, true, false);
            if (has_ref(res)) {
                Index varno = ((Scan*)plan)->scanrelid;
                AttrNumber attrno = get_attrno(res);
                fix_index_provided_op_list(root, varno, attrno, plan->targetlist, get_op(res));
                AttrNumber temp = extend_target_list(plan, varno, attrno);
                set_attrno(res, temp);
            }
            return res;
        }
        case T_ModifyTable:
        case T_VecModifyTable:
            foreach_cell (l, ((ModifyTable*)plan)->plans) {
                set_index_internal_refs(root, (Plan*)lfirst(l), false, false);
            }
            return empty_res;
        case T_Append:
        case T_VecAppend:
            foreach_cell (l, ((Append*)plan)->appendplans) {
                uint16 temp =
                    set_index_internal_refs(root, (Plan*)lfirst(l), false, false);
                if (!has_ref(temp)) {
                    return empty_res;
                ...
                if (res != empty_res) {
                    Assert(get_attrno(res) == get_attrno(temp));
                    Assert(get_op(res) == get_op(temp));
                } else {
                    res = temp;
                }
            }
            break;
        case T_MergeAppend:
            foreach_cell (l, ((MergeAppend*)plan)->mergeplans) {
                set_index_internal_refs(root, (Plan*)lfirst(l), false, false);
            }
            return empty_res;
        default:
    ...
    from_partiter = IsA(plan, PartIterator) || IsA(plan, VecPartIterator);
    uint32 res_left = plan->lefttree ?
        set_index_internal_refs(root, plan->lefttree, require_score, from_partiter) :
        empty_res;
    uint32 res_right = plan->righttree ?
        set_index_internal_refs(root, plan->righttree, require_score, from_partiter) :
        empty_res;
    if (!require_score) {
        return res;
    }
    if (!has_ref(res_left) && !has_ref(res_right) && res == empty_res) {
        return empty_res;
    }

    fix_op my_op;
    Index my_varno;
    AttrNumber my_attrno;
    if (has_ref(res_left) && has_ref(res_right)) {
        my_op = fix_target_bm25_combine_op;
        my_varno = get_attrno(res_left);
        my_attrno = get_attrno(res_right);
        set_op2(res, res_left);
        set_op2(res, res_right);
    } else if (has_ref(res_left)) {
        my_op = get_op(res_left);
        set_op2(res, res_left);
        my_varno = OUTER_VAR;
        my_attrno = get_attrno(res_left);
    } else if (res == empty_res) {
        my_op = get_op(res_right);
        set_op2(res, res_right);
        my_varno = INNER_VAR;
        my_attrno = get_attrno(res_right);
    } else {
        my_op = get_op(res);
        my_varno = OUTER_VAR;
        foreach_cell(lc, plan->targetlist) {
            Expr* expr = ((TargetEntry*)lfirst(lc))->expr;
            if (IsA(expr, Var)) {
                my_varno = ((Var*)expr)->varno;
                break;
            }
        }
        my_attrno = get_attrno(res);
    ...
    if (from_partiter) {
        switch (nodeTag(plan->lefttree)) {
            case T_VectorIndexScan:
            case T_TextMatchIndexScan:
                my_varno = ((Scan*)plan->lefttree)->scanrelid;
                break;
            default:
                my_varno = 0;   /* not needed */
        }
    }
    ...
    switch (nodeTag(plan)) {
        ...
        case T_VecToRow:
        case T_RowToVec: {
            fix_index_provided_op_list(root, my_varno, my_attrno, plan->targetlist, my_op);
            if (!old_require_score) {
                return res;
            }
            AttrNumber temp = extend_target_list(plan, my_varno, my_attrno, my_op);
            set_attrno(res, temp);
            if (my_attrno == temp) {
                return res;
            }
            switch (nodeTag(plan)) {
                ...
                case T_Unique:
                case T_VecUnique: {
                    Unique *u = (Unique*)plan;
                    for (int i = 0; i < u->numCols; ++i) {
                        if (u->uniqColIdx[i] == my_attrno) {
                            u->uniqColIdx[i] = temp;
                        }
                    }
                } break;
        ...
        /* join */
        /* bm25_score is volatile so that it cannot be used in join expr */
        /* assume all results are processed below the operator,
         * we only deal with pure plan here */
        case T_NestLoop:
        case T_VecNestLoop:
        case T_MergeJoin:
        case T_VecMergeJoin:
        case T_HashJoin:
        case T_VecHashJoin: {
            fix_index_provided_op_list(root, my_varno, my_attrno, ((Join*)plan)->joinqual, my_op);
        }
        case T_PartIterator:
        case T_VecPartIterator:
        /* agg */
        ...
        case T_Limit:
        case T_VecLimit: {
            fix_index_provided_op_list(root, my_varno, my_attrno, plan->qual, my_op);
            fix_index_provided_op_list(root, my_varno, my_attrno, plan->targetlist, my_op);
            if (old_require_score) {
                AttrNumber temp = extend_target_list(plan, my_varno, my_attrno, my_op);
                set_attrno(res, temp);
...
static bool op_is_index_provided(Oid opno)
{
    return opno == 5037u || opno == 5040u || opno == 5041u;
}
...
static Node* fix_target_vector_op(Node* node, fix_target_op_context* context)
{
    ...
    if (IsA(node, OpExpr)) {
        ...
        set_opfuncid(op);
        Oid opno = op->opno;
        if (op_is_index_provided(opno)) {
            foreach_cell(lc, op->args) {
                Var* var = (Var*)lfirst(lc);
                if (IsA(var, Var)) {
                    return (Node*)makeVar(var->varno, context->attrno, FLOAT8OID, -1, InvalidOid, 0);
                }
            }
        }
        return node;
    }
    return expression_tree_mutator(node, (Node* (*)(Node*, void*))fix_target_vector_op, context, false);
}
static bool has_target_vector_op(Node* node, void*)
{
    if (node == NULL) {
    ...
    if (IsA(node, OpExpr)) {
        ...
        set_opfuncid(op);
        Oid opno = op->opno;
        return op_is_index_provided(opno);
    }
    return expression_tree_walker(node, (bool (*)())has_target_vector_op, NULL);
}
static Node* fix_target_bm25_op(Node* node, fix_target_op_context* context)
{
    if (node == NULL) {
    ...
    if (IsA(node, FuncExpr)) {
        if (((FuncExpr*)node)->funcid == F_BM25_SCORE) {
            return (Node*)makeVar(context->varno, context->attrno, FLOAT4OID, -1, InvalidOid, 0);
        }
        return node;
    }
    return expression_tree_mutator(node, (Node* (*)(Node*, void*))fix_target_bm25_op, context, false);
}
static bool has_target_bm25_op(Node* node, void*)
{
    if (node == NULL) {
    ...
    if (IsA(node, FuncExpr)) {
        return ((FuncExpr*)node)->funcid == F_BM25_SCORE;
    }
    return expression_tree_walker(node, (bool (*)())has_target_bm25_op, NULL);
}
static Node* generate_bm25_combine_op(AttrNumber left_attr, AttrNumber right_attr)
{
    Var* left = makeVar(OUTER_VAR, left_attr, FLOAT4OID, -1, InvalidOid, 0);
    Var* right = makeVar(INNER_VAR, right_attr, FLOAT4OID, -1, InvalidOid, 0);
    return (Node*)makeFuncExpr(F_FLOAT4PL, FLOAT4OID, list_make2(left, right), InvalidOid,
        DEFAULT_COLLATION_OID, COERCE_DONTCARE);
}
static Node* fix_target_bm25_combine_op(Node* node, fix_target_op_context* context)
{
    if (node == NULL) {
    ...
    if (IsA(node, FuncExpr)) {
        if (((FuncExpr*)node)->funcid == F_BM25_SCORE) {
            return generate_bm25_combine_op(context->varno, context->attrno);
        }
        return node;
    }
    return expression_tree_mutator(node, (Node* (*)(Node*, void*))fix_target_bm25_combine_op, context, false);
}

static void fix_index_provided_op_list(PlannerInfo* root, Index relid, AttrNumber attrno,
                                       List* list, fix_op f)
{
    fix_target_op_context context = {relid, attrno};
    foreach_cell(lc, list) {
        lfirst(lc) = (void*)f((Node*)lfirst(lc), &context);
    }
}
