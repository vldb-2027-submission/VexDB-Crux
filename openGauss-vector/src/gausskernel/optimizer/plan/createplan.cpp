...
static VectorIndexScan* make_vectorindexscan(List* qptlist, List* qpqual, Index scanrelid,
    Oid indexid, List* indexqual, List* indexqualorig, List* indexorderby, List* indexorderbyorig,
    List* indextlist, ScanDirection indexscandir, double indexselectivity, bool is_partial, AttrNumber score_plot);
static TextMatchIndexScan* make_textmatchindexscan(List* qptlist, List* qpqual, Index scanrelid,
    Oid indexid, List* indexqual, List* indexqualorig, List* indexorderby, List* indexorderbyorig,
    List* indextlist, ScanDirection indexscandir, double indexselectivity, bool is_partial, AttrNumber score_plot);
...
static bool IsScanPath(NodeTag type)
{
    return (
        ...
        type == T_VectorIndexScan || type == T_TextMatchIndexScan
    );
}
    ...
    PlannerInfo* root, IndexPath* best_path, List* tlist, List* scan_clauses, bool indexonly)
{
    ...
    if (best_path->path.parent->orientation == REL_COL_ORIENTED) {
        ...
        if (indexonly) {
            ...
            if (best_path->path.pathtype == T_VectorIndexScan) {
                scan_plan = (Scan*)make_vectorindexscan(tlist,
                    qpqual,
                    baserelid,
                    indexoid,
                    fixed_indexquals,
                    ...
                    indexselectivity,
                    (best_path->indexinfo->indpred != NIL),
                    best_path->path.parent->max_attr + 1);
            } else if (best_path->path.pathtype == T_TextMatchIndexScan) {
                scan_plan = (Scan*)make_textmatchindexscan(tlist,
                    qpqual,
                    baserelid,
                    indexoid,
                    fixed_indexquals,
...
static List* fix_indexorderby_references(PlannerInfo* root, IndexPath* index_path)
{
    ...
    forboth(lcc, index_path->indexorderbys, lci, index_path->indexorderbycols)
    {
        ...
        if (index->relam == bm25_AM_OID) {
            clause = fix_bm25_orderby_qual(clause, index);
...
static VectorIndexScan* make_vectorindexscan(List* qptlist, List* qpqual, Index scanrelid,
    Oid indexid, List* indexqual, List* indexqualorig, List* indexorderby, List* indexorderbyorig,
    List* indextlist, ScanDirection indexscandir, double indexselectivity, bool is_partial,
    AttrNumber score_plot)
{
    VectorIndexScan* node = makeNode(VectorIndexScan);
    Plan* plan = &node->scan.plan;
    ...
    node->selectivity = indexselectivity;
    node->is_partial = is_partial;
    if (indextlist != NIL && strncmp("internal_result", ((TargetEntry *)llast(indextlist))->resname, 15) == 0) {
        node->has_score_output = false;
    } else {
        node->has_score_output = false;
    }
    node->indextlist = indextlist;
    node->score_plot = score_plot;
    return node;
}

static bool is_bm25_rank_op(Oid opno)
{
    return opno >= 5048u && opno <= 5053u;
}

static bool examine_bm25_rank_op(Index index_idx, List *indexqual)
{
    Bitmapset *varattnos = NULL;
    uint32 count = 0;
    foreach_cell(lc, indexqual) {
        OpExpr *op = (OpExpr *)lfirst(lc);
        if (!IsA(op, OpExpr) || !is_bm25_rank_op(op->opno)) {
            continue;
        }
        ++count;
        foreach_cell(lc2, op->args) {
            Var *var = (Var *)lfirst(lc2);
            if (!IsA(var, Var)) {
                continue;
            }
            if (var->varno != index_idx && var->varno != INDEX_VAR) {
                ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("FULLTEXT operator can only be used in on the indexed column")));
            }
...
static TextMatchIndexScan* make_textmatchindexscan(List* qptlist, List* qpqual, Index scanrelid,
    ...
    AttrNumber score_plot)
{
    TextMatchIndexScan* node = makeNode(TextMatchIndexScan);
    Plan* plan = &node->scan.plan;
    ...
    if (examine_bm25_rank_op(scanrelid, indexqual) && list_length(indextlist) > 0 &&
        strncmp("internal_result", ((TargetEntry *)llast(indextlist))->resname, 15) == 0) {
        node->has_score_output = true;
    } else {
        node->has_score_output = false;
    }
    node->indextlist = indextlist;
    ...
    Oid** p_sortOperators, Oid** p_collations, bool** p_nullsFirst)
{
    ...
    foreach (i, pathkeys) {
        ...
        if (ec->ec_has_non_index_volatile) {
            /*
...
bool IsPlanForPartitionScan(Plan* plan)
{
    ...
    if (IsA(plan, SeqScan) || IsA(plan, IndexScan) || IsA(plan, IndexOnlyScan) || IsA(plan, BitmapHeapScan) ||
        ...
        IsA(plan, VectorIndexScan) || IsA(plan, TextMatchIndexScan)) {
        return true;
    }
