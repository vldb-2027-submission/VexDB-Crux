...
bool contain_mutable_functions(Node* clause)
{
    ...
    context.deep = false;
    context.ignore_index_func = false;
 ...
 */
bool contain_volatile_functions(Node* clause, bool deep, bool ignore_index_func)
{
    check_function_context context;
...
static bool func_meet_check_conditon(Oid functionid, check_function_context* context)
{
    if (context->checktype == CONTAIN_FUNCTION_ID) {
        ...
        if (context->ignore_index_func && functionid == F_BM25_SCORE) {
            return false;
        }
