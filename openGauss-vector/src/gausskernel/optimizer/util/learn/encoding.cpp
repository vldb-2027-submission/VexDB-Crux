...
const OperationInfo G_OPERATION_INFO_TABLE[G_MAX_OPERATION_NUMBER] = {
    ...
    {T_VectorIndexScan,     TEXT_OPTNAME_SCAN,              TEXT_STRATEGY_SCAN_VECTOR_INDEX},
    {T_TextMatchIndexScan,  TEXT_OPTNAME_SCAN,              TEXT_STRATEGY_SCAN_TEXT_MATCH_INDEX},
...
static inline bool IsScan(Plan* plan)
{
    return IsA(plan, SubqueryScan) || IsA(plan, Scan) || IsA(plan, SeqScan) || IsA(plan, IndexScan) ||
           ...
           IsA(plan, VecBitmapHeapScan) || IsA(plan, VectorIndexScan) || IsA(plan, TextMatchIndexScan);
}
