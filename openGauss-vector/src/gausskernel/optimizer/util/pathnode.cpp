    ...
    Relids required_outer, Bitmapset *upper_params, double loop_count)
{
    ...
    if (indexonly) {
        pathnode->path.pathtype = T_IndexOnlyScan;
    } else if (index->relam == IVFFLAT_AM_OID || index->relam == IVFPQ_AM_OID ||
               index->relam == diskann_AM_OID || index->relam == hnsw_AM_OID ||
               index->relam == hybridann_AM_OID || index->relam == graph_index_AM_OID) {
        pathnode->path.pathtype = T_VectorIndexScan;
    } else if (index->relam == bm25_AM_OID) {
        pathnode->path.pathtype = T_TextMatchIndexScan;
    } else {
