...
/* now we only assume there are only one expr can be provided at maximum */
static Expr *retrieve_index_provided_target_entry(PlannerInfo* root, IndexOptInfo *index, Relation heap_rel)
{
    if (index->relam == diskann_AM_OID || index->relam == hnsw_AM_OID ||
        index->relam == IVFFLAT_AM_OID || index->relam == IVFPQ_AM_OID ||
        index->relam == hybridann_AM_OID || index->relam == graph_index_AM_OID) {
        const AttrNumber attr_num = index->indexkeys[0];
        Expr *left_arg;
        Oid type_oid = index->opcintype[0];;
        Assert(type_oid == FLOATVECTOROID || type_oid == HALFVECTOROID);
        if (attr_num == 0) {
            Assert(list_length(index->indexprs) > 0);
            left_arg = (Expr *)linitial(index->indexprs);
        } else {
            left_arg = (Expr *)makeVar(index->rel->relid, attr_num, type_oid,
                heap_rel->rd_att->attrs[attr_num - 1].atttypmod, index->indexcollations[0], 0);
        }

        Const *right_arg = makeConst(type_oid, -1, InvalidOid, 0,
                                     PointerGetDatum(NULL), false, false);
        Oid opno = InvalidOid;
        Oid opfamily = index->opfamily[0];
        if (type_oid == FLOATVECTOROID) {
            if (opfamily == 7817u || opfamily == 7880u || opfamily == 8007u ||
                opfamily == 7820u || opfamily == 7869u || opfamily == 7890u) {
                opno = 5041u;
            } else if (opfamily == 7836u || opfamily == 7838u || opfamily == 8008u ||
                       opfamily == 7821u || opfamily == 7870u || opfamily == 7891u) {
                opno = 5040u;
            } else {
                opno = 5037u;
                Assert(opfamily == 7816u || opfamily == 7879u || opfamily == 7818u ||
                       opfamily == 7819u || opfamily == 7868u || opfamily == 7889u);
            }
        } else if (type_oid == HALFVECTOROID) {
            if (opfamily == 7901u) {
                opno = 5073u;
            } else if (opfamily == 7902u) {
                opno = 5072u;
            } else {
                opno = 5069u;
                Assert(opfamily == 7900u);
            }
        } else {
            return NULL;
        }

        Expr *opexpr = make_opclause(opno, FLOAT8OID, false, left_arg, (Expr *)right_arg,
                                     InvalidOid, InvalidOid);
        return opexpr;
    }
    if (index->relam == bm25_AM_OID) {
        return (Expr *)makeFuncExpr(F_BM25_SCORE, FLOAT4OID, NIL, InvalidOid, InvalidOid, COERCE_DONTCARE);
    }
    return NULL;
}
...
List* build_index_tlist(PlannerInfo* root, IndexOptInfo* index, Relation heapRelation)
{
    ...
    Expr *index_provided_expr = retrieve_index_provided_target_entry(root, index, heapRelation);
    if (index_provided_expr) {
        TargetEntry *tle = makeTargetEntry(index_provided_expr, i + 1, "internal_result", false);
