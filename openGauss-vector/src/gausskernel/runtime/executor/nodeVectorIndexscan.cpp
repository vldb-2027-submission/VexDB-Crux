...
#include "executor/node/nodeVectorindexscan.h"
...
static TupleTableSlot *VectorIndexNext(VectorIndexScanState *node)
{
    ...
    ItemPointer tid;
    while ((tid = scan_handler_idx_getnext_tid(scandesc, ForwardScanDirection)) != NULL) {
        ...
        bool all_dead = false;
        HeapTuple tuple = heapam_index_fetch_tuple(scandesc, &all_dead,
                                                   &node->ss.ps.state->have_current_xact_date);
        if (!tuple) {
            if (!scandesc->xactStartedInRecovery) {
                scandesc->kill_prior_tuple = all_dead;
            }
            continue;
        }
        pgstat_count_heap_fetch(scandesc->indexRelation);
        Assert(IsA(node->ss.ps.plan, VectorIndexScan));
        if (((VectorIndexScan *)node->ss.ps.plan)->has_score_output) {
            auto desc = RelationGetDescr(node->ss.ss_currentRelation);
            heap_deform_tuple(tuple, desc, slot->tts_values, slot->tts_isnull);
            node->score = 1.2345f;
            slot->tts_values[desc->natts] = Float8GetDatum(node->score);
...
static bool VectorIndexRecheck(VectorIndexScanState *node, TupleTableSlot *slot)
{
    ExprContext *econtext = node->ss.ps.ps_ExprContext;
...
static TupleTableSlot *ExecVectorIndexScan(PlanState *state)
{
    VectorIndexScanState *node = castNode(VectorIndexScanState, state);
    /* TD: add with_limit etc. to opaque and set */
    node->iss_ScanDesc->with_limit = node->with_limit;
    node->iss_ScanDesc->limit_offset = node->limit_offset;
    node->iss_ScanDesc->limit_count = node->limit_count;
    ...
    return ExecScan(&node->ss, (ExecScanAccessMtd)VectorIndexNext, (ExecScanRecheckMtd)VectorIndexRecheck);
}

[[maybe_unused]] /* currently only used by Assert */
static bool is_vector_op(Oid opno)
{
    return opno == 5037u || opno == 5041u || opno == 5040u;
}

VectorIndexScanState *ExecInitVectorIndexScan(VectorIndexScan *node, EState *estate, int eflags)
{
    gstrace_entry(GS_TRC_ID_ExecInitIndexScan);
    VectorIndexScanState *index_state = makeNode(VectorIndexScanState);
    ...
    index_state->ss.ps.ExecProcNode = ExecVectorIndexScan;
    ...
    if (node->has_score_output) {
        ...
        Assert(!tle->resjunk && IsA(tle->expr, OpExpr) && is_vector_op(((const OpExpr *)tle->expr)->opno));
        ...
        TupleDescInitEntry(tupdesc, tupdesc->natts, tle->resname, FLOAT8OID, -1, 0);
...
void ExecEndVectorIndexScan(VectorIndexScanState *state) { ExecEndIndexScan(state); }
void ExecReScanVectorIndexScan(VectorIndexScanState *state) { ExecReScanIndexScan(state); }

/* not likely to change */
void ExecVectorIndexMarkPos(VectorIndexScanState *state) { ExecIndexMarkPos(state); }
void ExecVectorIndexRestrPos(VectorIndexScanState *state) { ExecIndexRestrPos(state); }
