...
static void setPlanStateLimitStatus(PlanState *state, bool with_limit, int64 limit_offset, int64 limit_count)
{
    while (state) {
        if (IsA(state, VectorIndexScanState)) {
            VectorIndexScanState *node = castNode(VectorIndexScanState, state);
            node->with_limit = with_limit;
            node->limit_offset = limit_offset;
            node->limit_count = limit_count;
        } else if (IsA(state, TextMatchIndexScanState)) {
...
static TupleTableSlot* ExecLimit(PlanState* state) /* return: a tuple or NULL */
{
    ...
    switch (node->lstate) {
        case LIMIT_INITIAL:
            ...
            setPlanStateLimitStatus(outer_plan, !node->noCount, node->offset, node->count);
