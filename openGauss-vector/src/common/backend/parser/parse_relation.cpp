...
char* get_rte_attribute_name(RangeTblEntry* rte, AttrNumber attnum)
{
    ...
    if (rte->rtekind == RTE_RELATION) {
        ...
        int nattr = rel->rd_att->natts;
        if (nattr + 1 != attnum) {
            relation_close(rel, AccessShareLock);
            ereport(ERROR, (errcode(ERRCODE_CACHE_LOOKUP_FAILED),
                errmsg("cache lookup failed for attribute %d of relation %u", attnum, rte->relid)));
        }
        int len = list_length(rel->rd_indexlist);
        if (len <= 0) {
            relation_close(rel, AccessShareLock);
            ereport(ERROR, (errcode(ERRCODE_CACHE_LOOKUP_FAILED),
                errmsg("cache lookup failed for attribute %d of relation %u", attnum, rte->relid)));
        }
        Oid *index_oids = (Oid *)palloc(sizeof(Oid) * len);
        Oid *cur = index_oids;
        foreach_cell(lc, rel->rd_indexlist) {
            *cur++ = lfirst_oid(lc);
        }
        relation_close(rel, AccessShareLock);
        for (int i = 0; i < len; i++) {
            HeapTuple tuple = SearchSysCache1(RELOID, ObjectIdGetDatum(index_oids[i]));
            if (HeapTupleIsValid(tuple)) {
                Form_pg_class pg_class_tuple = (Form_pg_class)GETSTRUCT(tuple);
                Oid amgettuple_oid = pg_class_tuple->relam;
                ReleaseSysCache(tuple);
                /* vector index result not implemented yet */
                if (amgettuple_oid == bm25_AM_OID) {
                    pfree(index_oids);
                    return pstrdup("index_provided_result");
                }
            }
        }
        pfree(index_oids);
