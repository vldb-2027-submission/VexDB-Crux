...
IndexStmt* transformIndexStmt(Oid relid, IndexStmt* stmt, const char* queryString)
{
    ...
    if (stmt->accessMethod == NULL) {
        ...
        if (pg_strcasecmp(stmt->accessMethod, DEFAULT_INDEX_TYPE) != 0 &&
            ...
            pg_strcasecmp(stmt->accessMethod, DEFAULT_BM25_INDEX_TYPE) != 0 ) {
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), 
                    errmsg("Global partition index only support btree and fulltext.")));
        }
        if (isColStore) {
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), 
                    errmsg("Global partition index does not support column store.")));
        ...
        if (!isColStore && (0 != pg_strcasecmp(stmt->accessMethod, DEFAULT_INDEX_TYPE)) &&
            ...
            (0 != pg_strcasecmp(stmt->accessMethod, DEFAULT_HYBRIDANN_INDEX_TYPE)) &&
            (0 != pg_strcasecmp(stmt->accessMethod, DEFAULT_IVFFLAT_INDEX_TYPE)) &&
            (0 != pg_strcasecmp(stmt->accessMethod, DEFAULT_IVFPQ_INDEX_TYPE)) &&
