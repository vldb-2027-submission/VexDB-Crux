...
FuncCandidateList func_select_candidate(int nargs, Oid* input_typeids, FuncCandidateList candidates)
{
    ...
    if (nunknowns < nargs) {
        ...
        if (known_type != UNKNOWNOID) {
            ...
            for (current_candidate = candidates; current_candidate != NULL;
                ...
                current_typeids = current_candidate->args;
                /* To resolve the conflict between floatvector and array,
                 * we requrie the use of floatvector to be explicit since it is specialized.
                 * Hence if there are candidates involving floatvector, let it resign. */
                bool has_vector = false;
                for (i = 0; i < nargs; i++) {
                    if (current_typeids[i] == FLOATVECTOROID ||
                        current_typeids[i] == HALFVECTOROID) {
                        has_vector = true;
                    }
                }
    ...
    /* try candidates with minimal transformation, only applies to numbers */
    last_candidate = NULL;
    ncandidates = 0;
    nbestMatch = 0;
    int min_priority_diff = INT_MAX;
    int min_total_priority_diff = INT_MAX;
    for (current_candidate = candidates; current_candidate != NULL; current_candidate = current_candidate->next) {
        current_typeids = current_candidate->args;
        nmatch = 0;
        int cur_min_priority_diff = 0;
        int cur_total_priority_diff = 0;
        for (i = 0; i < nargs; i++) {
            int slot_cat = GetCategoryPriority(slot_category[i]);
            int curr_cat = GetCategoryPriority(TypeCategory(current_typeids[i]));
            if (slot_cat < curr_cat) {
                nmatch++;
            } else if (slot_cat == curr_cat && slot_category[i] == 'N') {
                int current_priority = GetPriority(current_typeids[i]);
                int priority_diff = current_priority - GetPriority(input_typeids[i]);
                nmatch += priority_diff >= 0 ? 1 : 0;
                if (priority_diff > 0) {
                    cur_total_priority_diff += priority_diff;
                    if (priority_diff > cur_min_priority_diff) {
                        cur_min_priority_diff = priority_diff;
                    }
                }
            }
        }
        if (nmatch > nbestMatch || !last_candidate) {
            min_priority_diff = cur_min_priority_diff;
            min_total_priority_diff = cur_total_priority_diff;
            nbestMatch = nmatch;
            candidates = current_candidate;
            last_candidate = current_candidate;
            ncandidates = 1;
            ...
            continue;
        } else if (cur_min_priority_diff < min_priority_diff) {
            min_priority_diff = cur_min_priority_diff;
            min_total_priority_diff = cur_total_priority_diff;
            candidates = current_candidate;
            last_candidate = current_candidate;
            ...
            continue;
        } else if (cur_total_priority_diff < min_total_priority_diff) {
            min_total_priority_diff = cur_total_priority_diff;
            candidates = current_candidate;
            last_candidate = current_candidate;
            ncandidates = 1;
