        ...
        if (gs_signal_send((*pids)[i], SIGTERM, (*isthreadpool)[i]) < 0) {
            ...
            ereport(ERROR,
                (errcode(ERRCODE_SYSTEM_ERROR),
                    errmsg("pooler: Failed to send SIGTERM to vexdb thread:%lu in "
    ...
    if (clear) {
        ...
        for (i = 0; i < (int)count; i++) {
            if (gs_signal_send(entry[i].send_sig_pid, SIGUSR2, entry[i].is_thread_pool_session) < 0) {
                ereport(WARNING,
                    (errmsg("pooler: Failed to send SIGUSR2 to vexdb thread:%lu in "
                            "PoolManagerValidateConnection(), failed: %m",
                        entry[i].tid)));
            } else {
                ereport(LOG,
                    (errmsg("pooler: Success to send SIGUSR2 to vexdb thread:%lu in "
