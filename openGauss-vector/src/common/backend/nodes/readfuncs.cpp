...
static VectorIndexScan* _readVectorIndexScan(VectorIndexScan* local_node)
{
    READ_LOCALS_NULL(VectorIndexScan);
    _readIndexScan(local_node);
    return local_node;
}

static TextMatchIndexScan* _readTextMatchIndexScan(TextMatchIndexScan* local_node)
{
    READ_LOCALS_NULL(TextMatchIndexScan);
    _readIndexScan(local_node);
    return local_node;
}
...
#define MATCH(tokname, namelen) ((length == (namelen)) && (memcmp(token, (tokname), (namelen)) == 0))
    ...
    } else if (MATCH("VECTORINDEXSCAN", 16)) {
        return_value = _readVectorIndexScan(NULL);
    } else if (MATCH("TEXTMATCHINDEXSCAN", 19)) {
        return_value = _readTextMatchIndexScan(NULL);
