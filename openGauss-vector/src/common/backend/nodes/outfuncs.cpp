...
static void _outVectorIndexScan(StringInfo str, VectorIndexScan *scan)
{
    WRITE_NODE_TYPE("VECTORINDEXSCAN");
    _outCommonIndexScanPart<VectorIndexScan>(str, scan);
    {
        const IndexScan *node = (IndexScan *)scan;
        if (t_thrd.proc->workingVersionNum >= INPLACE_UPDATE_VERSION_NUM) {
            WRITE_BOOL_FIELD(is_ustore);
        }
        if (t_thrd.proc->workingVersionNum >= PLAN_SELECT_VERSION_NUM) {
...
static void _outTextMatchIndexScan(StringInfo str, TextMatchIndexScan *scan)
{
    WRITE_NODE_TYPE("TEXTMATCHINDEXSCAN");
    _outCommonIndexScanPart<TextMatchIndexScan>(str, scan);
    {
        const IndexScan *node = (IndexScan *)scan;
        if (t_thrd.proc->workingVersionNum >= INPLACE_UPDATE_VERSION_NUM) {
            WRITE_BOOL_FIELD(is_ustore);
    ...
    if (obj == NULL) {
        ...
        switch (nodeTag(obj)) {
            ...
            case T_VectorIndexScan:
                _outVectorIndexScan(str, (VectorIndexScan*)obj);
                break;
            case T_TextMatchIndexScan:
                _outTextMatchIndexScan(str, (TextMatchIndexScan*)obj);
                break;
