...
static void CopyIndexFields(const IndexScan* from, IndexScan* newnode)
{
    /* copy node superclass fields */
    CopyScanFields((const Scan*)from, (Scan*)newnode);
...
static IndexScan* _copyIndexScan(const IndexScan* from)
{
    ...
    CopyIndexFields(from, newnode);
    return newnode;
}

static VectorIndexScan *_copyVectorIndexScan(const VectorIndexScan *from)
{
    VectorIndexScan *newnode = makeNode(VectorIndexScan);
    CopyIndexFields(from, newnode);
    COPY_SCALAR_FIELD(has_score_output);
    COPY_NODE_FIELD(indextlist);
    COPY_SCALAR_FIELD(score_plot);
    return newnode;
}

static TextMatchIndexScan *_copyTextMatchIndexScan(const TextMatchIndexScan *from)
{
    TextMatchIndexScan *newnode = makeNode(TextMatchIndexScan);
    CopyIndexFields(from, newnode);
    COPY_SCALAR_FIELD(has_score_output);
    COPY_NODE_FIELD(indextlist);
    COPY_SCALAR_FIELD(score_plot);
...
void* copyObject(const void* from)
{
    ...
    switch (nodeTag(from)) {
        ...
        case T_VectorIndexScan:
            retval = _copyVectorIndexScan((VectorIndexScan*)from);
            break;
        case T_TextMatchIndexScan:
            retval = _copyTextMatchIndexScan((TextMatchIndexScan*)from);
            break;
