...
void index_drop(Oid indexId, bool concurrent, bool concurrent_lock_mode)
{
    ...
    /* Drop all backend indexer tasks belonging to the dropped index */
    vector_indexer_kill_tasks(userIndexRelation);

    /* release index in bulkbuffer */
    if (BULKBUF_SUPPORT(userIndexRelation)) {
        BULKBUF_MGR->auto_index_release(userIndexRelation);
    }

    /* erase timering */
    if (IS_GRAPH_INDEX(userIndexRelation)) {
        QtUpdateMgr *qt_update_mgr = (QtUpdateMgr *)g_instance.annvec_cxt.qt_update_mgr;
        if (!RelationIsPartitioned(userIndexRelation)) {
            qt_update_mgr->erase_timering(userIndexRelation->rd_id);
        } else {
            List *l = indexGetPartitionList(userIndexRelation, AccessShareLock);
            foreach_cell(lc, l) {
                Partition part = (Partition)lfirst(lc);
                Relation cindex = partitionGetRelation(userIndexRelation, part);
                qt_update_mgr->erase_timering(cindex->rd_id);
                releaseDummyRelation(&cindex);
            }
            releasePartitionList(userIndexRelation, &l, AccessShareLock);
        }
    }
                        ...
                        TableScanDesc scan)
{
	...
	Assert(snapshot == SnapshotAny || !anyvisible);
	 ...
	 /* temporarily disable the progress publish,
      because it depends some new data definition and useless for now */
	...
	// {
		...
		// 	pbscan = (ParallelBlockTableScanDesc) scan->rs_parallel;
	...
	recheck:
											 ...
											 scan->rs_cbuf))
			{
				...
				case HEAPTUPLE_INSERT_IN_PROGRESS:
					 ...
					 * applies.
					 */
					xwait = HeapTupleHeaderGetXmin(BufferGetPage(scan->rs_cbuf), heapTuple->t_data);
					if (!TransactionIdIsCurrentTransactionId(xwait))
					{
						if (!is_system_catalog)
				...
				case HEAPTUPLE_DELETE_IN_PROGRESS:
					...
					xwait = HeapTupleHeaderGetUpdateXid(BufferGetPage(scan->rs_cbuf), heapTuple->t_data);
					if (!TransactionIdIsCurrentTransactionId(xwait))
					{
						if (!is_system_catalog)
		...
		{
			/* Call the AM's callback routine to process the tuple */
			callback(indexRelation, heapTuple, values, isnull,
    ...
    void* baseDesc)
{
    ...
    // release partindex in bulk buffer
    BULKBUF_MGR->auto_partindex_release(indexPartId);
}
                   ...
                   AdaptMem *memInfo, bool dbWide, void *baseDesc, bool isTruncGTT)
{
    ...
    PG_TRY();
    {
        ...
        if (!RELATION_IS_PARTITIONED(heapRelation)) /* for non partitioned table */
        {
            ...
            /* release index in bulkbuffer */
            if (BULKBUF_SUPPORT(iRel)) {
                BULKBUF_MGR->auto_index_release(iRel);
            }
