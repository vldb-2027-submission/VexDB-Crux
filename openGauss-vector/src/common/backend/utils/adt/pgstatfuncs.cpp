...
Datum pg_stat_segment_extent_usage(PG_FUNCTION_ARGS)
{
    ...
    if (forknum == BCM_FORKNUM || forknum == INIT_FORKNUM || forknum == VECTOR_FORKNUM) {
        ereport(ERROR, (errmodule(MOD_SEGMENT_PAGE), errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg("We do not support BCM_FORKNUM, INIT_FORKNUM and VECTOR_FORKNUM yet."),
                        errdetail("BCM_FORKNUM is 3, INIT_FORKNUM is 4, VECTOR_FORKNUM is 5.")));
    }
    FuncCallContext *funcctx = NULL;
