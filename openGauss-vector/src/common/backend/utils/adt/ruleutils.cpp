...
static void apply_indexdef(StringInfo buf, char *def_str)
{
    constexpr char *dict_def_str = "dicts=";
    constexpr char *alg_def_str = "algorithms=";
    constexpr char *coef_def_str = "coefficients=";

    char *dict_str = strstr(def_str, dict_def_str);
    char *alg_str = strstr(def_str, alg_def_str);
    char *coef_str = strstr(def_str, coef_def_str);
    if (!dict_str && !alg_str && !coef_str) {
        appendStringInfo(buf, " WITH (%s)", def_str);
        return;
    }

    const auto add_quote_str = [buf](char *start, char *s, size_t param_len) -> char * {
        char *str = s + param_len;
        char str_c = *str;
        *str = '\0';
        appendStringInfoString(buf, start);
        *str = str_c;
        appendStringInfoChar(buf, '\"');
        char *str_end = strchr(str, ',');
        if (str_end) {
            str_c = *str_end;
            *str_end = '\0';
    ...
    Map<char *, size_t> s;
    if (dict_str) {
        s.emplace(dict_str, strlen(dict_def_str));
    }
    if (alg_str) {
        s.emplace(alg_str, strlen(alg_def_str));
    }
    if (coef_str) {
        s.emplace(coef_str, strlen(coef_def_str));
    }

    appendStringInfoString(buf, " WITH (");
    for (auto p : s) {
        Assert(def_str);
        def_str = add_quote_str(def_str, p.first, p.second);
    }
    if (def_str) {
        appendStringInfoString(buf, def_str);
    }
    appendStringInfoChar(buf, ')');
