...
void InitializeSessionUserIdStandalone(void)
{
    ...
    AssertState(!IsUnderPostmaster || IsAutoVacuumWorkerProcess() || IsJobSchedulerProcess() || IsJobWorkerProcess() ||
                ...
                IsRbWorkerProcess() || t_thrd.role == PARALLEL_DECODE || t_thrd.role == VECINDEX_WORKER || t_thrd.role == LOGICAL_READ_RECORD);
#endif   /* ENABLE_MULTIPLE_NODES */
...
#endif
                {
                    ...
                    ereport(FATAL,
                        (errcode(ERRCODE_LOCK_FILE_EXISTS),
                            ...
                            isDDLock
                                ? ((encoded_pid < 0)
                                          ? errhint("Is another vexdb (PID %d) running in data directory \"%s\"?",
                                                (int)other_pid,
                                                refName)
                                          : errhint("Is another postmaster (PID %d) running in data directory \"%s\"?",
                                                (int)other_pid,
                                                refName))
                                : ((encoded_pid < 0) ? errhint("Is another vexdb (PID %d) \
                                                                using socket file \"%s\"?",
                                                         (int)other_pid,
                                                         refName)
