...
typedef struct ResourceOwnerData {
    ...
    /* track vector buffer pins */
    int nvecbuffers;
    VecBufferLoc* vecbuffers;
    int maxvecbuffers;

    /* track bm25 dictionary */
    int nbm25dicts;
    TokenizerResource **bm25dicts;
    int maxbm25dicts;
    ...
    ResourceOwner owner, ResourceReleasePhase phase, bool isCommit, bool isTopLevel)
{
    ...
    if (phase == RESOURCE_RELEASE_BEFORE_LOCKS) {
        ...
        while (owner->nvecbuffers > 0) {
            const auto &loc = owner->vecbuffers[owner->nvecbuffers - 1];
            if (isCommit)
                ereport(WARNING, (errmsg("vector buffer (%u,%u) reference leak", loc.buf_offset, loc.offset)));
            release_vector_buffer(loc);
        }
        while (owner->nbm25dicts > 0) {
            TokenizerResource *dict_resource = owner->bm25dicts[owner->nbm25dicts - 1];
            if (isCommit)
                ereport(WARNING, (errmsg("bm25 dict %u reference leak", dict_resource->dict_id)));
            bm25_tokenizer::release_bm25_dict(dict_resource);
        }
...
static void ResourceOwnerFreeOwner(ResourceOwner owner, bool whole)
{
    if (owner->valid) {
        ...
        pfree_ext(owner->vecbuffers);
        pfree_ext(owner->bm25dicts);
    }
...
static void ResourceOwnerConcatPart1(ResourceOwner target, ResourceOwner source)
{
    ...
    while (source->nvecbuffers > 0) {
        ResourceOwnerEnlargeVectorBuffers(target);
        ResourceOwnerRememberVectorBuffer(target, source->vecbuffers[--source->nvecbuffers]);
    }

    while (source->nbm25dicts > 0) {
        ResourceOwnerEnlargeBm25Dicts(target);
        ResourceOwnerRememberBm25Dict(target, source->bm25dicts[--source->nbm25dicts]);
    }
}
...
void ResourceOwnerConcat(ResourceOwner target, ResourceOwner source)
{
    ...
    Assert(sizeof(ResourceOwnerData) == 480ul); /* The current size of ResourceOwnerData is 496 */
...
void ResourceOwnerEnlargeVectorBuffers(ResourceOwner owner)
{
    if (owner == NULL || owner->nvecbuffers < owner->maxvecbuffers) {
        return; /* nothing to do */
    }
    int newmax;
    if (owner->vecbuffers == NULL) {
        newmax = 4;
        owner->vecbuffers = (VecBufferLoc*)MemoryContextAlloc(owner->memCxt, newmax * sizeof(VecBufferLoc));
        owner->maxvecbuffers = newmax;
    } else {
        newmax = owner->maxvecbuffers * 2;
        owner->vecbuffers = (VecBufferLoc*)repalloc(owner->vecbuffers, newmax * sizeof(VecBufferLoc));
        owner->maxvecbuffers = newmax;
    }
}

void ResourceOwnerEnlargeBm25Dicts(ResourceOwner owner)
{
    if (owner == NULL || owner->nbm25dicts < owner->maxbm25dicts) {
        return; /* nothing to do */
    }
    int newmax;
    if (owner->bm25dicts == NULL) {
        newmax = 4;
        owner->bm25dicts = (TokenizerResource **)MemoryContextAlloc(owner->memCxt, newmax * sizeof(TokenizerResource *));
        owner->maxbm25dicts = newmax;
    } else {
        newmax = owner->maxbm25dicts * 2;
        owner->bm25dicts = (TokenizerResource **)repalloc(owner->bm25dicts, newmax * sizeof(TokenizerResource *));
        owner->maxbm25dicts = newmax;
    }
}
...
FORCE_INLINE
void ResourceOwnerRememberVectorBuffer(ResourceOwner owner, const VecBufferLoc &buffer)
{
    if (owner != NULL) {
        Assert(owner->nvecbuffers < owner->maxvecbuffers);
        owner->vecbuffers[owner->nvecbuffers] = buffer;
        owner->nvecbuffers++;
    }
}

FORCE_INLINE
void ResourceOwnerRememberBm25Dict(ResourceOwner owner, const void *d)
{
    TokenizerResource *dict = (TokenizerResource *)d;
    if (owner != NULL) {
        Assert(owner->nbm25dicts < owner->maxbm25dicts);
        owner->bm25dicts[owner->nbm25dicts] = dict;
        owner->nbm25dicts++;
    }
}
...
void ResourceOwnerForgetVectorBuffer(ResourceOwner owner, const VecBufferLoc &buffer)
{
    if (owner != NULL && owner->valid) {
        VecBufferLoc* buffers = owner->vecbuffers;
        int nb1 = owner->nvecbuffers - 1;
        int i;
        ...
        for (i = nb1; i >= 0; i--) {
            if (buffers[i] == buffer) {
                ...
                owner->nvecbuffers = nb1;
                return;
            }
        }
        ereport(ERROR, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_GRANTED),
            errmsg("vector buffer (%u,%u) is not owned by resource owner %s",
                   buffer.buf_offset, buffer.offset, owner->name)));
    }
}

void ResourceOwnerForgetBm25Dict(ResourceOwner owner, const void *d)
{
    TokenizerResource *dict = (TokenizerResource *)d;
    if (owner != NULL && owner->valid) {
        TokenizerResource **dicts = owner->bm25dicts;
        int nb1 = owner->nbm25dicts - 1;
        int i;
        ...
        for (i = nb1; i >= 0; i--) {
            if (dicts[i] == dict) {
                ...
                owner->nbm25dicts = nb1;
                return;
            }
        }
        ereport(ERROR, (errcode(ERRCODE_WARNING_PRIVILEGE_NOT_GRANTED),
                        errmsg("bm25 dict %u is not owned by resource owner %s",
                               dict->dict_id, owner->name)));
    }
}
...
bool CurrentResourceOwnerIsEmpty(ResourceOwner owner)
{
    ...
    Assert(owner->nvecbuffers == 0);
    Assert(owner->nbm25dicts == 0);
    return true;
}
/*
