...
struct Tuplesortstate {
    ...
    int forward_sorted_pos{0}; /* before this pos, data has been sorted */
    bool partially_sorted{false};
...
static int  comparetup_tiddatum(const SortTuple* a, const SortTuple* b, Tuplesortstate* state);
static void copytup_tiddatum(Tuplesortstate* state, SortTuple* stup, void* tup);
static void writetup_tiddatum(Tuplesortstate* state, int tapenum, SortTuple* stup);
static void readtup_tiddatum(Tuplesortstate* state, SortTuple* stup, int tapenum, unsigned int len);
static void reversedirection_tiddatum(Tuplesortstate* state);
...
void tuplesort_set_state_limitinfo(Tuplesortstate* state, int64 limit_offset, int64 limit_count)
{
    state->with_limit = true;
    state->limit_offset = limit_offset;
    state->limit_count = limit_count;
}
/*
Plesase be noted that the datum should be fixed length data type.
*/
Tuplesortstate* tuplesort_begin_tiddatum(Oid sortOperator, Oid sortCollation, bool nullsFirstFlag, int workMem, bool randomAccess)
{
...
#ifdef TRACE_SORT
    if (u_sess->attr.attr_common.trace_sort) {
        elog(LOG, "begin tid and datum sort: workMem = %d, randomAccess = %c", workMem, randomAccess ? 't' : 'f');
    }
#endif
    ...
    state->comparetup = comparetup_tiddatum;
    state->copytup = copytup_tiddatum;
    state->writetup = writetup_tiddatum;
    state->readtup = readtup_tiddatum;
#ifdef PGXC
    state->getlen = getlen;
#endif
    state->reversedirection = reversedirection_tiddatum;
 ...
 */
ScanKey _hybridbt_mkscankey_nodata(Relation rel)
{
    ScanKey skey;
    int indnkeyatts;
    ...
    /*skip fill the fisrt colum as it doesn't participate sort*/
    for (i = 1; i < indnkeyatts; i++) {
         ...
         */
        procinfo = index_getprocinfo(rel, i + 1, (uint16)disk_container::HYBRID_BTORDER_PROC);
...
Tuplesortstate* tuplesort_begin_index_btree(
    Relation indexRel, bool enforceUnique, int workMem, SortCoordinate coordinate, bool randomAccess, int maxMem, TupleDesc tupDesc, uint32 firstColAttrNum)
{
    Tuplesortstate* state = tuplesort_begin_common(workMem, randomAccess, coordinate);
    MemoryContext oldcontext;
    ...
    state->firstColAttrNum = firstColAttrNum;
    if (state->firstColAttrNum == 1) {
        state->indexScanKey = _bt_mkscankey_nodata(indexRel);
    } else {
        state->indexScanKey = _hybridbt_mkscankey_nodata(indexRel);
    }
    ...
    Tuplesortstate* state, Relation rel, ItemPointer self, Datum* values, const bool* isnull)
{
    ...
    SortTuple stup;
    stup.tupindex = 0;
    TupleDesc tupdesc = state->tupDesc == NULL ? RelationGetDescr(rel) : state->tupDesc;
    stup.tuple = index_form_tuple(tupdesc, values, isnull);

    ((IndexTuple)stup.tuple)->t_tid = *self;
    USEMEM(state, GetMemoryChunkSpace(stup.tuple));
    /* set up first-column key value */
    stup.datum1 = index_getattr((IndexTuple)stup.tuple, state->firstColAttrNum, tupdesc, &stup.isnull1);
...
/*
 * Accept one t_tid and Datum while collecting input data for sort.
 *
 */
void tuplesort_puttiddatum(Tuplesortstate* state, const ItemPointerData &t_tid, const Datum &val)
{
    MemoryContext oldcontext = MemoryContextSwitchTo(state->tuplecontext);
    SortTuple stup;
    stup.tupindex = 0;
    ...
    stup.datum1 = val;
    stup.isnull1 = false;
    stup.t_tid = t_tid; /* no separate storage */
    MemoryContextSwitchTo(state->sortcontext);
...
void sorttuple_sort(Tuplesortstate* state, SortTuple* sortTuples, int count, int startIndex, int k)
{
    qsort_ssup_topk(sortTuples, count, state->onlyKey, startIndex, k);
}
        ...
        case TSS_SORTEDINMEM:
            ...
            if (forward) {
                if (state->current < state->memtupcount) {
                    if(state->partially_sorted && state->current > state->forward_sorted_pos) {
                        qsort_ssup_topk(state->memtuples, state->memtupcount, state->onlyKey, state->forward_sorted_pos + 1, state->limit_count);
                        state->partially_sorted = (state->forward_sorted_pos + 1 + state->limit_count) < state->memtupcount -1;
                        state->forward_sorted_pos = std::min(state->forward_sorted_pos + 1 + state->limit_count, int64(state->memtupcount -1));
...
bool tuplesort_gettid(Tuplesortstate* state, bool forward, ItemPointerData* tid)
{
    MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
    SortTuple stup;
...
bool tuplesort_getsorttuple(Tuplesortstate* state, bool forward, SortTuple* stup)
{
    MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
...
static void init_tape_buffers(Tuplesortstate *state, int numInputTapes)
{
    ...
    for (tapenum = 0; tapenum < state->maxTapes; tapenum++) {
        ...
        LogicalTapeAssignReadBufferSize(state->tapeset, tapenum, numBlocks * BLCKSZ);
    }
}
...
static void mergeonerun(Tuplesortstate* state)
{
    ...
    while (state->memtupcount > 0) {
        ...
        if (!state->tid_datum_type && state->memtuples[0].tuple) {
...
static void tuplesort_sort_memtuples(Tuplesortstate *state)
{
    if (state->memtupcount > 1) {
        if (state->onlyKey != NULL) {
            if (state->status ==  TSS_INITIAL && state->with_limit 
                 && (state->limit_offset + state->limit_count) < state->memtupcount) {
                qsort_ssup_topk(state->memtuples, state->memtupcount, state->onlyKey, 0, (state->limit_offset + state->limit_count));
                state->partially_sorted = true; 
                state->forward_sorted_pos = (state->limit_offset + state->limit_count);
            } else {
...
static int comparetup_index_btree(const SortTuple* a, const SortTuple* b, Tuplesortstate* state)
{
    ...
    scanKey += (state->firstColAttrNum - 1);
    ...
    keysz = state->nKeys;
    tupDes = state->tupDesc == NULL ? RelationGetDescr(state->indexRel) : state->tupDesc;
    scanKey++;
    for (nkey = state->firstColAttrNum + 1; nkey <= keysz; nkey++, scanKey++) {
        Datum datum1, datum2;
        bool isnull1 = false, isnull2 = false;
...
static void copytup_index(Tuplesortstate* state, SortTuple* stup, void* tup)
{
    ...
    /* set up first-column key value */
    stup->datum1 = index_getattr(newtuple, state->firstColAttrNum, state->tupDesc == NULL ? RelationGetDescr(state->indexRel) : state->tupDesc, &stup->isnull1);
}
...
static void readtup_index(Tuplesortstate* state, SortTuple* stup, int tapenum, unsigned int len)
{
    ...
    stup->tuple = (void*)tuple;
    /* set up first-column key value */
    stup->datum1 = index_getattr(tuple, state->firstColAttrNum, state->tupDesc == NULL ? RelationGetDescr(state->indexRel) : state->tupDesc, &stup->isnull1);
}
...
static void copytup_tiddatum(Tuplesortstate* state, SortTuple* stup, void* tup)
{
    ...
    ereport(ERROR,
        (errmodule(MOD_EXECUTOR),
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("copytup_tiddatum() should not be called"))));

}
static void writetup_tiddatum(Tuplesortstate* state, int tapenum, SortTuple* stup)
{
    void* waddr = NULL;
    unsigned int tuptidlen;
    unsigned int tupdatumlen;
    unsigned int tuplen;
    unsigned int writtenlen;
    tuptidlen = sizeof(ItemPointerData);
    tupdatumlen = sizeof(Datum);
    tuplen = tuptidlen + tupdatumlen;
    writtenlen = tuplen + sizeof(unsigned int);
    LogicalTapeWrite(state->tapeset, tapenum, (void*)&writtenlen, sizeof(writtenlen));
    waddr = &stup->t_tid;
    LogicalTapeWrite(state->tapeset, tapenum, waddr, tuptidlen);
    waddr = &stup->datum1;
    LogicalTapeWrite(state->tapeset, tapenum, waddr, tupdatumlen);
...
static void readtup_tiddatum(Tuplesortstate* state, SortTuple* stup, int tapenum, unsigned int len)
{
    ...
    if (tuplen == 0) {
        /* it's NULL */
        ItemPointerSetInvalid(&(stup->t_tid));
        stup->datum1 = (Datum)0;
        stup->isnull1 = true;
    } else {
        Assert(tuplen == sizeof(ItemPointerData) + sizeof(Datum));
        LogicalTapeReadExact(state->tapeset, tapenum, &stup->t_tid, sizeof(ItemPointerData));
        LogicalTapeReadExact(state->tapeset, tapenum, &stup->datum1, sizeof(Datum));
        stup->isnull1 = false;
    }
