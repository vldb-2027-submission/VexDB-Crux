...
void *hash_seq_iter(HASH_SEQ_STATUS* status)
{
    HTAB* hashp = NULL;
    HASHHDR* hctl = NULL;
    uint32 max_bucket;
...
static HASHBUCKET hash_locate_bucket(HTAB* hashp, const void* keyPtr, uint32 hashvalue,
                                     HASHBUCKET*& prevBucketPtr)
{
    ...
    HashCompareFunc match = hashp->match;
    Size keysize = hashp->keysize;
...
void hash_iter_next(HASH_SEQ_STATUS* status)
{
    HTAB* hashp = NULL;
    HASHHDR* hctl = NULL;
    uint32 max_bucket;
    ...
    HASHSEGMENT segp;
    uint32 curBucket;
    HASHELEMENT* curElem = status->curEntry;
    ...
    status->curEntry = curElem;
    status->curBucket = curBucket;
}

void remove_hash_bucket(HTAB* hashp, HASHBUCKET removeBucket)
{
    HASHHDR* hctl = hashp->hctl;
    HASHBUCKET* prevBucketPtr = &removeBucket;
    hash_locate_bucket(hashp, ELEMENTKEY(removeBucket), removeBucket->hashvalue, prevBucketPtr);
    int freelist_idx = FREELIST_IDX(hctl, removeBucket->hashvalue);
    Assert(hctl->freeList[freelist_idx].nentries > 0);
    hctl->freeList[freelist_idx].nentries--;
    *prevBucketPtr = removeBucket->link;
    removeBucket->link = hctl->freeList[freelist_idx].freeList;
    hctl->freeList[freelist_idx].freeList = removeBucket;
}

template <HASHACTION action>
HASHBUCKET buf_hash_bucket(HTAB* hashp, const void* keyPtr, uint32 hashvalue, bool* foundPtr)
{
    if (action != HASH_FIND && action != HASH_ENTER && action != HASH_REMOVE) {
        elog(ERROR, "unrecognized hash action code: %d", (int)action);
        return NULL; /* keep compiler quiet */
    }

    HASHBUCKET* prevBucketPtr = NULL;
    HASHBUCKET currBucket = hash_locate_bucket(hashp, keyPtr, hashvalue, prevBucketPtr);
    *foundPtr = currBucket != NULL;
    if (action == HASH_FIND) {
        return currBucket;
    }
    /* Return existing element if found, else create one */
    if (action == HASH_ENTER && currBucket != NULL) {
        return currBucket;
    }
    HASHHDR* hctl = hashp->hctl;
    int freelist_idx = FREELIST_IDX(hctl, hashvalue);
...
template HASHBUCKET buf_hash_bucket<HASH_FIND>(HTAB*, const void*, uint32, bool*);
template HASHBUCKET buf_hash_bucket<HASH_ENTER>(HTAB*, const void*, uint32, bool*);
template HASHBUCKET buf_hash_bucket<HASH_REMOVE>(HTAB*, const void*, uint32, bool*);
