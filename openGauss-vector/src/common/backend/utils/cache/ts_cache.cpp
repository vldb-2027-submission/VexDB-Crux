...
TSDictionaryCacheEntry* lookup_ts_dictionary_cache(Oid dictId)
{
    ...
    if (u_sess->tscache_cxt.lastUsedDictionary && u_sess->tscache_cxt.lastUsedDictionary->dictId == dictId &&
        u_sess->tscache_cxt.lastUsedDictionary->isvalid && !u_sess->tscache_cxt.lastUsedDictionary->isJieba)
    ...
    if (entry == NULL || !entry->isvalid || entry->isJieba) {
        /*
         * If we didn't find one, we want to make one. But first look up the
         * object to be sure the OID is real.
         */
        ...
        if (OidIsValid(templ->tmplinit)) {
            ...
            if (templ->tmplinit == F_DJIEBA_INIT) {
                entry->isJieba = true;
                entry->dictData = get_bm25_dict(dictId, dictoptions);
            } else {
                entry->isJieba = false;
                /*
                 ...
                 */
                auto old_ctx = MemoryContextSwitchTo(entry->dictCtx);
