    ...
    if (shmid < 0) {
        ...
        ereport(FATAL,
            (errmsg("could not create shared memory segment: %m"),
                ...
                (errno == EINVAL) ? errhint("This error usually means that vexdb's request for a shared memory "
                                            "segment exceeded your kernel's SHMMAX parameter.  You can either "
                                            "reduce the request size or reconfigure the kernel with larger SHMMAX.  "
                                            "To reduce the request size (currently %lu bytes), reduce "
                                            "vexdb's shared memory usage, perhaps by reducing shared_buffers.\n"
                                            "If the request size is already small, it's possible that it is less than "
                                            "your kernel's SHMMIN parameter, in which case raising the request size or "
                                            "reconfiguring SHMMIN is called for.\n"
                                            "The vexdb documentation contains more information about shared "
                                            "memory configuration.", (unsigned long)size)
                    : 0,
                (errno == ENOMEM) ? errhint("This error usually means that vexdb's request for a shared "
                ...
                (errno == ENOSPC) ? errhint("This error does *not* mean that you have run out of disk space.  "
                                            ...
                                            "reduce vexdb's shared memory request (currently %lu bytes), "
