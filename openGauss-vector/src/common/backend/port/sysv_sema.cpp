        ...
        ereport(FATAL,
            (errmsg("could not create semaphores: %m"),
                ...
                (errno == ENOSPC) ? errhint("This error does *not* mean that you have run out of disk space.  "
                                            ...
                                            "respective kernel parameter.  Alternatively, reduce vexdb's "
                                            "consumption of semaphores by reducing its max_connections parameter.\n"
                                            "The vexdb documentation contains more information about "
                                            "configuring your system for vexdb.")
                    : 0));
    }
        ...
        ereport(FATAL,
            (errmsg_internal("semctl(%d, %d, SETVAL, %d) failed: %m", semId, semNum, value),
                (errno == ERANGE) ? errhint("You possibly need to raise your kernel's SEMVMX value to be at least "
                                            "%d.  Look into the vexdb documentation for details.", value)
                    : 0));
}
...
static IpcSemaphoreId IpcSemaphoreCreate(int numSems)
{
    ...
    for (nextSemaKey++;; nextSemaKey++) {
        ...
        /*
         * The sema set appears to be from a dead vexdb process, or from a
