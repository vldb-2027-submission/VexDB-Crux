...
function get_gs_version()
{
    ...
    product=$(cat build/script/vexdb.ver | grep 'PRODUCT' | awk -F "=" '{print $2}')
    version=$(cat build/script/vexdb.ver | grep 'VERSION' | awk -F "=" '{print $2}')
...
OPENGS_TOP_PATH=${TRUNK_CODE_PATH}/${vexdb}
