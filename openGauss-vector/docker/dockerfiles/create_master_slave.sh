...
#
# IDENTIFICATION
#    VexDBKernel/server/docker/dockerfiles/create_master_slave.sh
#
#-------------------------------------------------------------------------
...
read -p "Please input MASTER_NODENAME [vexdb_master]: " MASTER_NODENAME
MASTER_NODENAME=${MASTER_NODENAME:-vexdb_master}
echo "MASTER_NODENAME set $MASTER_NODENAME"

read -p "Please input SLAVE_NODENAME [vexdb_slave1]: " SLAVE_NODENAME
SLAVE_NODENAME=${SLAVE_NODENAME:-vexdb_slave1}
echo "SLAVE_NODENAME set $SLAVE_NODENAME"

read -p "Please input vexdb VERSION [5.0.0]: " VERSION
VERSION=${VERSION:-5.0.0}
echo "vexdb VERSION set $VERSION"

echo "starting  "

docker network create --subnet=$OG_SUBNET vexdbnetwork \
|| {
  echo ""
  echo "ERROR: vexdb Database Network was NOT successfully created."
  echo "HINT: vexdbnetwork Maybe Already Exsist Please Execute 'docker network rm vexdbnetwork' "
  exit 1
}
echo "vexdb Database Network Created."

docker run --network vexdbnetwork --ip $MASTER_IP --privileged=true \
...
vexdb:$VERSION -M primary \
|| {
  echo ""
  echo "ERROR: vexdb Database Master Docker Container was NOT successfully created."
  exit 1
}
echo "vexdb Database Master Docker Container created."

sleep 30s

docker run --network vexdbnetwork --ip $SLAVE_1_IP --privileged=true \
...
vexdb:$VERSION -M standby \
|| {
  echo ""
  echo "ERROR: vexdb Database Slave1 Docker Container was NOT successfully created."
  exit 1
}
echo "vexdb Database Slave1 Docker Container created."
