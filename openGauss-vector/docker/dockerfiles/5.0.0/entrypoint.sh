...
# used to create initial vexdb directories and if run as root, ensure ownership belong to the omm user
docker_create_db_directories() {
        ...
        if [ "$user" = '0' ]; then
                ...
                find /var/run/vexdb \! -user omm -exec chown omm '{}' +
        fi
}
...
#                eval 'gs_initdb --pwfile=<(echo "$GS_PASSWORD") --nodename=vexdb '"$POSTGRES_INITDB_ARGS"' "$@"'
                eval 'gs_initdb --pwfile=<(echo "$GS_PASSWORD") --nodename=vexdb -D $PGDATA'
        fi
                ...
                cat >&2 <<-'EOWARN'
                        ********************************************************************************
                        WARNING: GS_HOST_AUTH_METHOD has been set to "trust". This will allow
                                 anyone with access to the vexdb port to access your database without
...
docker_setup_db() {
        ...
        if [ "$GS_DB" != 'postgres' ]; then
                GS_DB= docker_process_sql --dbname postgres --set db="$GS_DB" --set passwd="$GS_PASSWORD" --set passwd="$GS_PASSWORD" <<-'EOSQL'
                        CREATE DATABASE :"db" ;
                        create user vexdb with login password :"passwd" ;
                        grant all privileges to vexdb;
...
vexdb_setup_hba_conf() {
        {
                ...
                echo "host replication vexdb 0.0.0.0/0 md5"
...
# check arguments for an option that would cause vexdb to stop
# return true if there is one
docker_setup_slot() {
cp /usr/local/vexdb/wal2json.so /usr/local/vexdb/lib/postgresql
                GS_DB= docker_process_sql --dbname postgres --set db="$GS_DB" --set passwd="$GS_PASSWORD" --set user="$GS_USERNAME" <<-'EOSQL'
                        select * from pg_create_logical_replication_slot('wal2json', 'wal2json');
                        create table vexdb.test (id int primary key, name varchar2(20));
                        insert into vexdb.test values(1,'yun');
                        insert into vexdb.test values(2,'he');
                        insert into vexdb.test values(3,'enmo');
                        ALTER TABLE vexdb.test REPLICA IDENTITY FULL;
EOSQL
}
...
_vexdb_want_help() {
        ...
        for arg; do
                ...
                if [ "$arg" == "-M" ]; then
                        ...
                        echo "vexdb DB SERVER_MODE = $SERVER_MODE"
                        shift
                fi
                count=$[$count + 1]
        done
...
_main() {
        ...
        if [ "$1" = 'vexdb' ] && ! _vexdb_want_help "$@"; then
                docker_setup_env
...
#                        docker_setup_slot
                        ...
                        echo
                        echo 'vexdb  init process complete; ready for start up.'
                        echo
                else
                        echo
                        echo 'vexdb Database directory appears to contain a database; Skipping initialization'
                        echo
                fi
        fi
        exec "$@"
