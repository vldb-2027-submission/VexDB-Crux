...
Arguments:
   ...
   -D                          vexdb data dir, default /var/lib/vexdb/data
   -B                          upgrade root dir, default /var/lib/vexdb/data/upgrade
   -N                          new package dir, default /var/lib/vexdb/data/upgrade/pkg_new
...
function check_db_process() {
  #ps wwx | grep "$GAUSSHOME/bin/vexdb" | grep -v grep > /dev/null
 ps -ef|grep vexdb | grep -v grep > /dev/null
}
...
function check_pkg() {
  ...
  bin_name="vexdb-3.0.1-CentOS-64bit.tar.bz2"
  binfile="vexdb-3.0.1-CentOS-64bit.tar.bz2"
  shafile="vexdb-3.0.1-CentOS-64bit.sha256"
...
function start_dbnode() {
  ...
  log "start vexdb by cmd: $start_cmd $start_option"
...
function rollback_pre() {
  ...
  if [[ "$current_step" -lt 1 ]]; then
    ...
    if [[ "$big_cfg" == "True" ]]; then
      if ! check_db_process; then
        die " Vexdb is not running" ${err_rollback_pre}
      fi
...
function rollback_bin() {
  ...
  if [[ "$current_step" -lt 3 ]]; then
    ...
    record_step 3
    if ! stop_dbnode; then
      die "Stop vexdb failed" ${err_rollback_bin}
    fi
...
function rollback_post() {
  ...
  if [[ "$current_step" -lt 5 ]]; then
    ...
    if [[ "$big_cfg" == "True" ]]; then
      if ! check_db_process; then
        die " Vexdb is not running" ${err_rollback_post}
      fi
      if ! reload_upgrade_config upgrade_mode 2; then
...
function upgrade_pre_step2() {
  ...
  if [[ "$big_cfg" == "True" ]]; then
    if ! check_db_process; then
      die " Vexdb is not running" ${err_upgrade_pre}
    fi
...
function upgrade_bin_step4() {
  ...
  if [[ "$big_cfg" == "True" ]]; then
    ...
    if ! start_dbnode "$old_cfg"; then
      die "Start vexdb failed" ${err_upgrade_bin}
    fi
  fi
  record_step 4
...
function query_start_mode() {
  ...
  if grep -Ewq "new_version=\w{8}" "$GAUSS_TMP_PATH"/version_flag ; then
    new_version=$(grep new_version "$GAUSS_TMP_PATH"/version_flag | awk -F= '{print $2}')
    if vexdb -V|grep -wq "$new_version";then
