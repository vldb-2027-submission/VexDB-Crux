...
function install_vexdb()
{
    ...
    #configure
    make distclean -sj 6 >> "$LOG_FILE" 2>&1

    echo "Begin configure." >> "$LOG_FILE" 2>&1
    chmod 755 configure

    if [ "$product_mode"x == "vexdb"x ]; then
    ...
    if [ "$build_with_license"x == "YES"x ]; then
        with_license="--with-license"
    else
        with_license=""
    fi
    
    shared_opt="--gcc-version=${gcc_version}.${gcc_sub_version} --prefix="${BUILD_DIR}" --3rd=${binarylib_dir} --enable-thread-safety ${enable_readline} ${with_tassl} ${with_license} --without-zlib"
    if [ "$product_mode"x == "vexdb"x ]; then
        if [ "$version_mode"x == "release"x ]; then
            # configure -D__USE_NUMA -D__ARM_LSE with arm vexdb mode
            if [ "$PLATFORM_ARCH"X == "aarch64"X ] ; then
                echo "configure -D__USE_NUMA -D__ARM_LSE with arm vexdb mode"
        ...
        shared_opt="--gcc-version=${gcc_version}.${gcc_sub_version} --prefix="${BUILD_DIR}" --3rd=${binarylib_dir} --enable-thread-safety ${enable_readline}  ${with_tassl} ${with_license} --without-zlib  --without-gssapi --without-krb5"
        if [ "$version_mode"x == "release"x ]; then
    ...
    make clean >> "$LOG_FILE" 2>&1
    make -sj 6 >> "$LOG_FILE" 2>&1
    make install -sj 6 >> "$LOG_FILE" 2>&1
