...
function target_file_copy()
{
    ...
    ln -s "${kernel_package_name}" "${ogkernel_package_name}" >> "$LOG_FILE" 2>&1
    ...
    ln -s "${sha256_name}" "${ogsha256_name}"  >> "$LOG_FILE" 2>&1
    if [ -d "${2}" ]; then
        rm -rf ${2}
    fi
}
...
function make_package_srv()
{
    ...
    # pkg install uninstall scripts:install.sh, uninstall.sh, vexdb_lite.conf, upgrade_errorcode.sh
    for filename in install.sh uninstall.sh vexdb_lite.conf
    do
    ...
    chmod 600 ./vexdb_lite.conf upgrade_config.sh
    ...
    cp ${BUILD_DIR}/"${ogkernel_package_name}" ./
    cp ${BUILD_DIR}/"${ogsha256_name}" ./
...
function vexdb_pkg()
{
    echo "Start package vexdb."
    separate_symbol
    make_package_srv
    make_package_libpq
    echo "End package vexdb."
}