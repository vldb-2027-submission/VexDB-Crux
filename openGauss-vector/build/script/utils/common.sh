...
function log()
{
    echo "[makevexdb] $(date +%y-%m-%d' '%T): $@"
    echo "[makevexdb] $(date +%y-%m-%d' '%T): $@" >> "$LOG_FILE" 2>&1
}
...
vexdb_200_file="${BUILD_TOOLS_PATH}/license_control/vexdb.version.VexDB200"
vexdb_300_file="${BUILD_TOOLS_PATH}/license_control/vexdb.version.VexDB300"
vexdb_200_standard_file="${BUILD_TOOLS_PATH}/license_control/vexdb.license.VexDB200_Standard"
vexdb_version_file="${ROOT_DIR}/src/gausskernel/process/postmaster/vexdb_version.cpp"


if [ -f "$SCRIPT_DIR/vexdb.ver" ];then
    declare version_number=$(cat ${SCRIPT_DIR}/vexdb.ver | grep 'VERSION' | awk -F "=" '{print $2}')
else
    echo "vexdb.ver not found!"
    exit 1
fi
...
declare ogkernel_package_name="${kernel_package_name/vexdb/openGauss}"
declare ogsha256_name="${sha256_name/vexdb/openGauss}"
