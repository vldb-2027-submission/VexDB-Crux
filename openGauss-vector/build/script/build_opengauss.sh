...
# descript: Compile vexdb

# It is just a wrapper to package_internal.sh
# Example: ./build_vexdb.sh -3rd path/third_party_binarylibs/
...
function print_help()
{
    ...
    -pm                    product mode, values parameter is  vexdb.
...
{
    echo $(cat ${SCRIPT_DIR}/vexdb.ver | grep 'VERSION' | awk -F "=" '{print $2}')
}
...
function main()
{
    echo "[makevexdb] $(date +%y-%m-%d' '%T): script dir : ${SCRIPT_DIR}"
    echo "[makevexdb] $(date +%y-%m-%d' '%T): Work root dir : ${ROOT_DIR}"
    read_vexdb_version
    read_vexdb_number
    vexdb_pkg_pre_clean
    vexdb_build
