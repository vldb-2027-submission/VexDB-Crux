...
declare mppdb_version='vexdb Lite'
...
function print_help()
{
    ...
    -pm                    product mode, values parameter is single, multiple or vexdb, default value is multiple.
...
function install_vexdb()
{
    ...
    echo "Begin run cmake for vexdb server" >> "$LOG_FILE" 2>&1
    ...
    cd ${CMAKE_BUILD_DIR}
    cmake .. ${CMAKE_OPT}
    echo "Begin make and install vexdb server" >> "$LOG_FILE" 2>&1
    ...
    spec="vexdbkernel"
    if ( cat $SCRIPT_DIR/gauss.spec | grep 'PRODUCT' | grep 'VexDB Kernel' >/dev/null 2>&1 ); then
        spec="vexdbkernel"
    elif ( cat $SCRIPT_DIR/gauss.spec | grep 'PRODUCT' | grep 'vexdb' >/dev/null 2>&1 ); then
        spec="vexdb"
    fi
    ...
    commitid=$(LD_PRELOAD=''  ${BUILD_DIR}/bin/vexdb -V | cut -d ")" -f 1 | awk '{print $NF}')
...
function make_package()
{
    ...
    case "$pkgname" in
        server)
            ...
            cp ${ROOT_DIR}/${open_gauss}/liteom/vexdb_lite.conf ./
            if [ $? -ne 0 ]
            then
                die "copy ${ROOT_DIR}/${open_gauss}/liteom/vexdb_lite.conf to ${BUILD_DIR}/temp failed"
            fi
